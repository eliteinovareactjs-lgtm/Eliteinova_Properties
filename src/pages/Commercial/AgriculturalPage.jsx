...
import AgriculturalFilter from "../../components/filters/Commercial/AgriculturalFilter";
import Agricultural from "../../components/propertycard/Commercial/Agricultural";

const AgriculturalPage = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const [activeButton, setActiveButton] = useState("Rent");
  const [activeAgriculturalType, setActiveAgriculturalType] = useState("Agricultural");
  ...
  useEffect(() => {
    ...
    if (activeType) {
      setActiveAgriculturalType(activeType.name);
    } else if (currentPath === "/commercial" || currentPath === "/commercial/") {
      setActiveAgriculturalType("All");
    }
  }, [location.pathname]);

  const handleNavigation = (path, typeName = null) => {
    if (typeName) setActiveAgriculturalType(typeName);
    navigate(path);
  };
  ...
  const CategoryMarquee = ({ mobile = false }) => {
    ...
    const isAllActive = activeAgriculturalType === "All";
        ...
        <div className="marquee-container group/marquee overflow-hidden flex-1 min-w-0">
          <div className="marquee-track flex items-center gap-3.5 md:gap-5 w-max group-hover/marquee:[animation-play-state:paused]">
            {duplicated.map((category, index) => {
              const isActive = activeAgriculturalType === category.name;
