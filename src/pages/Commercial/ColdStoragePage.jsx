...
import { ChevronDown, Search, Home, MapPin, Star, Filter, X, Building, Landmark, Warehouse, Building2, Briefcase, Hotel, LayoutGrid, Grid3X3, Crown, Castle, Building as BuildingIcon } from "lucide-react";
...
import ColdStorageFilter from "../../components/filters/Commercial/ColdStorageFilter";
import ColdStorage from "../../components/propertycard/Commercial/ColdStorage";

const ColdStoragePage = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const [activeButton, setActiveButton] = useState("Rent");
  const [activeColdStorageType, setActiveColdStorageType] = useState("ColdStorage");
  ...
  // The pinned "All" category (stays fixed on the left) — with an image
  const allCategory = {
    name: "All",
    path: "/commercial",
    image: allImg,
  ...
  useEffect(() => {
    ...
    const activeType = ColdStorageTypes.find(type => type.path === currentPath);
    if (activeType) {
      setActiveColdStorageType(activeType.name);
    } else if (currentPath === "/commercial" || currentPath === "/commercial/") {
      setActiveColdStorageType("All");
    }
  }, [location.pathname]);

  const handleNavigation = (path, typeName = null) => {
    if (typeName) setActiveColdStorageType(typeName);
    navigate(path);
  };
  ...
  const SearchBar = () => (
    <div className="relative flex-1 group">
      ...
      <input
        type="text"
        placeholder="Search cold storages by city, locality, or business park"
  ...
  const CategoryMarquee = ({ mobile = false }) => {
    ...
    const isAllActive = activeColdStorageType === "All";
        ...
        <div className="marquee-container group/marquee overflow-hidden flex-1 min-w-0">
          <div className="marquee-track flex items-center gap-3.5 md:gap-5 w-max group-hover/marquee:[animation-play-state:paused]">
            {duplicated.map((category, index) => {
              const isActive = activeColdStorageType === category.name;
  ...
  return (
    <div className="w-full min-h-screen relative bg-gradient-to-b from-teal-50 via-white to-teal-50">
      <div className="relative z-10">
            ...
            <div className="flex flex-row min-h-[170px] sm:min-h-[220px] md:min-h-[280px] lg:min-h-[330px]">
              ...
              <div className="flex flex-col justify-center w-[38%] sm:w-[37%] md:w-[36%] lg:w-[35%] shrink-0 px-2.5 sm:px-5 md:px-6 lg:px-10 py-2.5 sm:py-4 md:py-6 lg:py-7 z-20">
                ...
                <p className="mt-1 sm:mt-2 md:mt-2.5 lg:mt-3 max-w-[120px] sm:max-w-[200px] md:max-w-[280px] lg:max-w-[340px] text-[#31544E] text-[8px] sm:text-[10px] md:text-xs lg:text-sm leading-snug lg:leading-relaxed">
                  Discover modern cold storage spaces with world-class
        ...
        {/* ══════════════════════════════════════════════
            STICKY NAVBAR  —  MOBILE (SAME AS APARTMENT PAGE)
        ══════════════════════════════════════════════ */}
