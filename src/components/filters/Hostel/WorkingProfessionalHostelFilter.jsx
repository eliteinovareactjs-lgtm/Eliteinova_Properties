// WorkingProfessionalHostelFilter.jsx
...
import {
  ...
  Briefcase, Monitor, Printer, Laptop, Coffee as CoffeeIcon,
  Calendar as CalendarIcon, Building as BuildingIcon,
  Music, Gamepad, Globe, PartyPopper, Briefcase as BriefcaseIcon,
  Car as CarIcon, Bike, Bus as BusIcon, Train, Clock as ClockIcon
} from 'lucide-react';
...
// Main Component
const WorkingProfessionalHostelFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    amenities: [],
    professionalAmenities: [],
    foodIncluded: '', foodType: '', mealsPerDay: '', kitchenAccess: '',
    security24x7: '', cctv: '', securityGuard: '', curfewTime: '', visitorPolicy: '', idProofRequired: '',
    immediateOccupancy: '', availableFrom: '', minStayDuration: '',
    immediatePossession: '', readyToBuy: '', underConstruction: '',
    contactOwner: false, contactAgent: false, contactManager: false, preferredContactTime: '',
    proximityToOffice: '', transportAccess: '', parkingAvailable: '', quietHours: ''
  });

  // Options Arrays
  const sharingOptions = [
    { value: 'Single', label: 'Single Executive Room', icon: '👤' },
    { value: 'Double', label: 'Double Sharing', icon: '👥' },
    { value: 'Triple', label: 'Triple Sharing', icon: '👤👤👤' },
  ...
  const curfewOptions = [
    ...
    { value: '11pm', label: '🌙 Before 11:00 PM' },
  ...
  const proximityOptions = [
    { value: 'within1km', label: '📏 Within 1 km' },
    { value: 'within3km', label: '📏 Within 3 km' },
    { value: 'within5km', label: '📏 Within 5 km' },
    { value: 'within10km', label: '📏 Within 10 km' }
  ];

  const transportOptions = [
    { value: 'Metro', label: '🚇 Metro Station Nearby' },
    { value: 'Bus', label: '🚌 Bus Stop Nearby' },
    { value: 'Railway', label: '🚆 Railway Station Nearby' },
    { value: 'Airport', label: '✈️ Airport Nearby' },
    { value: 'All', label: '🚍 All Transport Options' }
  ];

  const parkingOptions = [
    { value: 'Free', label: '🆓 Free Parking' },
    { value: 'Paid', label: '💰 Paid Parking' },
    { value: 'Valet', label: '🧑‍✈️ Valet Parking' },
    { value: 'None', label: '🚫 No Parking' }
  ];

  const quietHoursOptions = [
    { value: 'Strict', label: '🔇 Strict Quiet Hours' },
    { value: 'Moderate', label: '🔉 Moderate Quiet Hours' },
  ...
  const professionalAmenityOptions = [
    { value: 'Work Desk', label: 'Work Desk', icon: <Monitor className="w-4 h-4" />, color: 'from-blue-400 to-blue-600' },
    { value: 'Ergonomic Chair', label: 'Ergonomic Chair', icon: <Sofa className="w-4 h-4" />, color: 'from-green-400 to-green-600' },
    { value: 'Printer Access', label: 'Printer Access', icon: <Printer className="w-4 h-4" />, color: 'from-purple-400 to-purple-600' },
    { value: 'Conference Room', label: 'Conference Room', icon: <BuildingIcon className="w-4 h-4" />, color: 'from-indigo-400 to-indigo-600' },
    { value: 'High-speed Internet', label: 'High-speed Internet', icon: <Wifi className="w-4 h-4" />, color: 'from-cyan-400 to-cyan-600' },
    { value: 'Business Center', label: 'Business Center', icon: <BriefcaseIcon className="w-4 h-4" />, color: 'from-orange-400 to-orange-600' },
    { value: 'Coffee Machine', label: 'Coffee Machine', icon: <CoffeeIcon className="w-4 h-4" />, color: 'from-red-400 to-red-600' },
    { value: 'Meeting Rooms', label: 'Meeting Rooms', icon: <Users className="w-4 h-4" />, color: 'from-teal-400 to-teal-600' }
  ];
  ...
  const sections = [
    ...
    { id: 'professional', label: 'Work Amenities', icon: '💼' },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Working Professional Hostel' };
  ...
  const renderBasicDetails = () => (
    <div className="space-y-3">
      <div>
        ...
        <input type="text" value="Working Professional Hostel" disabled className="w-full px-3 py-2 rounded-xl border-2 border-teal-200 bg-gray-50/80 text-sm text-gray-600" />
      </div>
      <div>
      ...
      <div>
        ...
        <div className="flex gap-3 flex-wrap">
          {['Owner', 'Agent', 'Property Manager'].map(type => (
  ...
  const renderLocationDetails = () => (
    <div className="space-y-3">
      ...
      <div>
        <label className="text-xs text-teal-800 font-semibold block mb-1">Distance From Workplace</label>
        <input type="text" placeholder="e.g. within 2 km of office" className="w-full px-3 py-2 rounded-xl border-2 border-teal-200 bg-white/90 text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/30" value={filters.distanceFrom} onChange={(e) => handleInputChange('distanceFrom', e.target.value)} />
      </div>
    </div>
  );
  ...
  const renderProfessionalAmenities = () => (
    <div className="space-y-3">
      <div>
        <label className="text-xs text-teal-800 font-semibold block mb-2">Work Amenities</label>
        <div className="grid grid-cols-2 gap-2">
          {professionalAmenityOptions.map((a) => (
            <label key={a.value} className="flex items-center gap-2 p-2.5 rounded-xl bg-white border-2 border-teal-200 hover:border-teal-500 cursor-pointer transition-all duration-300 hover:shadow-md">
              <input type="checkbox" checked={filters.professionalAmenities.includes(a.value)} onChange={() => handleArrayToggle('professionalAmenities', a.value)} className="w-4 h-4 rounded-lg border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-2 focus:ring-teal-500/30 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className={`text-teal-600 bg-gradient-to-r ${a.color} bg-clip-text text-transparent`}>{a.icon}</span>
              <span className="text-xs text-teal-700 font-medium">{a.label}</span>
            </label>
          ))}
      ...
      <div>
        <label className="text-xs text-teal-800 font-semibold block mb-2">Proximity to Office</label>
        <div className="flex gap-2 flex-wrap">
          {proximityOptions.map((p) => (
            <label key={p.value} className="flex items-center gap-2 cursor-pointer group">
              <input type="radio" name="proximityToOffice" value={p.value} checked={filters.proximityToOffice === p.value} onChange={(e) => handleRadioChange('proximityToOffice', e.target.value)} className="w-4 h-4 border-2 border-teal-300 text-teal-600 focus:ring-2 focus:ring-teal-500/30 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600 font-medium">{p.label}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      <div>
        <label className="text-xs text-teal-800 font-semibold block mb-2">Transport Access</label>
        <div className="flex flex-wrap gap-2">
          {transportOptions.map((t) => (
            <label key={t.value} className="flex items-center gap-2 cursor-pointer group">
              <input type="checkbox" checked={filters.transportAccess.includes(t.value)} onChange={() => handleArrayToggle('transportAccess', t.value)} className="w-4 h-4 rounded-lg border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-2 focus:ring-teal-500/30 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600 font-medium">{t.label}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs text-teal-800 font-semibold block mb-2">Parking Available</label>
          <div className="flex flex-wrap gap-2">
            {parkingOptions.map((p) => (
              <label key={p.value} className="flex items-center gap-2 cursor-pointer group">
                <input type="radio" name="parkingAvailable" value={p.value} checked={filters.parkingAvailable === p.value} onChange={(e) => handleRadioChange('parkingAvailable', e.target.value)} className="w-4 h-4 border-2 border-teal-300 text-teal-600 focus:ring-2 focus:ring-teal-500/30 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                <span className="text-xs text-teal-700 group-hover:text-teal-600 font-medium">{p.label}</span>
              </label>
            ))}
          </div>
        ...
        <div>
          <label className="text-xs text-teal-800 font-semibold block mb-2">Quiet Hours</label>
          <div className="flex flex-wrap gap-2">
            {quietHoursOptions.map((q) => (
              <label key={q.value} className="flex items-center gap-2 cursor-pointer group">
                <input type="radio" name="quietHours" value={q.value} checked={filters.quietHours === q.value} onChange={(e) => handleRadioChange('quietHours', e.target.value)} className="w-4 h-4 border-2 border-teal-300 text-teal-600 focus:ring-2 focus:ring-teal-500/30 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                <span className="text-xs text-teal-700 group-hover:text-teal-600 font-medium">{q.label}</span>
              </label>
            ))}
          </div>
        </div>
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          { key: 'securityGuard', label: 'Security Guard', icon: <UserCheck className="w-4 h-4" /> },
  ...
  const renderContactPreference = () => (
    <div className="space-y-3">
      <div>
        ...
        <div className="flex gap-3 flex-wrap">
          {['Owner', 'Agent', 'Property Manager'].map(type => {
            const field = `contact${type.replace(/\s/g, '')}`;
            return (
              <label key={type} className="flex items-center gap-2 cursor-pointer group">
    ...
    <div className="bg-white rounded-3xl shadow-2xl border-2 border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '90vh' }}>
      ...
      <div className="sticky top-0 z-10 bg-white border-b-2 border-teal-100">
        <div className="flex justify-between items-center px-6 py-4 bg-gradient-to-r from-teal-600 to-emerald-600">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-white font-semibold text-sm md:text-base">Find Your Perfect Professional Hostel</h3>
      ...
      <div className="overflow-y-auto flex-1 p-4 custom-scroll" style={{ maxHeight: 'calc(90vh - 120px)' }}>
        <div className="space-y-1">
          ...
          <Section id="professional" label="Work Amenities" icon="💼">
            {renderProfessionalAmenities()}
          </Section>
...
export default WorkingProfessionalHostelFilter;