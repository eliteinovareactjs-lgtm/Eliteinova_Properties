...
import {
  ...
  PartyPopper, Music, Gamepad, Globe
} from 'lucide-react';
...
// Main Component
const CoLivingSpaceFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    security24x7: '', cctv: '', communityManager: '', curfewTime: '', visitorPolicy: '', idProofRequired: '',
    ...
    eventsAndActivities: [], petFriendly: '', workFromHomeFriendly: ''
  });
  ...
  const communityAmenityOptions = [
    { value: 'Community Kitchen', label: 'Community Kitchen', icon: <CookingPot className="w-4 h-4" />, color: 'from-orange-400 to-orange-600' },
    { value: 'Rooftop Terrace', label: 'Rooftop Terrace', icon: <Sun className="w-4 h-4" />, color: 'from-yellow-400 to-yellow-600' },
    { value: 'Game Zone', label: 'Game Zone', icon: <Gamepad className="w-4 h-4" />, color: 'from-green-400 to-green-600' },
    { value: 'Co-working Space', label: 'Co-working Space', icon: <Globe className="w-4 h-4" />, color: 'from-blue-400 to-blue-600' },
    { value: 'Event Space', label: 'Event Space', icon: <PartyPopper className="w-4 h-4" />, color: 'from-pink-400 to-pink-600' },
    { value: 'Music Room', label: 'Music Room', icon: <Music className="w-4 h-4" />, color: 'from-purple-400 to-purple-600' }
  ];

  const eventOptions = [
    { value: 'Weekly Dinners', label: '🍽️ Weekly Dinners' },
    { value: 'Movie Nights', label: '🎬 Movie Nights' },
    { value: 'Game Tournaments', label: '🎮 Game Tournaments' },
    { value: 'Workshops', label: '📚 Workshops' },
    { value: 'Fitness Sessions', label: '💪 Fitness Sessions' },
  ...
  const sections = [
    ...
    { id: 'community', label: 'Community Features', icon: '🤝' },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Co-Living Space' };
  ...
  const renderBasicDetails = () => (
    <div className="space-y-3">
      <div>
        ...
        <input type="text" value="Co-Living Space" disabled className="w-full px-3 py-2 rounded-xl border-2 border-teal-200 bg-gray-50/80 text-sm text-gray-600" />
      </div>
      <div>
      ...
      <div>
        ...
        <div className="flex gap-3 flex-wrap">
          {['Owner', 'Agent', 'Community Manager'].map(type => (
  ...
  const renderLocationDetails = () => (
    <div className="space-y-3">
      ...
      <div>
        ...
        <input type="text" placeholder="e.g. within 2 km of workplace" className="w-full px-3 py-2 rounded-xl border-2 border-teal-200 bg-white/90 text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/30" value={filters.distanceFrom} onChange={(e) => handleInputChange('distanceFrom', e.target.value)} />
      </div>
    </div>
  );
  ...
  const renderCommunityFeatures = () => (
    <div className="space-y-3">
      <div>
        <label className="text-xs text-teal-800 font-semibold block mb-2">Community Amenities</label>
        <div className="grid grid-cols-2 gap-2">
          {communityAmenityOptions.map((a) => (
            <label key={a.value} className="flex items-center gap-2 p-2.5 rounded-xl bg-white border-2 border-teal-200 hover:border-teal-500 cursor-pointer transition-all duration-300 hover:shadow-md">
              <input type="checkbox" checked={filters.communityAmenities.includes(a.value)} onChange={() => handleArrayToggle('communityAmenities', a.value)} className="w-4 h-4 rounded-lg border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-2 focus:ring-teal-500/30 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className={`text-teal-600 bg-gradient-to-r ${a.color} bg-clip-text text-transparent`}>{a.icon}</span>
              <span className="text-xs text-teal-700 font-medium">{a.label}</span>
            </label>
          ))}
      ...
      <div>
        <label className="text-xs text-teal-800 font-semibold block mb-2">Events & Activities</label>
        <div className="grid grid-cols-2 gap-2">
          {eventOptions.map((e) => (
            <label key={e.value} className="flex items-center gap-2 p-2.5 rounded-xl bg-white border-2 border-teal-200 hover:border-teal-500 cursor-pointer transition-all duration-300 hover:shadow-md">
              <input type="checkbox" checked={filters.eventsAndActivities.includes(e.value)} onChange={() => handleArrayToggle('eventsAndActivities', e.value)} className="w-4 h-4 rounded-lg border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-2 focus:ring-teal-500/30 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 font-medium">{e.label}</span>
            </label>
          ))}
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs text-teal-800 font-semibold block mb-2">Pet Friendly</label>
          <div className="flex gap-4">
            {['Yes', 'No'].map(option => (
              <label key={option} className="flex items-center gap-2 cursor-pointer group">
                <input type="radio" name="petFriendly" value={option} checked={filters.petFriendly === option} onChange={(e) => handleRadioChange('petFriendly', e.target.value)} className="w-4 h-4 border-2 border-teal-300 text-teal-600 focus:ring-2 focus:ring-teal-500/30 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                <span className="text-sm text-teal-700 group-hover:text-teal-600 font-medium">{option}</span>
              </label>
            ))}
          </div>
        ...
        <div>
          <label className="text-xs text-teal-800 font-semibold block mb-2">Work From Home Friendly</label>
          <div className="flex gap-4">
            {['Yes', 'No'].map(option => (
              <label key={option} className="flex items-center gap-2 cursor-pointer group">
                <input type="radio" name="workFromHomeFriendly" value={option} checked={filters.workFromHomeFriendly === option} onChange={(e) => handleRadioChange('workFromHomeFriendly', e.target.value)} className="w-4 h-4 border-2 border-teal-300 text-teal-600 focus:ring-2 focus:ring-teal-500/30 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                <span className="text-sm text-teal-700 group-hover:text-teal-600 font-medium">{option}</span>
              </label>
            ))}
          </div>
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          { key: 'communityManager', label: 'Community Manager', icon: <UserCheck className="w-4 h-4" /> },
    ...
    <div className="bg-white rounded-3xl shadow-2xl border-2 border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '90vh' }}>
      ...
      <div className="sticky top-0 z-10 bg-white border-b-2 border-teal-100">
        <div className="flex justify-between items-center px-6 py-4 bg-gradient-to-r from-teal-600 to-emerald-600">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-white font-semibold text-sm md:text-base">Find Your Perfect Co-Living Space</h3>
      ...
      <div className="overflow-y-auto flex-1 p-4 custom-scroll" style={{ maxHeight: 'calc(90vh - 120px)' }}>
        <div className="space-y-1">
          ...
          <Section id="community" label="Community Features" icon="🤝">
...
export default CoLivingSpaceFilter;