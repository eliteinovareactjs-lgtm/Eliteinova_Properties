...
import {
  ...
  Warehouse, PawPrint, Flower2, Recycle, Apple, Milk
} from 'lucide-react';
...
const CoconutLandAreaUnitSelect = ({ value, onChange }) => {
  const [isOpen, setIsOpen] = useState(false);
  const selectRef = useRef(null);
  const units = [
...
const CoconutFarmLandFilter = ({ activeTab = 'Lease', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    // Land Details
    landArea: '', landAreaUnit: 'acres', numberOfTrees: '', treeAge: '', yieldCapacity: '', soilType: '', waterSource: '', irrigationFacility: '', facing: '', cornerLand: '', roadWidth: '', boundaryAvailable: '',
  ...
  const farmSuitabilityOptions = [
    'Coconut Farming', 'Tender Coconut Business', 'Organic Coconut Farming',
    'Copra Production', 'Coconut Oil Production', 'Intercropping Farming',
    'Farm Stay / Eco Tourism'
  ];
  ...
  const nearbyOptions = [
    ...
    'Coconut Processing Unit Nearby', 'Transport Access'
  ];
  ...
  const mainSections = [
    ...
    { id: 'land', label: '🌴 Coconut Farm', icon: <Trees className="w-3.5 h-3.5" /> },
    { id: 'farmSuitability', label: '🥥 Suitability', icon: <Apple className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Coconut Farm Land' };
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Coconut Farm Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
  ...
  const renderLandSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Trees className="w-3.5 h-3.5" /> Coconut Farm Details</h3>
      <div className="space-y-2">
        <div className="grid grid-cols-4 gap-2 items-end">
          ...
          <CoconutLandAreaUnitSelect value={filters.landAreaUnit} onChange={(val) => handleInputChange('landAreaUnit', val)} />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <input type="number" placeholder="Number of Coconut Trees" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.numberOfTrees} onChange={(e) => handleInputChange('numberOfTrees', e.target.value)} />
          <input type="text" placeholder="Coconut Tree Age (years)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.treeAge} onChange={(e) => handleInputChange('treeAge', e.target.value)} />
          <input type="text" placeholder="Yield Capacity (per month/year)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.yieldCapacity} onChange={(e) => handleInputChange('yieldCapacity', e.target.value)} />
  ...
  const renderFarmSuitabilitySection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Apple className="w-3.5 h-3.5" /> Farm Suitability</h3>
        ...
        <div className="sticky top-0 z-10 bg-white border-b border-teal-100">
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-white font-semibold text-sm">Filter Coconut Farm Land</h3>
            </div>
            {onClose && (
...
export default CoconutFarmLandFilter;
