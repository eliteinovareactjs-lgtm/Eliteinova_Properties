...
const TownshipDevelopmentLandFilter = ({ activeTab = 'Rent', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    // Township Development Suitability
    townshipSuitability: [],
    ...
    landConversionApproved: '', environmentalClearance: '',
    encumbranceFree: '', loanEligible: '', landSurveyCompleted: '', titleDeedVerified: '',
    leaseAgreementAvailable: '',
    // Availability
    immediateLeaseAvailable: '', readyToRegister: '', immediatePossession: '', vacantLandAvailable: '',
    approvedLayoutAvailable: '', readyForTownshipDevelopment: '', availableFrom: '',
    longTermLeaseAvailable: '', shortTermLeaseAvailable: '',
    // Nearby Access
    nearbyAccess: [],
    ...
    buyerPreferences: [],
    // Contact
    contactOwner: false, contactAgent: false, contactDeveloper: false, contactLandowner: false, preferredContactTime: ''
  });
  ...
  const townshipSuitabilityOptions = [
    'Residential Township', 'Integrated Township', 'Gated Community',
    'Villa Development', 'Apartment Development', 'Mixed-Use Township',
    'Smart City Project', 'Affordable Housing Project', 'Luxury Township Development',
    'Senior Living Community', 'Educational Township', 'Healthcare Township'
  ];

  const infrastructureUtilitiesOptions = [
    'Water Connection Available', 'Borewell Facility', 'Electricity Connection Available',
    '3-Phase Power Supply', 'Sewage Connection Available', 'Drainage System Available',
    'Storm Water Drain Available', 'Internet / Fiber Connectivity',
    'Internal Roads Available', 'Street Lighting Available'
  ];

  const amenitiesOptions = [
    'Gated Property', 'Security Available', 'CCTV Surveillance',
    'Landscaped Area', 'Clubhouse Space', 'Park / Open Space Available',
    'Children\'s Play Area', 'Parking Area Available', 'Community Facility Space'
  ];

  const nearbyOptions = [
    'National Highway', 'State Highway', 'Bus Stand', 'Railway Station',
    'Metro Station', 'Airport', 'School / College', 'Hospital',
    'Shopping Mall', 'IT Park', 'Industrial Area', 'Commercial Hub'
  ];

  const developmentPotentialOptions = [
    'Smart City Zone', 'Growth Corridor', 'Residential Expansion Zone',
    'Mixed-Use Development Zone', 'Government Approved Township Zone',
    'High Appreciation Potential'
  ];

  const tenantPreferencesOptions = [
    'Real Estate Developer', 'Township Developer', 'Builder',
    'Housing Board', 'Corporate Developer', 'Infrastructure Company',
    'Investment Group', 'Joint Development Partner'
  ];
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Township Development Land' };
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Township Development Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex flex-wrap gap-3">
              {['Owner', 'Agent', 'Developer', 'Landowner'].map(type => (
  ...
  const renderTownshipSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><CityIcon className="w-3.5 h-3.5" /> Township Development Suitability</h3>
      <div className="grid grid-cols-2 gap-1.5">
        {townshipSuitabilityOptions.map(option => (
          <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
            <input
              type="checkbox"
              checked={filters.townshipSuitability.includes(option)}
              onChange={() => handleArrayToggle('townshipSuitability', option)}
              className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
            />
  ...
  const renderLegalSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <YesNoRadioGroup label="Corporation / Municipality Approved" name="corporationApproved" value={filters.corporationApproved} onChange={(val) => handleRadioChange('corporationApproved', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Approved Layout Available" name="approvedLayoutAvailable" value={filters.approvedLayoutAvailable} onChange={(val) => handleRadioChange('approvedLayoutAvailable', val)} />
            <YesNoRadioGroup label="Ready for Township Development" name="readyForTownshipDevelopment" value={filters.readyForTownshipDevelopment} onChange={(val) => handleRadioChange('readyForTownshipDevelopment', val)} />
            <YesNoRadioGroup label="Long-Term Lease Available" name="longTermLeaseAvailable" value={filters.longTermLeaseAvailable} onChange={(val) => handleRadioChange('longTermLeaseAvailable', val)} />
            <YesNoRadioGroup label="Short-Term Lease Available" name="shortTermLeaseAvailable" value={filters.shortTermLeaseAvailable} onChange={(val) => handleRadioChange('shortTermLeaseAvailable', val)} />
            <CustomDatePicker label="Available From Date" value={filters.availableFrom} onChange={(val) => handleInputChange('availableFrom', val)} />
            {filters.longTermLeaseAvailable === 'Yes' && (
              <CustomSelect label="Long-Term Lease Duration" options={longTermLeaseOptions} value={filters.leaseDuration} onChange={(val) => handleInputChange('leaseDuration', val)} placeholder="Select Duration" />
            )}
          </div>
        </div>
      );
    ...
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        <div className="flex flex-wrap gap-3">
          ...
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <input type="checkbox" checked={filters.contactLandowner} onChange={(e) => handleCheckboxChange('contactLandowner', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
            <span className="text-xs text-gray-700 font-medium group-hover:text-teal-500">Contact Landowner</span>
          </label>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
        <div className="sticky top-0 z-10 bg-white border-b border-teal-100">
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-white font-semibold text-sm">Filter Township Development Land</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'township' && renderTownshipSection()}
