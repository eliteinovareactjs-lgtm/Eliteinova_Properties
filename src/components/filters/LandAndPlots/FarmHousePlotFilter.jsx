...
import { 
  ...
  Square, Home, Bus, CheckCircle, TrendingUp, Clock, Trees,
  Droplets, Sun, Sprout, Fence, Car, Eye, Mountain, Warehouse,
  Wifi, Droplet, Lock, Users, Scissors, Key
} from 'lucide-react';
...
// Custom Tree Icon for Farm
const TreeIcon = ({ className }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <path d="M12 2L3 12h4v8h10v-8h4L12 2z" stroke="currentColor" fill="none"/>
    <path d="M12 10v10" stroke="currentColor"/>
...
// Farm Area Unit Select - For Farm (Acres, Cents, Sq.ft, Hectares)
const FarmAreaUnitSelect = ({ value, onChange }) => {
  const [isOpen, setIsOpen] = useState(false);
  const selectRef = useRef(null);
  const units = [
...
const FarmHousePlotFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    // Farm & Nature Features
    agriculturalLandAttached: '', 
    fertileSoilAvailable: '', 
    waterSource: '', 
    irrigationFacility: '', 
    greenSurroundings: '', 
    waterView: '', 
    treePlantation: '', 
    organicFarmingSuitable: '',
  ...
  const mainSections = [
    ...
    { id: 'farm', label: '🌾 Farm Features', icon: <TreeIcon className="w-3.5 h-3.5" /> },
    { id: 'infrastructure', label: '🏗️ Infrastructure', icon: <Warehouse className="w-3.5 h-3.5" /> },
    { id: 'amenities', label: '✨ Amenities', icon: <Users className="w-3.5 h-3.5" /> },
    { id: 'legal', label: '⚖️ Legal', icon: <Shield className="w-3.5 h-3.5" /> },
    { id: 'suitable', label: '🏠 Suitable For', icon: <Building className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Farm House Plot' };
  ...
  const clearAllFilters = () => {
    setFilters({
      listingType: [], city: '', locality: '', farmCommunity: '', landmark: '', pincode: '', nearbyConnectivity: '',
      minRent: '', maxRent: '', securityDeposit: '', maintenanceIncluded: '', rentNegotiable: '',
      minPrice: '', maxPrice: '', preferredPricePerUnit: '', loanRequired: '', priceNegotiable: '',
      minSellPrice: '', maxSellPrice: '', sellPriceNegotiable: '', maintenanceCharges: '', propertyTax: '',
      minLeaseAmount: '', maxLeaseAmount: '', refundableDeposit: '', advanceAmount: '', leaseDuration: '', leaseNegotiable: '',
      plotArea: '', plotAreaUnit: 'acres', plotLength: '', plotWidth: '', facing: '', cornerPlot: '', roadWidth: '', boundaryWall: '', plotShape: '',
      agriculturalLandAttached: '', fertileSoilAvailable: '', waterSource: '', irrigationFacility: '', greenSurroundings: '', waterView: '', treePlantation: '', organicFarmingSuitable: '',
      farmHouseAvailable: '', borewellAvailable: '', electricityConnection: '', dripIrrigation: '', storageShed: '', caretakerRoom: '', internalRoads: '',
      gatedCommunity: '', security24x7: '', cctvSurveillance: '', streetLights: '', rainwaterHarvesting: '', gardenArea: '', parkingSpace: '', swimmingPool: '',
      agriculturalApproval: '', dtcpApproved: '', cmdaApproved: '', reraApproved: '', pattaAvailable: '', encumbranceFree: '',
      rentalAgreementReady: '', leaseAgreementReady: '', titleDeedVerified: '',
      suitableForTypes: [],
      immediateAvailability: '', availableFrom: '', minRentalDuration: '', leaseRenewalOption: '', readyToRegister: '', immediatePossession: '', underDevelopmentAcceptable: '',
      nearbyAccess: [],
    ...
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Farm House Plot" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <input type="text" placeholder="Area / Village / Locality" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.locality} onChange={(e) => handleInputChange('locality', e.target.value)} />
          <input type="text" placeholder="Farm Community / Layout Name" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.farmCommunity} onChange={(e) => handleInputChange('farmCommunity', e.target.value)} />
        ...
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <input type="number" placeholder="Preferred Price Per Acre / Sq.ft (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.preferredPricePerUnit} onChange={(e) => handleInputChange('preferredPricePerUnit', e.target.value)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <input type="number" placeholder="Price Per Acre / Sq.ft (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.preferredPricePerUnit} onChange={(e) => handleInputChange('preferredPricePerUnit', e.target.value)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
    ...
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        <div className="grid grid-cols-4 gap-2 items-end">
          ...
          <FarmAreaUnitSelect value={filters.plotAreaUnit} onChange={(val) => handleInputChange('plotAreaUnit', val)} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <input type="text" placeholder="Road Access Width (ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.roadWidth} onChange={(e) => handleInputChange('roadWidth', e.target.value)} />
  ...
  const renderFarmSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><TreeIcon className="w-3.5 h-3.5" /> Farm & Nature Features</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        <YesNoRadioGroup label="Agricultural Land Attached" name="agriculturalLandAttached" value={filters.agriculturalLandAttached} onChange={(val) => handleRadioChange('agriculturalLandAttached', val)} />
        <YesNoRadioGroup label="Fertile Soil Available" name="fertileSoilAvailable" value={filters.fertileSoilAvailable} onChange={(val) => handleRadioChange('fertileSoilAvailable', val)} />
        <CustomSelect label="Water Source" options={waterSourceOptions} value={filters.waterSource} onChange={(val) => handleInputChange('waterSource', val)} placeholder="Select Source" />
        <YesNoRadioGroup label="Irrigation Facility" name="irrigationFacility" value={filters.irrigationFacility} onChange={(val) => handleRadioChange('irrigationFacility', val)} />
        <YesNoRadioGroup label="Green Surroundings" name="greenSurroundings" value={filters.greenSurroundings} onChange={(val) => handleRadioChange('greenSurroundings', val)} />
        <CustomSelect label="View Type" options={waterViewOptions} value={filters.waterView} onChange={(val) => handleInputChange('waterView', val)} placeholder="Select View" />
        <YesNoRadioGroup label="Tree Plantation" name="treePlantation" value={filters.treePlantation} onChange={(val) => handleRadioChange('treePlantation', val)} />
        <YesNoRadioGroup label="Organic Farming Suitable" name="organicFarmingSuitable" value={filters.organicFarmingSuitable} onChange={(val) => handleRadioChange('organicFarmingSuitable', val)} />
      </div>
    </div>
  );
  ...
  const renderInfrastructureSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Warehouse className="w-3.5 h-3.5" /> Farm Infrastructure</h3>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.farmHouseAvailable === 'Yes'} onChange={(e) => handleRadioChange('farmHouseAvailable', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Farm House</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.borewellAvailable === 'Yes'} onChange={(e) => handleRadioChange('borewellAvailable', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Borewell</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.electricityConnection === 'Yes'} onChange={(e) => handleRadioChange('electricityConnection', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Electricity</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.dripIrrigation === 'Yes'} onChange={(e) => handleRadioChange('dripIrrigation', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Drip Irrigation</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.storageShed === 'Yes'} onChange={(e) => handleRadioChange('storageShed', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Storage Shed</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.caretakerRoom === 'Yes'} onChange={(e) => handleRadioChange('caretakerRoom', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Caretaker Room</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.internalRoads === 'Yes'} onChange={(e) => handleRadioChange('internalRoads', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
  ...
  const renderAmenitiesSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Users className="w-3.5 h-3.5" /> Amenities</h3>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.gatedCommunity === 'Yes'} onChange={(e) => handleRadioChange('gatedCommunity', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Gated Community</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.security24x7 === 'Yes'} onChange={(e) => handleRadioChange('security24x7', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">24/7 Security</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.cctvSurveillance === 'Yes'} onChange={(e) => handleRadioChange('cctvSurveillance', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">CCTV Surveillance</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.streetLights === 'Yes'} onChange={(e) => handleRadioChange('streetLights', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Street Lights</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.rainwaterHarvesting === 'Yes'} onChange={(e) => handleRadioChange('rainwaterHarvesting', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Rainwater Harvesting</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.gardenArea === 'Yes'} onChange={(e) => handleRadioChange('gardenArea', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Garden / Lawn</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.parkingSpace === 'Yes'} onChange={(e) => handleRadioChange('parkingSpace', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Parking Space</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.swimmingPool === 'Yes'} onChange={(e) => handleRadioChange('swimmingPool', e.target.checked ? 'Yes' : 'No')} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Swimming Pool</span>
        </label>
      </div>
    </div>
  );
  ...
  const renderLegalSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        <YesNoRadioGroup label="Agricultural Land Approval" name="agriculturalApproval" value={filters.agriculturalApproval} onChange={(val) => handleRadioChange('agriculturalApproval', val)} />
        <YesNoRadioGroup label="DTCP / CMDA Approved" name="dtcpApproved" value={filters.dtcpApproved} onChange={(val) => handleRadioChange('dtcpApproved', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Under Development Layout" name="underDevelopmentAcceptable" value={filters.underDevelopmentAcceptable} onChange={(val) => handleRadioChange('underDevelopmentAcceptable', val)} />
          </div>
        </div>
      );
    } else {
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <YesNoRadioGroup label="Ready to Register" name="readyToRegister" value={filters.readyToRegister} onChange={(val) => handleRadioChange('readyToRegister', val)} />
            <YesNoRadioGroup label="Immediate Possession" name="immediatePossession" value={filters.immediatePossession} onChange={(val) => handleRadioChange('immediatePossession', val)} />
            <YesNoRadioGroup label="Under Development Acceptable" name="underDevelopmentAcceptable" value={filters.underDevelopmentAcceptable} onChange={(val) => handleRadioChange('underDevelopmentAcceptable', val)} />
          </div>
        </div>
      );
    }
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <TreeIcon className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-lg">Filter Farm House Plot</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'farm' && renderFarmSection()}
...
export default FarmHousePlotFilter;