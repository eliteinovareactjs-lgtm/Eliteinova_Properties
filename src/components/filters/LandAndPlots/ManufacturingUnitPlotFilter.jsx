...
const ManufacturingUnitPlotFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    minRent: '', maxRent: '', advanceAmount: '', rentNegotiable: '',
    minLeaseAmount: '', maxLeaseAmount: '', securityDeposit: '', leaseDuration: '', leaseNegotiable: '',
    ...
    // Plot Details
    plotArea: '', plotAreaUnit: 'acres', builtupArea: '', openYardArea: '',
    plotLength: '', plotWidth: '', facing: '', cornerPlot: '', 
    roadWidth: '', boundaryWall: '', levelledLand: '', soilType: '', waterLoggingFree: '',
    ...
    dtcpApproved: '', cmdaApproved: '', industrialApproval: '', factoryLicenseEligible: '',
    ...
    // Availability
    readyToOccupy: '', immediatePossession: '', vacantPlotAvailable: '', operationalSiteAvailable: '',
    readyToRegister: '', preDevelopedSiteAvailable: '',
    // Nearby Access
    nearbyAccess: [],
    // Contact
  ...
  const manufacturingSuitabilityOptions = [
    'Engineering Manufacturing', 'Automobile Manufacturing', 'Textile Manufacturing', 'Food Processing Unit',
    'Chemical Manufacturing', 'Pharmaceutical Unit', 'Electronics Manufacturing', 'Packaging Industry',
    'Plastic Industry', 'Heavy Machinery Industry'
  ];
  ...
  const infrastructureUtilitiesOptions = [
    ...
    'Container Truck Access', 'Loading / Unloading Dock'
  ];

  const amenitiesOptions = [
    'Gated Industrial Park', 'Security / Watchman', 'CCTV Surveillance', 'Internal Roads',
  ...
  const mainSections = [
    ...
    { id: 'manufacturing', label: '🏭 Manufacturing', icon: <Cpu className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Manufacturing Unit Plot' };
    ...
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Manufacturing Unit Plot" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <input type="text" placeholder="Industrial Zone / SIPCOT / SEZ / MIDC / GIDC" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.industrialZone} onChange={(e) => handleInputChange('industrialZone', e.target.value)} />
  ...
  const renderPlotSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <input type="text" placeholder="Built-up Area Allowed (sq.ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.builtupArea} onChange={(e) => handleInputChange('builtupArea', e.target.value)} />
  ...
  const renderManufacturingSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Cpu className="w-3.5 h-3.5" /> Manufacturing Suitability</h3>
      <div className="grid grid-cols-2 gap-1.5">
        {manufacturingSuitabilityOptions.map(option => (
          <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
            <input
              type="checkbox"
              checked={filters.manufacturingSuitability.includes(option)}
              onChange={() => handleArrayToggle('manufacturingSuitability', option)}
              className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
            />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">{option}</span>
          </label>
  ...
  const renderLegalSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <YesNoRadioGroup label="Factory License Eligible" name="factoryLicenseEligible" value={filters.factoryLicenseEligible} onChange={(val) => handleRadioChange('factoryLicenseEligible', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Vacant Manufacturing Plot Available" name="vacantPlotAvailable" value={filters.vacantPlotAvailable} onChange={(val) => handleRadioChange('vacantPlotAvailable', val)} />
            <YesNoRadioGroup label="Operational Manufacturing Site Available" name="operationalSiteAvailable" value={filters.operationalSiteAvailable} onChange={(val) => handleRadioChange('operationalSiteAvailable', val)} />
          </div>
        </div>
      );
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Immediate Possession" name="immediatePossession" value={filters.immediatePossession} onChange={(val) => handleRadioChange('immediatePossession', val)} />
            <YesNoRadioGroup label="Vacant Manufacturing Plot Available" name="vacantPlotAvailable" value={filters.vacantPlotAvailable} onChange={(val) => handleRadioChange('vacantPlotAvailable', val)} />
            <YesNoRadioGroup label="Pre-Developed Manufacturing Site Available" name="preDevelopedSiteAvailable" value={filters.preDevelopedSiteAvailable} onChange={(val) => handleRadioChange('preDevelopedSiteAvailable', val)} />
          </div>
        </div>
      );
    } else {
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <YesNoRadioGroup label="Ready to Register" name="readyToRegister" value={filters.readyToRegister} onChange={(val) => handleRadioChange('readyToRegister', val)} />
            <YesNoRadioGroup label="Immediate Possession" name="immediatePossession" value={filters.immediatePossession} onChange={(val) => handleRadioChange('immediatePossession', val)} />
            <YesNoRadioGroup label="Vacant Manufacturing Plot Preferred" name="vacantPlotAvailable" value={filters.vacantPlotAvailable} onChange={(val) => handleRadioChange('vacantPlotAvailable', val)} />
            <YesNoRadioGroup label="Pre-Developed Manufacturing Site Preferred" name="preDevelopedSiteAvailable" value={filters.preDevelopedSiteAvailable} onChange={(val) => handleRadioChange('preDevelopedSiteAvailable', val)} />
          </div>
        </div>
      );
    }
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <Settings className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-sm">Filter Manufacturing Unit Plot</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'manufacturing' && renderManufacturingSection()}
...
export default ManufacturingUnitPlotFilter;