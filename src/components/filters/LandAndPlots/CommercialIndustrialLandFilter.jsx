...
const CommercialIndustrialLandFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    // Land Details
    totalLandArea: '', landAreaUnit: 'acres', usableLandArea: '', expandableLand: '',
    landShape: '', landLevel: '', facing: '', roadWidth: '', cornerPlotAvailable: '', boundaryWall: '',
    ...
    pattaAvailable: '', dtcpApproved: '', cmdaApproved: '', industrialApproval: '',
    environmentalClearance: '', pollutionClearance: '', fireSafetyApproval: '',
    encumbranceFree: '', loanEligible: '', landSurveyCompleted: '', titleDeedVerified: '',
    rentalAgreementAvailable: '', leaseAgreementAvailable: '',
    // Availability
    immediateOccupancy: '', readyToRegister: '', immediatePossession: '', vacantLandAvailable: '', developedIndustrialPlot: '',
    readyForCommercial: '', readyForIndustrial: '', availableFrom: '', longTermAvailable: '', shortTermAvailable: '',
    // Nearby Access
    nearbyAccess: [],
    // Investment Features
    investmentFeatures: [],
    ...
    buyerPreferences: [],
    // Contact
    contactOwner: false, contactAgent: false, contactDeveloper: false, contactCompany: false, preferredContactTime: ''
  });
  ...
  const commercialSuitabilityOptions = [
    'Retail Complex', 'Shopping Center', 'Showroom', 'Office Building',
    'Business Park', 'Hotel / Resort', 'Hospital / Healthcare Facility',
    'Educational Institution', 'Petrol Bunk', 'Commercial Development Project'
  ];

  const industrialSuitabilityOptions = [
    'Manufacturing Unit', 'Factory Setup', 'Warehouse', 'Logistics Hub',
    'Distribution Center', 'Cold Storage Facility', 'Industrial Shed Development',
    'Engineering Unit', 'Automobile Industry', 'Food Processing Unit', 'Textile Industry'
  ];

  const infrastructureUtilitiesOptions = [
    'Water Connection Available', 'Borewell Facility', 'Electricity Connection Available',
    '3-Phase Power Supply', 'Dedicated Transformer Available', 'Drainage Connection Available',
    'Sewage Connection Available', 'Gas Pipeline Available', 'Internet / Fiber Connectivity',
    'Internal Roads Available', 'Street Lighting Available'
  ];

  const transportationAccessOptions = [
    'National Highway Access', 'State Highway Access', 'Truck Access',
    'Trailer Access', 'Container Vehicle Access', 'Railway Freight Terminal Nearby',
    'Inland Container Depot (ICD) Nearby', 'Airport Nearby', 'Seaport / Harbor Nearby'
  ];

  const amenitiesOptions = [
    'Gated Property', 'Security Available', 'CCTV Surveillance', 'Office Space Available',
    'Parking Space Available', 'Truck Parking Area', 'Driver Rest Area',
    'Staff Accommodation', 'Weighbridge Facility'
  ];

  const nearbyOptions = [
    'National Highway', 'State Highway', 'Industrial Estate', 'SEZ',
    'Logistics Park', 'Commercial Complex', 'Railway Freight Terminal',
    'Airport', 'Seaport / Harbor', 'Bus Stand'
  ];

  const investmentFeaturesOptions = [
    'High Appreciation Potential', 'Industrial Growth Corridor', 'Commercial Growth Zone',
    'Smart City Zone', 'Township Development Area', 'Logistics Hub Nearby',
    'Manufacturing Cluster Nearby'
  ];

  const tenantPreferencesOptions = [
    'Commercial Business', 'Manufacturing Company', 'Warehouse Operator',
    'Logistics Company', 'Retail Chain', 'Corporate Office',
    'Educational Institution', 'Healthcare Organization', 'Developer / Builder'
  ];

  const buyerPreferencesOptions = [
    'Commercial Investor', 'Industrial Investor', 'Manufacturing Company',
    'Warehouse Operator', 'Logistics Company', 'Retail Chain',
    'Corporate Buyer', 'Developer / Builder'
  ];
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Commercial + Industrial Land' };
    ...
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Commercial + Industrial Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex flex-wrap gap-3">
              {['Owner', 'Agent', 'Developer', 'Company'].map(type => (
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <YesNoRadioGroup label="Highway Frontage" name="highwayFrontage" value={filters.highwayFrontage} onChange={(val) => handleRadioChange('highwayFrontage', val)} />
  ...
  const renderCommercialSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-2 gap-1.5">
        {commercialSuitabilityOptions.map(option => (
          <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
            <input
              type="checkbox"
              checked={filters.commercialSuitability.includes(option)}
              onChange={() => handleArrayToggle('commercialSuitability', option)}
              className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
            />
  ...
  const renderIndustrialSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-2 gap-1.5">
        {industrialSuitabilityOptions.map(option => (
          <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
            <input
              type="checkbox"
              checked={filters.industrialSuitability.includes(option)}
              onChange={() => handleArrayToggle('industrialSuitability', option)}
              className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
            />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">{option}</span>
          </label>
  ...
  const renderLegalSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <YesNoRadioGroup label="Pollution Control Clearance Available" name="pollutionClearance" value={filters.pollutionClearance} onChange={(val) => handleRadioChange('pollutionClearance', val)} />
        <YesNoRadioGroup label="Fire Safety Approval Available" name="fireSafetyApproval" value={filters.fireSafetyApproval} onChange={(val) => handleRadioChange('fireSafetyApproval', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Developed Industrial Plot Preferred" name="developedIndustrialPlot" value={filters.developedIndustrialPlot} onChange={(val) => handleRadioChange('developedIndustrialPlot', val)} />
            <YesNoRadioGroup label="Ready for Commercial Development" name="readyForCommercial" value={filters.readyForCommercial} onChange={(val) => handleRadioChange('readyForCommercial', val)} />
            <YesNoRadioGroup label="Ready for Industrial Development" name="readyForIndustrial" value={filters.readyForIndustrial} onChange={(val) => handleRadioChange('readyForIndustrial', val)} />
          </div>
        </div>
      );
    } else if (currentTab === 'Rent' || currentTab === 'Lease') {
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Ready for Commercial Use" name="readyForCommercial" value={filters.readyForCommercial} onChange={(val) => handleRadioChange('readyForCommercial', val)} />
            <YesNoRadioGroup label="Ready for Industrial Use" name="readyForIndustrial" value={filters.readyForIndustrial} onChange={(val) => handleRadioChange('readyForIndustrial', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Immediate Possession" name="immediatePossession" value={filters.immediatePossession} onChange={(val) => handleRadioChange('immediatePossession', val)} />
            <YesNoRadioGroup label="Vacant Land Available" name="vacantLandAvailable" value={filters.vacantLandAvailable} onChange={(val) => handleRadioChange('vacantLandAvailable', val)} />
            <YesNoRadioGroup label="Developed Industrial Plot Available" name="developedIndustrialPlot" value={filters.developedIndustrialPlot} onChange={(val) => handleRadioChange('developedIndustrialPlot', val)} />
            <YesNoRadioGroup label="Ready for Commercial Development" name="readyForCommercial" value={filters.readyForCommercial} onChange={(val) => handleRadioChange('readyForCommercial', val)} />
            <YesNoRadioGroup label="Ready for Industrial Development" name="readyForIndustrial" value={filters.readyForIndustrial} onChange={(val) => handleRadioChange('readyForIndustrial', val)} />
          </div>
        </div>
    ...
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        <div className="flex flex-wrap gap-3">
          ...
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <input type="checkbox" checked={filters.contactCompany} onChange={(e) => handleCheckboxChange('contactCompany', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
            <span className="text-xs text-gray-700 font-medium group-hover:text-teal-500">Contact Company</span>
          </label>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
        <div className="sticky top-0 z-10 bg-white border-b border-teal-100">
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-white font-semibold text-sm">Filter Commercial + Industrial Land</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'commercial' && renderCommercialSection()}
            {activeMainSection === 'industrial' && renderIndustrialSection()}
