...
import {
  ...
  Landmark, Rocket, Target, Award, BarChart3, LineChart,
  TrendingUp as TrendingUpIcon, LucideRocket
} from 'lucide-react';
...
const FutureInvestmentPlotFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    mainRoadFacing: '', cornerPlot: '', gatedCommunity: '', nearbyConnectivity: '',
    upcomingDevelopmentArea: '',
    // Rent
    ...
    // Investment Purpose & Preferences
    investmentPurpose: [],
    investmentPreferences: [],
    // Investment & Growth Features (Lease)
    growthFeatures: [],
    suitableFor: [],
    // Infrastructure
  ...
  const investmentPurposeOptions = {
    buy: [
      'Long-Term Investment', 'Residential Development', 'Villa Project',
      'Apartment Project', 'Commercial Development', 'Retail Complex',
      'Warehouse', 'Industrial Development', 'Mixed-Use Development'
    ],
    rent: [
      'Temporary Office', 'Construction Site', 'Material Storage',
      'Vehicle Parking', 'Equipment Yard', 'Warehouse Yard',
      'Commercial Yard', 'Event Space', 'Industrial Yard'
    ],
    lease: [
      'Residential Development', 'Villa Projects', 'Apartment Development',
      'Commercial Complex', 'Office Space', 'Retail Shops',
      'Warehouse', 'Logistics Park', 'Industrial Unit',
      'Mixed-Use Development', 'Educational Institution', 'Hospital',
      'Hotel / Resort', 'Temporary Project Site'
    ]
  };

  const investmentPreferencesOptions = [
    'High Appreciation Area', 'Smart City Zone', 'Upcoming Highway',
    'Metro Corridor', 'Airport Expansion Zone', 'IT Corridor',
    'Industrial Corridor', 'SEZ Nearby', 'Future Township',
    'High Rental Potential'
  ];

  const growthFeaturesOptions = [
    'Future Growth Corridor', 'Smart City Project Nearby',
    'Industrial Corridor Nearby', 'IT Park Nearby', 'SEZ Nearby',
    'Metro Rail Project Nearby', 'Ring Road Nearby',
    'Highway Expansion Project', 'Airport Expansion Nearby',
    'Commercial Growth Zone', 'Residential Expansion Zone',
    'Green Zone', 'High Appreciation Potential'
  ];

  const nearbyOptions = {
    buy: [
      'National Highway', 'State Highway', 'Metro Station', 'Railway Station',
      'Airport', 'IT Park', 'Industrial Park', 'Educational Institutions',
    ...
    lease: [
      'National Highway', 'State Highway', 'Bus Stop', 'Railway Station',
      'Metro Station', 'Airport', 'IT Park', 'Industrial Hub',
  ...
  const mainSections = [
    ...
    { id: 'investment', label: '📈 Investment', icon: <TrendingUpIcon className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Future Investment Plot' };
    ...
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          <div>
            ...
            <input type="text" value="Future Investment Plot" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          ...
          {currentTab === 'Lease' && (
            <YesNoRadioGroup label="Upcoming Development Area" name="upcomingDevelopmentArea" value={filters.upcomingDevelopmentArea} onChange={(val) => handleRadioChange('upcomingDevelopmentArea', val)} />
          )}
  ...
  const renderInvestmentSection = () => {
    if (currentTab === 'Lease') {
      return (
        <div className="space-y-3">
          <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
            <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><BarChart3 className="w-3.5 h-3.5" /> Investment & Growth Features</h3>
            <CheckboxGroup 
              options={growthFeaturesOptions} 
              values={filters.growthFeatures} 
              onChange={(val) => handleInputChange('growthFeatures', val)} 
              columns={2} 
            />
          </div>
          <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
            ...
            <CheckboxGroup 
              options={investmentPurposeOptions.lease} 
    ...
    const purposeOptions = investmentPurposeOptions[currentTab.toLowerCase()] || investmentPurposeOptions.buy;
    return (
      <div className="space-y-3">
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm">
            {currentTab === 'Buy' ? <Target className="w-3.5 h-3.5" /> : <Target className="w-3.5 h-3.5" />}
            {currentTab === 'Buy' ? 'Preferred Investment Purpose' : 'Suitable For'}
          </h3>
          <CheckboxGroup options={purposeOptions} values={filters.investmentPurpose} onChange={(val) => handleInputChange('investmentPurpose', val)} columns={2} />
        </div>
        {currentTab === 'Buy' && (
          <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
            <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><TrendingUpIcon className="w-3.5 h-3.5" /> Investment Preferences</h3>
            <CheckboxGroup 
              options={investmentPreferencesOptions} 
              values={filters.investmentPreferences} 
              onChange={(val) => handleInputChange('investmentPreferences', val)} 
              columns={2} 
            />
          </div>
        )}
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Pre-Launch Project" name="preLaunchProject" value={filters.preLaunchProject} onChange={(val) => handleRadioChange('preLaunchProject', val)} />
          </div>
        </div>
      );
    } else {
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <Rocket className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-lg">Filter Future Investment Plot</h3>
            </div>
            {onClose && (
...
export default FutureInvestmentPlotFilter;