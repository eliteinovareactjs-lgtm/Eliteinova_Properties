...
const CityIcon = ({ className }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <path d="M4 21V8l6-4 6 4v13" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M8 21V12h8v9" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M8 12h8" stroke="currentColor" strokeLinecap="round"/>
    <path d="M8 16h4" stroke="currentColor" strokeLinecap="round"/>
    <path d="M8 20h2" stroke="currentColor" strokeLinecap="round"/>
    <path d="M14 20h2" stroke="currentColor" strokeLinecap="round"/>
  </svg>
);
...
const LayoutDevelopmentLandFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    mainRoadFacing: '', cornerLand: '', gatedLayout: '', nearbyConnectivity: '',
    // Rent
    ...
    // Preferred Development / Suitable For
    preferredDevelopment: [],
    // Layout Development Features (Lease)
    layoutFeatures: [],
    suitableFor: [],
    // Infrastructure
    ...
    // Amenities
    gatedCommunity: '', security247: false, cctvSurveillance: false,
    clubHouse: false, childrensPark: false, landscapedGarden: false,
    walkingTrack: false, visitorParking: false, rainwaterHarvesting: false,
    compoundWall: false, joggingTrack: false, communityHall: false,
    // Legal Requirements
    titleDeedVerified: '', loanEligible: '', reraApproved: '',
    dtcpApproved: '', cmdaApproved: '', panchayatApproved: '',
    layoutApproved: '', encumbranceCertificate: '',
    // Legal for Rent
  ...
  const preferredDevelopmentOptions = {
    buy: [
      'Residential Layout', 'Villa Community', 'Gated Community',
      'Township', 'Commercial Layout', 'Mixed-Use Development',
      'Apartment Project', 'Educational Campus', 'Hospital', 'Retail Complex'
    ],
    rent: [
      'Construction Site', 'Material Storage', 'Layout Office',
      'Temporary Site Office', 'Equipment Yard', 'Worker Accommodation',
      'Parking Yard', 'Commercial Yard', 'Event Ground'
    ],
    lease: [
      'Residential Plots', 'Villa Projects', 'Apartment Development',
      'Gated Community', 'Township', 'Commercial Complex',
      'Mixed-Use Development', 'Educational Institution', 'Hospital',
      'Retail Shops', 'Office Space', 'Warehouse', 'Temporary Project Site'
    ]
  };

  const layoutFeaturesOptions = [
    'Residential Layout', 'Villa Layout', 'Premium Gated Community',
    'Township Development', 'Mixed-Use Layout', 'Commercial Layout',
    'Industrial Layout', 'Corner Plots Available', 'Park Facing Plots',
    'Wide Internal Roads', 'Avenue Plantation', 'Drainage System',
    'Street Lighting', 'Underground Electricity', 'Underground Water Pipeline'
  ];

  const nearbyOptions = {
    buy: [
      'Highway Access', 'Bus Stop', 'Railway Station', 'Metro Station',
      'Airport', 'Schools & Colleges', 'Hospitals', 'Shopping Mall',
      'Market Area', 'IT Park', 'Industrial Hub', 'Bank / ATM'
    ],
    rent: [
      'Highway Access', 'Bus Stop', 'Railway Station', 'Metro Station',
      'Airport', 'Schools & Colleges', 'Hospitals', 'Shopping Mall',
  ...
  const mainSections = [
    ...
    { id: 'development', label: '🏗️ Development', icon: <Layout className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Layout Development Land' };
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          <div>
            ...
            <input type="text" value="Layout Development Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
  ...
  const renderDevelopmentSection = () => {
    if (currentTab === 'Lease') {
      return (
        <div className="space-y-3">
          <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
            <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Layout className="w-3.5 h-3.5" /> Layout Development Features</h3>
            <CheckboxGroup 
              options={layoutFeaturesOptions} 
              values={filters.layoutFeatures} 
              onChange={(val) => handleInputChange('layoutFeatures', val)} 
              columns={2} 
            />
          </div>
          <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
            ...
            <CheckboxGroup 
              options={preferredDevelopmentOptions.lease} 
    ...
    const options = preferredDevelopmentOptions[currentTab.toLowerCase()] || preferredDevelopmentOptions.buy;
    return (
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm">
          {currentTab === 'Buy' ? <Layout className="w-3.5 h-3.5" /> : <Building className="w-3.5 h-3.5" />}
          {currentTab === 'Buy' ? 'Preferred Development' : 'Suitable For'}
        </h3>
        <CheckboxGroup options={options} values={filters.preferredDevelopment} onChange={(val) => handleInputChange('preferredDevelopment', val)} columns={2} />
      </div>
    );
  };
    ...
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        ...
        <label className="flex items-center gap-1.5 cursor-pointer group">
          ...
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Street Lighting</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          ...
          {currentTab === 'Lease' && (
            <>
              <label className="flex items-center gap-1.5 cursor-pointer group">
                <input type="checkbox" checked={filters.joggingTrack} onChange={(e) => handleCheckboxChange('joggingTrack', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
                <span className="text-xs text-gray-700 group-hover:text-teal-500">Jogging Track</span>
              </label>
              <label className="flex items-center gap-1.5 cursor-pointer group">
                <input type="checkbox" checked={filters.communityHall} onChange={(e) => handleCheckboxChange('communityHall', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
                <span className="text-xs text-gray-700 group-hover:text-teal-500">Community Hall</span>
              </label>
            </>
          )}
        </div>
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
            ...
            <YesNoRadioGroup label="Layout Approved" name="layoutApproved" value={filters.layoutApproved} onChange={(val) => handleRadioChange('layoutApproved', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Fully Developed Layout" name="fullyDevelopedLayout" value={filters.fullyDevelopedLayout} onChange={(val) => handleRadioChange('fullyDevelopedLayout', val)} />
          </div>
        </div>
      );
    } else {
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Under Development" name="underDevelopmentLease" value={filters.underDevelopmentLease} onChange={(val) => handleRadioChange('underDevelopmentLease', val)} />
            <YesNoRadioGroup label="Fully Developed Layout" name="fullyDevelopedLayoutLease" value={filters.fullyDevelopedLayoutLease} onChange={(val) => handleRadioChange('fullyDevelopedLayoutLease', val)} />
          </div>
        </div>
      );
    }
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <Layout className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-lg">Filter Layout Development Land</h3>
            </div>
            {onClose && (
...
export default LayoutDevelopmentLandFilter;