...
import {
  ...
  Server, Network, Database, Globe, Cpu
} from 'lucide-react';
...
const ITParkLandFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    listingType: [],
    // Location
    city: '', itCorridor: '', locality: '', landmark: '', pincode: '', mainRoadFacing: '', highwayAccess: '', nearbyConnectivity: '',
    // Rent
    ...
    // Land Details
    landArea: '', landAreaUnit: 'sqft', plotLength: '', plotWidth: '', facing: '', cornerLand: '', roadWidth: '', boundaryWall: '', landShape: '',
    // For Buy specific
    preferredPlotLength: '', preferredPlotWidth: '', minRoadWidthRequired: '', boundaryWallPreferred: '', landShapePreference: '',
    // IT Park Suitability
    itParkSuitability: [],
    // Infrastructure & Utilities
    waterConnection: false, electricityConnection: false, highVoltagePower: false, drainageFacility: false, fiberInternet: false, generatorBackup: false, parkingSpace: false, heavyVehicleAccess: false,
    // Amenities
    gatedLayout: '', security247: false, cctvSurveillance: false, visitorParking: false, wideInternalRoads: false, cafeteriaSpace: false, fireSafetyProvision: false, liftElevatorProvision: false,
    // Legal & Approval
    dtcpApproved: '', cmdaApproved: '', reraApproved: '', itApproval: '', pattaAvailable: '', encumbranceFree: '',
    // Buy specific legal
    encumbranceFreePreferred: '', loanEligibleLandRequired: '', titleDeedVerified: '',
    // Sell specific legal
    loanEligible: '',
    // Rent/Lease specific
  ...
  const itParkSuitabilityOptions = {
    rent: [
      'Software Company Campus', 'IT Office Complex', 'Tech Park Development',
      'Startup Hub', 'Co-working Space', 'Data Center Setup',
      'BPO / KPO Operations', 'Corporate Headquarters'
    ],
    buy: [
      'Software Company Campus', 'IT Office Complex', 'Tech Park Development',
      'Startup Hub', 'Co-working Space', 'Data Center Setup',
  ...
  const nearbyOptions = {
    rent: [
      ...
      'IT Companies Nearby', 'Commercial Hub', 'Hotels / Restaurants'
    ],
    buy: [
      'Metro / Bus Stop', 'Railway Station', 'Airport Access',
      'IT Companies Nearby', 'Commercial Hub', 'Hotels / Restaurants'
  ...
  const mainSections = [
    ...
    { id: 'suitability', label: '🏢 IT Suitability', icon: <Server className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'IT Park Land' };
    ...
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          <div>
            ...
            <input type="text" value="IT Park Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          ...
          <input type="text" placeholder="IT Corridor / Business District" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.itCorridor} onChange={(e) => handleInputChange('itCorridor', e.target.value)} />
      ...
      <div className="space-y-2">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div className="flex gap-2 items-center min-w-0 overflow-hidden">
            <input type="text" placeholder="Land Area" className="flex-1 min-w-0 px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.landArea} onChange={(e) => handleInputChange('landArea', e.target.value)} />
            <PlotAreaUnitSelect value={filters.landAreaUnit} onChange={(val) => handleInputChange('landAreaUnit', val)} />
          </div>
          {currentTab === 'Buy' ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          ...
          {currentTab === 'Buy' ? (
            <>
              <CustomSelect label="Land Shape Preference" options={landShapeOptions} value={filters.landShapePreference} onChange={(val) => handleInputChange('landShapePreference', val)} placeholder="Select Shape" />
  ...
  const renderSuitabilitySection = () => {
    const options = itParkSuitabilityOptions[currentTab.toLowerCase()] || itParkSuitabilityOptions.buy;
    return (
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Server className="w-3.5 h-3.5" /> IT Park Suitability</h3>
        <CheckboxGroup options={options} values={filters.itParkSuitability} onChange={(val) => handleInputChange('itParkSuitability', val)} columns={2} />
      </div>
    );
  };
  ...
  const renderUtilitiesSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        ...
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.highVoltagePower} onChange={(e) => handleCheckboxChange('highVoltagePower', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">High Voltage Power Supply</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
        ...
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.fiberInternet} onChange={(e) => handleCheckboxChange('fiberInternet', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Fiber Internet Connectivity</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          ...
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Generator / Power Backup Provision</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
  ...
  const renderAmenitiesSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        <YesNoRadioGroup label="Gated Commercial Layout" name="gatedLayout" value={filters.gatedLayout} onChange={(val) => handleRadioChange('gatedLayout', val)} />
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          <label className="flex items-center gap-1.5 cursor-pointer group">
          ...
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <input type="checkbox" checked={filters.fireSafetyProvision} onChange={(e) => handleCheckboxChange('fireSafetyProvision', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">Fire Safety Provision</span>
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <input type="checkbox" checked={filters.liftElevatorProvision} onChange={(e) => handleCheckboxChange('liftElevatorProvision', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">Lift / Elevator Provision</span>
          </label>
        </div>
      </div>
    </div>
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
            ...
            <YesNoRadioGroup label="Commercial / IT Approval Available" name="itApproval" value={filters.itApproval} onChange={(val) => handleRadioChange('itApproval', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
            ...
            <YesNoRadioGroup label="CMDA Approved" name="cmdaApproved" value={filters.cmdaApproved} onChange={(val) => handleRadioChange('cmdaApproved', val)} />
            <YesNoRadioGroup label="RERA Approved" name="reraApproved" value={filters.reraApproved} onChange={(val) => handleRadioChange('reraApproved', val)} />
            <YesNoRadioGroup label="Commercial / IT Approval Required" name="itApproval" value={filters.itApproval} onChange={(val) => handleRadioChange('itApproval', val)} />
            <YesNoRadioGroup label="Patta Available" name="pattaAvailable" value={filters.pattaAvailable} onChange={(val) => handleRadioChange('pattaAvailable', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Under Development IT Layout" name="underDevelopmentLayout" value={filters.underDevelopmentLayout} onChange={(val) => handleRadioChange('underDevelopmentLayout', val)} />
          </div>
        </div>
      );
    } else {
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <YesNoRadioGroup label="Ready to Register" name="readyToRegister" value={filters.readyToRegister} onChange={(val) => handleRadioChange('readyToRegister', val)} />
            <YesNoRadioGroup label="Immediate Possession" name="immediatePossession" value={filters.immediatePossession} onChange={(val) => handleRadioChange('immediatePossession', val)} />
            <YesNoRadioGroup label="Under Development IT Layout Acceptable" name="underDevelopmentAcceptable" value={filters.underDevelopmentAcceptable} onChange={(val) => handleRadioChange('underDevelopmentAcceptable', val)} />
          </div>
        </div>
      );
    }
        ...
        <div className="sticky top-0 z-10 bg-white border-b border-teal-100">
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-white font-semibold text-lg">Filter IT Park Land</h3>
            </div>
            {onClose && (
