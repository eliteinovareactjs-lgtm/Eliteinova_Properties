...
const FactoryLandFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    // Land Details
    landArea: '', landAreaUnit: 'acres', openYardArea: '', builtupArea: '', 
    ...
    // Availability
    readyToOccupy: '', immediatePossession: '', vacantLandAvailable: '', operationalSiteAvailable: '',
    readyToRegister: '', underDevelopmentLayout: '',
    // Nearby Access
    nearbyAccess: [],
    // Contact
  ...
  const factorySuitabilityOptions = [
    'Manufacturing Unit', 'Engineering Industry', 'Automobile Industry', 'Textile Industry',
    'Food Processing Unit', 'Chemical Industry', 'Pharmaceutical Industry', 'Electronics Manufacturing',
    'Packaging Industry', 'Logistics & Warehouse Setup'
  ];
  ...
  const infrastructureUtilitiesOptions = [
    ...
    'Container Truck Access', 'Loading / Unloading Area'
  ];

  const amenitiesOptions = [
    'Gated Industrial Park', 'Security / Watchman', 'CCTV Surveillance', 'Internal Roads',
    'Street Lights', 'Parking Space', 'Fire Safety System', 'Common Utilities Available',
    'Worker Accommodation', 'Office Space Available'
  ];
  ...
  const mainSections = [
    ...
    { id: 'suitability', label: '🏭 Factory', icon: <Factory className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Factory Land' };
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Factory Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
 ...
 const renderSuitabilitySection = () => (
  <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
    <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Factory className="w-3.5 h-3.5" /> Factory Suitability</h3>
    <div className="grid grid-cols-2 gap-1.5">
      {factorySuitabilityOptions.map(option => (
        <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
          <input
            type="checkbox"
            checked={filters.factorySuitability.includes(option)}
            onChange={() => handleArrayToggle('factorySuitability', option)}
            className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
          />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">{option}</span>
        </label>
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Vacant Factory Land Available" name="vacantLandAvailable" value={filters.vacantLandAvailable} onChange={(val) => handleRadioChange('vacantLandAvailable', val)} />
            {currentTab === 'Rent' && (
              <YesNoRadioGroup label="Operational Industrial Site Available" name="operationalSiteAvailable" value={filters.operationalSiteAvailable} onChange={(val) => handleRadioChange('operationalSiteAvailable', val)} />
            )}
            {currentTab === 'Lease' && (
              <YesNoRadioGroup label="Pre-Developed Industrial Site Available" name="operationalSiteAvailable" value={filters.operationalSiteAvailable} onChange={(val) => handleRadioChange('operationalSiteAvailable', val)} />
            )}
          </div>
        </div>
      );
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <YesNoRadioGroup label="Ready to Register" name="readyToRegister" value={filters.readyToRegister} onChange={(val) => handleRadioChange('readyToRegister', val)} />
            <YesNoRadioGroup label="Immediate Possession" name="immediatePossession" value={filters.immediatePossession} onChange={(val) => handleRadioChange('immediatePossession', val)} />
            <YesNoRadioGroup label="Vacant Factory Land Preferred" name="vacantLandAvailable" value={filters.vacantLandAvailable} onChange={(val) => handleRadioChange('vacantLandAvailable', val)} />
            <YesNoRadioGroup label="Pre-Developed Industrial Site Preferred" name="operationalSiteAvailable" value={filters.operationalSiteAvailable} onChange={(val) => handleRadioChange('operationalSiteAvailable', val)} />
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <Factory className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-sm">Filter Factory Land</h3>
            </div>
            {onClose && (
...
export default FactoryLandFilter;