...
const CommercialPlotFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    listingType: [],
    // Location
    city: '', locality: '', landmark: '', pincode: '', mainRoadFacing: '', highwayFacing: '', nearbyConnectivity: '',
    commercialPlotPreferred: '',
    // Rent
    ...
    // Plot Details
    plotArea: '', plotAreaUnit: 'sqft', plotLength: '', plotWidth: '', facing: '', commercialPlot: '', roadWidth: '', boundaryWall: '', plotShape: '', openGroundCovered: '',
    // For Buy specific
    ...
    waterConnection: false, borewellFacility: false, electricityConnection: false, drainageFacility: false, streetLights: false, truckHeavyVehicleAccess: false, internetBroadband: false,
    // Amenities
    gatedLayout: '', security247: false, cctvSurveillance: false, visitorParking: false, wideInternalRoads: false, fireSafetyProvision: false,
    // Legal & Approval
  ...
  const commercialSuitabilityOptions = {
    rent: [
      'Retail Shop Setup', 'Showroom Construction', 'Office Building',
      'Warehouse / Storage', 'Hotel / Restaurant', 'Petrol Bunk',
      'Commercial Complex', 'Parking Area', 'Industrial Use'
    ],
    buy: [
    ...
    lease: [
      'Retail Shop Construction', 'Showroom Development', 'Office Building',
      'Warehouse / Storage', 'Hotel / Restaurant', 'Petrol Bunk',
      'Commercial Complex', 'Parking Facility', 'Industrial Use', 'Temporary Commercial Setup'
    ]
  };
  ...
  const mainSections = [
    ...
    { id: 'suitability', label: '🏪 Suitability', icon: <Store className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Commercial Plot' };
    ...
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          <div>
            ...
            <input type="text" value="Commercial Plot" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          ...
          {currentTab === 'Buy' && (
            <YesNoRadioGroup label="commercial Plot Preferred" name="commercialPlotPreferred" value={filters.commercialPlotPreferred} onChange={(val) => handleRadioChange('commercialPlotPreferred', val)} />
          )}
          {currentTab !== 'Buy' && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          ...
          {currentTab === 'Buy' ? (
            ...
            <>
              ...
              {(currentTab === 'Sell' || currentTab === 'Lease') && (
                <div className="md:col-span-2">
                  <label className="text-xs text-teal-800 font-semibold block mb-1">Open Ground / Covered Area</label>
                  <input type="text" placeholder="Open Ground / Covered Area Details" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.openGroundCovered} onChange={(e) => handleInputChange('openGroundCovered', e.target.value)} />
                </div>
              )}
            </>
          )}
  ...
  const renderSuitabilitySection = () => {
    ...
    return (
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <CheckboxGroup options={options} values={filters.commercialSuitability} onChange={(val) => handleInputChange('commercialSuitability', val)} columns={2} />
      </div>
    );
  };
  ...
  const renderUtilitiesSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        ...
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.truckHeavyVehicleAccess} onChange={(e) => handleCheckboxChange('truckHeavyVehicleAccess', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Truck / Heavy Vehicle Access</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
        ...
        <div className="sticky top-0 z-10 bg-white border-b border-teal-100">
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-white font-semibold text-lg">Filter Commercial Plot</h3>
            </div>
            {onClose && (
...
export default CommercialPlotFilter;