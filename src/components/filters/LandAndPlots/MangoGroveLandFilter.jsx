...
import {
  ...
  Warehouse, PawPrint, Flower2, Recycle, Apple, Milk, Citrus
} from 'lucide-react';
...
const MangoLandAreaUnitSelect = ({ value, onChange }) => {
  const [isOpen, setIsOpen] = useState(false);
  const selectRef = useRef(null);
  const units = [
...
const MangoGroveLandFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    // Land Details
    landArea: '', landAreaUnit: 'acres', numberOfTrees: '', treeAge: '', mangoVariety: '', yieldCapacity: '', soilType: '', waterSource: '', irrigationFacility: '', facing: '', cornerLand: '', roadWidth: '', boundaryAvailable: '',
  ...
  const mangoVarietyOptions = [
    { value: 'Alphonso', label: 'Alphonso' },
    { value: 'Banganapalli', label: 'Banganapalli' },
    { value: 'Imam Pasand', label: 'Imam Pasand' },
    { value: 'Totapuri', label: 'Totapuri' },
    { value: 'Neelam', label: 'Neelam' },
    { value: 'Others', label: 'Others' }
  ];

  const farmSuitabilityOptions = [
    'Mango Cultivation', 'Organic Mango Farming', 'Fruit Export Business',
    'Mango Processing Unit', 'Farm Stay / Eco Tourism', 'Intercropping Farming'
  ];

  const infrastructureUtilitiesOptions = [
    'Water Connection Available', 'Borewell Facility', 'Electricity Connection Available',
    '3-Phase Agricultural Power', 'Drip Irrigation Setup', 'Storage Shed Available',
    'Farm House Available', 'Tractor / Heavy Vehicle Access'
  ];

  const amenitiesOptions = [
    'Gated Farm Land', 'Security / Caretaker', 'Rainwater Harvesting',
    'Internal Farm Roads', 'Worker Accommodation'
  ];
  ...
  const nearbyOptions = [
    ...
    'Fruit Processing Unit Nearby', 'Transport Access'
  ];
  ...
  const mainSections = [
    ...
    { id: 'land', label: '🥭 Mango Grove', icon: <Citrus className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Mango Grove Land' };
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Mango Grove Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
  ...
  const renderLandSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Citrus className="w-3.5 h-3.5" /> Mango Grove Details</h3>
      <div className="space-y-2">
        <div className="grid grid-cols-4 gap-2 items-end">
          ...
          <MangoLandAreaUnitSelect value={filters.landAreaUnit} onChange={(val) => handleInputChange('landAreaUnit', val)} />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <input type="number" placeholder="Number of Mango Trees" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.numberOfTrees} onChange={(e) => handleInputChange('numberOfTrees', e.target.value)} />
          <input type="text" placeholder="Mango Tree Age (years)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.treeAge} onChange={(e) => handleInputChange('treeAge', e.target.value)} />
          <CustomSelect label="Mango Variety" options={mangoVarietyOptions} value={filters.mangoVariety} onChange={(val) => handleInputChange('mangoVariety', val)} placeholder="Select Variety" />
          <input type="text" placeholder="Expected Yield Capacity" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.yieldCapacity} onChange={(e) => handleInputChange('yieldCapacity', e.target.value)} />
        ...
        <div className="sticky top-0 z-10 bg-white border-b border-teal-100">
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-white font-semibold text-sm">Filter Mango Grove Land</h3>
            </div>
            {onClose && (
