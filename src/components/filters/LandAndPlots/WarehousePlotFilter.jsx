...
import { 
  ...
  Box, Package, Thermometer, Snowflake, Anchor
} from 'lucide-react';
...
const WarehousePlotFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    // Plot Details
    plotArea: '', plotAreaUnit: 'acres', openStorageYard: '', builtupArea: '', truckParkingCapacity: '',
    ...
    dtcpApproved: '', cmdaApproved: '', warehouseApproval: '',
  ...
  const warehouseSuitabilityOptions = [
    'General Warehousing', 'Cold Storage Warehouse', 'E-Commerce Fulfillment Center', 'Distribution Center',
    'Container Storage Yard', 'FMCG Storage', 'Agricultural Storage', 'Pharmaceutical Warehouse',
    'Industrial Goods Storage', 'Bulk Storage Facility'
  ];
  ...
  const infrastructureUtilitiesOptions = [
    ...
    'Container Truck Access', 'Loading Dock Available', 'Dock Levelers Available', 'Weighbridge Nearby'
  ];

  const amenitiesOptions = [
    'Gated Warehouse Park', 'Security / Watchman', 'CCTV Surveillance', 'Internal Roads',
  ...
  const nearbyOptions = [
    ...
    'Seaport Nearby', 'Bus Route Access', 'Industrial Corridor Access'
  ];
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Warehouse Plot' };
    ...
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Warehouse Plot" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <input type="text" placeholder="Industrial Zone / Logistics Park / Warehouse Corridor" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.industrialZone} onChange={(e) => handleInputChange('industrialZone', e.target.value)} />
  ...
  const renderPlotSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <input type="text" placeholder="Open Storage Yard Area (sq.ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.openStorageYard} onChange={(e) => handleInputChange('openStorageYard', e.target.value)} />
  ...
  const renderWarehouseSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Warehouse className="w-3.5 h-3.5" /> Warehouse Suitability</h3>
      <div className="grid grid-cols-2 gap-1.5">
        {warehouseSuitabilityOptions.map(option => (
          <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
            <input
              type="checkbox"
              checked={filters.warehouseSuitability.includes(option)}
              onChange={() => handleArrayToggle('warehouseSuitability', option)}
              className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
            />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">{option}</span>
          </label>
  ...
  const renderLegalSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <YesNoRadioGroup label="Industrial / Warehouse Approval Available" name="warehouseApproval" value={filters.warehouseApproval} onChange={(val) => handleRadioChange('warehouseApproval', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Vacant Warehouse Plot Available" name="vacantPlotAvailable" value={filters.vacantPlotAvailable} onChange={(val) => handleRadioChange('vacantPlotAvailable', val)} />
            <YesNoRadioGroup label="Operational Warehouse Site Available" name="operationalSiteAvailable" value={filters.operationalSiteAvailable} onChange={(val) => handleRadioChange('operationalSiteAvailable', val)} />
          </div>
        </div>
      );
    } else if (currentTab === 'Sell') {
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <YesNoRadioGroup label="Ready to Register" name="readyToRegister" value={filters.readyToRegister} onChange={(val) => handleRadioChange('readyToRegister', val)} />
            <YesNoRadioGroup label="Immediate Possession" name="immediatePossession" value={filters.immediatePossession} onChange={(val) => handleRadioChange('immediatePossession', val)} />
            <YesNoRadioGroup label="Vacant Warehouse Plot Preferred" name="vacantPlotAvailable" value={filters.vacantPlotAvailable} onChange={(val) => handleRadioChange('vacantPlotAvailable', val)} />
            <YesNoRadioGroup label="Operational Warehouse Site Preferred" name="operationalSiteAvailable" value={filters.operationalSiteAvailable} onChange={(val) => handleRadioChange('operationalSiteAvailable', val)} />
          </div>
        </div>
      );
    }
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <Package className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-sm">Filter Warehouse Plot</h3>
            </div>
            {onClose && (
...
export default WarehousePlotFilter;