...
import {
  ...
  Leaf, Coffee, Tent, Compass, TreePine, MountainSnow, Flower2,
  Sprout, SunMedium, Wind, Droplets, Binoculars
} from 'lucide-react';
...
const EcoTourismLandFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    nearbyNature: [], mainRoadFacing: '', cornerLand: '', gatedProperty: '', nearbyConnectivity: '',
    // Rent
    ...
    // Land Details
    totalArea: '', areaUnit: 'sqft', landLength: '', landWidth: '',
    frontageWidth: '', roadWidth: '', facing: '', boundaryWall: '',
    landLevel: '', soilType: '', ownershipType: '',
    // Nature & Scenic Features (Lease)
    scenicFeatures: [],
    // Preferred Usage / Suitable For
    preferredUsage: [],
    // Infrastructure
    electricityConnection: false, waterSupply: false, borewell: false,
    drainage: false, internalRoads: false, internetConnectivity: false,
    solarPowerReady: false,
    // Amenities
    gatedPropertyAmenity: '', security247: false, cctvSurveillance: false,
    walkingTrails: false, landscapedGarden: false, parkingArea: false,
    rainwaterHarvesting: false, solarLighting: false, childrensPlayArea: false,
    // Legal Requirements
    titleDeedVerified: '', loanEligible: '', reraApproved: '',
    dtcpApproved: '', cmdaApproved: '', panchayatApproved: '',
    environmentalClearance: '', tourismLicenseEligible: '', encumbranceCertificate: '',
    // Legal for Rent
    rentalAgreementReady: '',
    // Legal for Lease
    commercialLeaseAllowed: '', resortDevelopmentAllowed: '',
    renewableLease: '', subleaseAllowed: '',
    // Availability
    readyForRegistration: '', immediatePossession: '', vacantLand: '',
    immediateOccupancy: '', availableFrom: '', minRentalDuration: '',
    availableImmediately: '', readyForLease: '', shortTermLease: '',
    longTermLease: '', vacantLandLease: '',
    // Nearby Access
    nearbyAccess: [],
    // Contact
  ...
  const nearbyNatureOptions = [
    'Hill', 'Forest', 'River', 'Lake', 'Beach'
  ];

  const scenicFeaturesOptions = [
    'Forest View', 'Hill View', 'River View', 'Lake View', 'Beach View',
    'Valley View', 'Waterfall Nearby', 'Green Belt View', 'Wildlife Area Nearby',
    'Nature Trail Access'
  ];

  const preferredUsageOptions = {
    buy: [
      'Eco Resort', 'Nature Resort', 'Farm Stay', 'Camping Site',
      'Glamping Site', 'Adventure Tourism', 'Wellness Retreat',
      'Yoga Center', 'Organic Farming', 'Holiday Resort', 'Hospitality Development'
    ],
    rent: [
      'Eco Resort', 'Nature Resort', 'Farm Stay', 'Glamping Site',
      'Camping Site', 'Adventure Tourism', 'Wellness Retreat',
      'Yoga Center', 'Organic Farming', 'Event Venue', 'Commercial Eco Project'
    ],
    lease: [
      'Eco Resort', 'Nature Resort', 'Glamping Site', 'Camping Site',
      'Farm Stay', 'Holiday Resort', 'Adventure Tourism', 'Wellness Retreat',
      'Yoga & Meditation Center', 'Organic Farm', 'Eco Village',
      'Hospitality Development', 'Educational Nature Camp', 'Adventure Park',
      'Outdoor Event Venue'
    ]
  };
  ...
  const nearbyOptions = {
    buy: [
      'Highway Access', 'Forest Area', 'Hill Station', 'River',
      'Lake', 'Beach', 'Tourist Attractions', 'Bus Stop',
      'Railway Station', 'Airport', 'Hospitals', 'Market Area', 'Bank / ATM'
    ],
    rent: [
      'Highway Access', 'Forest Area', 'Hill Station', 'River',
      'Lake', 'Beach', 'Tourist Attractions', 'Bus Stop',
    ...
    lease: [
      'Highway Access', 'Bus Stop', 'Railway Station', 'Airport',
      'Forest Area', 'Hill Station', 'River', 'Lake', 'Beach',
      'Tourist Attractions', 'Hospitals', 'Market Area', 'Bank / ATM'
    ]
  };
  ...
  const mainSections = [
    ...
    { id: 'nature', label: '🌿 Nature', icon: <TreePine className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Eco Tourism Land' };
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          <div>
            ...
            <input type="text" value="Eco Tourism Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          ...
          <div className="md:col-span-2">
            <CheckboxGroup 
              label="Nearby Nature" 
              options={nearbyNatureOptions} 
              values={filters.nearbyNature} 
              onChange={(val) => handleInputChange('nearbyNature', val)} 
              columns={5} 
            />
          </div>
  ...
  const renderNatureSection = () => {
    if (currentTab !== 'Lease') return null;
    return (
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><TreePine className="w-3.5 h-3.5" /> Nature & Scenic Features</h3>
        <CheckboxGroup 
          options={scenicFeaturesOptions} 
          values={filters.scenicFeatures} 
          onChange={(val) => handleInputChange('scenicFeatures', val)} 
          columns={2} 
        />
      </div>
    );
  ...
  const renderInfrastructureSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        ...
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.solarPowerReady} onChange={(e) => handleCheckboxChange('solarPowerReady', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
  ...
  const renderAmenitiesSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        <YesNoRadioGroup label="Gated Property" name="gatedPropertyAmenity" value={filters.gatedPropertyAmenity} onChange={(val) => handleRadioChange('gatedPropertyAmenity', val)} />
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          <label className="flex items-center gap-1.5 cursor-pointer group">
          ...
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <input type="checkbox" checked={filters.walkingTrails} onChange={(e) => handleCheckboxChange('walkingTrails', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">Walking Trails</span>
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer group">
          ...
          {currentTab === 'Lease' && (
            <>
              <label className="flex items-center gap-1.5 cursor-pointer group">
                <input type="checkbox" checked={filters.solarLighting} onChange={(e) => handleCheckboxChange('solarLighting', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
                <span className="text-xs text-gray-700 group-hover:text-teal-500">Solar Lighting</span>
              </label>
              <label className="flex items-center gap-1.5 cursor-pointer group">
                <input type="checkbox" checked={filters.childrensPlayArea} onChange={(e) => handleCheckboxChange('childrensPlayArea', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
                <span className="text-xs text-gray-700 group-hover:text-teal-500">Children's Play Area</span>
              </label>
            </>
          )}
        </div>
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
            ...
            <YesNoRadioGroup label="Tourism License Eligible" name="tourismLicenseEligible" value={filters.tourismLicenseEligible} onChange={(val) => handleRadioChange('tourismLicenseEligible', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
            ...
            <YesNoRadioGroup label="Resort Development Allowed" name="resortDevelopmentAllowed" value={filters.resortDevelopmentAllowed} onChange={(val) => handleRadioChange('resortDevelopmentAllowed', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Vacant Land" name="vacantLandLease" value={filters.vacantLandLease} onChange={(val) => handleRadioChange('vacantLandLease', val)} />
          </div>
        </div>
      );
    }
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <TreePine className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-lg">Filter Eco Tourism Land</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'nature' && renderNatureSection()}
...
export default EcoTourismLandFilter;