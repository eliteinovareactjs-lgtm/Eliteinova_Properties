...
import {
  ...
  Leaf, Coffee, Tent, Anchor, Ship, Umbrella, Wind, Droplet, MountainSnow
} from 'lucide-react';
...
const BeachSidePlotFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    beachName: '', distanceToBeach: '', beachFacing: '', mainRoadFacing: '', 
    cornerPlot: '', gatedCommunity: '', nearbyConnectivity: '',
    // Rent
    ...
    clubHouse: false, childrensPark: false, walkingTrack: false, 
    landscapedGarden: false, visitorParking: false, rainwaterHarvesting: false,
    ...
    // Legal for Lease
    commercialLeaseAllowed: '', resortDevelopmentAllowed: '', 
    ...
    crzZone: '', floodRisk: '', erosionRisk: '', elevationAboveSea: '',
    // Documents
    patta: false, chitta: false, fmbSketch: false, surveyNumber: '',
    // Additional
    threePhasePower: false, solarReady: false, privateBeachAccess: false
  });
  ...
  const beachViewOptions = [
    { value: 'Direct', label: 'Direct Beach View' },
    { value: 'Partial', label: 'Partial Beach View' },
    { value: 'Ocean', label: 'Ocean View' },
    { value: 'Sea', label: 'Sea View' }
  ];

  const viewTypeOptions = [
    'Direct Beach View', 'Partial Beach View', 'Beach Access',
    'Sea View', 'Ocean View', 'Sunrise View', 'Sunset View',
    'Garden View', 'Green Belt View', 'Waterfront Property'
  ];

  const preferredUsageOptions = {
    buy: [
      'Residential House', 'Luxury Villa', 'Beach Villa', 'Holiday Home',
      'Weekend Home', 'Resort Development', 'Hotel', 'Eco Tourism',
      'Commercial Development', 'Mixed-Use Development'
    ],
    rent: [
      'Beach Resort', 'Holiday Home', 'Weekend Home', 'Beach Villa',
      'Camping Site', 'Event Venue', 'Restaurant / Café', 'Eco Tourism',
      'Commercial Yard', 'Temporary Office'
    ],
    lease: [
      'Beach Resort', 'Luxury Villa', 'Beach Villa', 'Holiday Home',
      'Weekend Home', 'Resort Development', 'Hotel', 'Eco Tourism Project',
      'Water Sports Business', 'Beach Club', 'Restaurant / Café',
      'Commercial Development', 'Mixed-Use Development', 'Event Venue',
      'Camping Site', 'Glamping Site', 'Film Shooting Location'
    ]
  };

  const nearbyOptions = {
    buy: [
      'Beach', 'Marina / Harbor', 'Fishing Harbor', 'Bus Stop',
      'Railway Station', 'Airport', 'Highway Access', 'Schools & Colleges',
      'Hospitals', 'Shopping Mall', 'Market Area', 'Bank / ATM',
      'Tourist Attractions'
    ],
    rent: [
      'Beach', 'Marina / Harbor', 'Fishing Harbor', 'Bus Stop',
    ...
    lease: [
      'Beach', 'Marina / Harbor', 'Fishing Harbor', 'Bus Stop',
      'Railway Station', 'Airport', 'Highway Access', 'National Highway',
      'Schools & Colleges', 'Hospitals', 'Shopping Mall', 'Market Area',
      'Bank / ATM', 'Tourist Attractions'
    ]
  };
  ...
  const crzZoneOptions = [
    { value: 'CRZ-I', label: 'CRZ-I (Ecologically Sensitive)' },
    { value: 'CRZ-II', label: 'CRZ-II (Urban/Developed Areas)' },
    { value: 'CRZ-III', label: 'CRZ-III (Rural/Undisturbed Areas)' },
    { value: 'CRZ-IV', label: 'CRZ-IV (Water Areas)' }
  ];
  ...
  const mainSections = [
    ...
    { id: 'view', label: '👁️ View', icon: <Eye className="w-3.5 h-3.5" /> },
    ...
    { id: 'environmental', label: '🌊 Environmental', icon: <Waves className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Beach Side Plot' };
    ...
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          <div>
            ...
            <input type="text" value="Beach Side Plot" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          ...
          <input type="text" placeholder="Beach Name" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.beachName} onChange={(e) => handleInputChange('beachName', e.target.value)} />
          <input type="text" placeholder="Distance to Beach (km)" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.distanceToBeach} onChange={(e) => handleInputChange('distanceToBeach', e.target.value)} />
          <YesNoRadioGroup label="Beach Facing" name="beachFacing" value={filters.beachFacing} onChange={(val) => handleRadioChange('beachFacing', val)} />
    ...
    return (
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm">
          <Eye className="w-3.5 h-3.5" /> View & Location Features
        </h3>
        <div className="space-y-2">
          <RadioGroup 
            name="beachView" 
            options={beachViewOptions} 
            value={filters.beachView} 
            onChange={(val) => handleRadioChange('beachView', val)} 
          />
          <CheckboxGroup 
            label="View Type" 
            options={getViewOptions()} 
            values={filters.viewType} 
            onChange={(val) => handleInputChange('viewType', val)} 
            columns={3} 
          />
        </div>
      </div>
  ...
  const renderUsageSection = () => {
    ...
    return (
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm">
          {currentTab === 'Buy' ? <Building className="w-3.5 h-3.5" /> : currentTab === 'Rent' ? <Building className="w-3.5 h-3.5" /> : <Building className="w-3.5 h-3.5" />}
  ...
  const renderInfrastructureSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        ...
        {(currentTab === 'Lease' || currentTab === 'Buy') && (
          <label className="flex items-center gap-1.5 cursor-pointer group">
        ...
        {(currentTab === 'Lease' || currentTab === 'Buy') && (
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <input type="checkbox" checked={filters.solarReady} onChange={(e) => handleCheckboxChange('solarReady', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">Solar Power Ready</span>
          </label>
        )}
      </div>
    </div>
  ...
  const renderAmenitiesSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        <YesNoRadioGroup label="Gated Community" name="gatedCommunityAmenity" value={filters.gatedCommunityAmenity} onChange={(val) => handleRadioChange('gatedCommunityAmenity', val)} />
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          <label className="flex items-center gap-1.5 cursor-pointer group">
          ...
          {currentTab === 'Lease' && (
            <>
              ...
              <label className="flex items-center gap-1.5 cursor-pointer group">
                <input type="checkbox" checked={filters.privateBeachAccess} onChange={(e) => handleCheckboxChange('privateBeachAccess', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
                <span className="text-xs text-gray-700 group-hover:text-teal-500">Private Beach Access</span>
              </label>
            </>
          )}
        </div>
    ...
    if (currentTab === 'Rent') {
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
            ...
            <YesNoRadioGroup label="CRZ Clearance" name="crzClearance" value={filters.crzClearance} onChange={(val) => handleRadioChange('crzClearance', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
            <YesNoRadioGroup label="Title Deed Verified" name="titleDeedVerified" value={filters.titleDeedVerified} onChange={(val) => handleRadioChange('titleDeedVerified', val)} />
            <YesNoRadioGroup label="Lease Agreement Available" name="rentalAgreementReady" value={filters.rentalAgreementReady} onChange={(val) => handleRadioChange('rentalAgreementReady', val)} />
            <YesNoRadioGroup label="RERA Approved" name="reraApproved" value={filters.reraApproved} onChange={(val) => handleRadioChange('reraApproved', val)} />
            <YesNoRadioGroup label="DTCP Approved" name="dtcpApproved" value={filters.dtcpApproved} onChange={(val) => handleRadioChange('dtcpApproved', val)} />
            <YesNoRadioGroup label="CMDA Approved" name="cmdaApproved" value={filters.cmdaApproved} onChange={(val) => handleRadioChange('cmdaApproved', val)} />
            <YesNoRadioGroup label="Panchayat Approved" name="panchayatApproved" value={filters.panchayatApproved} onChange={(val) => handleRadioChange('panchayatApproved', val)} />
    ...
    if (currentTab === 'Rent') {
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Clock className="w-3.5 h-3.5" /> Availability</h3>
          <div className="space-y-2">
            <YesNoRadioGroup label="Immediate Lease Available" name="availableImmediately" value={filters.availableImmediately} onChange={(val) => handleRadioChange('availableImmediately', val)} />
            <YesNoRadioGroup label="Ready for Occupation" name="readyForLease" value={filters.readyForLease} onChange={(val) => handleRadioChange('readyForLease', val)} />
            <YesNoRadioGroup label="Vacant Plot" name="vacantPlotLease" value={filters.vacantPlotLease} onChange={(val) => handleRadioChange('vacantPlotLease', val)} />
  ...
  const renderEnvironmentalSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Waves className="w-3.5 h-3.5" /> Environmental & Coastal Features</h3>
      <div className="space-y-2">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          <CustomSelect label="CRZ Zone Classification" options={crzZoneOptions} value={filters.crzZone} onChange={(val) => handleInputChange('crzZone', val)} placeholder="Select CRZ Zone" />
          <CustomSelect label="Flood Risk Level" options={floodRiskOptions} value={filters.floodRisk} onChange={(val) => handleInputChange('floodRisk', val)} placeholder="Select Risk Level" />
          <CustomSelect label="Coastal Erosion Risk" options={erosionRiskOptions} value={filters.erosionRisk} onChange={(val) => handleInputChange('erosionRisk', val)} placeholder="Select Risk Level" />
          <input type="text" placeholder="Elevation Above Sea Level (m)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.elevationAboveSea} onChange={(e) => handleInputChange('elevationAboveSea', e.target.value)} />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          <YesNoRadioGroup label="Tsunami Safe Zone" name="tsunamiSafe" value={filters.tsunamiSafe} onChange={(val) => handleRadioChange('tsunamiSafe', val)} />
          <input type="text" placeholder="Sand Quality" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.sandQuality} onChange={(e) => handleInputChange('sandQuality', e.target.value)} />
        </div>
      </div>
    </div>
  );
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
        <div className="sticky top-0 z-10 bg-white border-b border-teal-100">
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-white font-semibold text-lg">Filter Beach Side Plot</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'environmental' && renderEnvironmentalSection()}
...
export default BeachSidePlotFilter;