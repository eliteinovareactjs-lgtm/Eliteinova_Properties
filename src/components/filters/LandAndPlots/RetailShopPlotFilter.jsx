// RetailShopPlotFilter.jsx
import React, { useState, useRef, useEffect } from 'react';
import { 
  X, ChevronDown, Building, MapPin, IndianRupee, Ruler, Shield, 
  Phone, FileText, RefreshCw, DollarSign, Calendar, Zap, 
  Square, Home, Bus, CheckCircle, TrendingUp, Clock, Store, ShoppingBag, 
  ShoppingCart, Pill, Coffee, Sofa, Scissors, Wifi, Car, Lock, Camera, 
  Flame, Truck, Trees, Users
} from 'lucide-react';
...
const RetailShopPlotFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    streetLights: false, parkingSpace: false, internetReady: false, deliveryVehicleAccess: false,
    
    // Amenities
    gatedLayout: false, security247: false, cctvSurveillance: false, visitorParking: false,
    wideAccessRoad: false, fireSafety: false,
  ...
  const retailSuitabilityOptions = [
    'Grocery Store', 'Supermarket', 'Fashion Store', 'Mobile / Electronics Shop',
    'Medical Shop', 'Bakery / Sweet Shop', 'Furniture Showroom', 'Franchise Outlet', 'Multi-brand Retail Store'
  ];

  const nearbyAccessOptions = [
    'Bus Stop / Metro', 'Railway Station', 'Residential Area',
    'Shopping Zone', 'Commercial Hub', 'Highway Access'
  ];
  ...
  const mainSections = [
    ...
    { id: 'retail', label: '🏪 Retail', icon: <Store className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Retail Shop Plot' };
    ...
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Retail Shop Plot" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <input type="text" placeholder="Area / Market Area / Locality" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.area} onChange={(e) => handleInputChange('area', e.target.value)} />
  ...
  const renderRetailSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Store className="w-3.5 h-3.5" /> Retail Suitability</h3>
      <CheckboxGroup options={retailSuitabilityOptions} values={filters.retailSuitability} onChange={(val) => handleInputChange('retailSuitability', val)} columns={2} />
    </div>
  );
  ...
  const renderUtilitiesSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        ...
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.deliveryVehicleAccess} onChange={(e) => handleCheckboxChange('deliveryVehicleAccess', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Delivery Access</span>
        </label>
      </div>
    </div>
  );
  ...
  const renderAmenitiesSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        ...
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.wideAccessRoad} onChange={(e) => handleCheckboxChange('wideAccessRoad', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Wide Access Road</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
    ...
    <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
      <div className="sticky top-0 z-10 bg-white border-b border-teal-100">
        <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-white font-semibold text-lg">Filter Retail Shop Plot</h3>
          </div>
          {onClose && (
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="p-3 space-y-3">
          ...
          {activeMainSection === 'retail' && renderRetailSection()}
...
export default RetailShopPlotFilter;