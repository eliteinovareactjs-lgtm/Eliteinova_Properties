...
const SchoolCollegeLandFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    mainRoadAccess: '', cornerProperty: '', nearbyConnectivity: '',
    ...
    landShape: '', landLevel: '', facing: '', roadWidth: '', boundaryWall: '', playgroundSpace: '',
    // Institution Suitability
    institutionSuitability: [],
    ...
    educationalZoningApproved: '', landConversionApproved: '',
    encumbranceFree: '', loanEligible: '', landSurveyCompleted: '', titleDeedVerified: '',
    rentalAgreementAvailable: '', leaseAgreementAvailable: '',
    // Availability
    immediateOccupancy: '', vacantLandAvailable: '',
    institutionReadyLand: '', longTermAvailable: '', shortTermAvailable: '',
    availableFrom: '',
    readyToRegister: '', immediatePossession: '', developedCampusLand: '',
  ...
  const schoolPurposeOptions = [
    'Primary School', 'Middle School', 'High School', 'Higher Secondary School',
    'CBSE School', 'ICSE School', 'State Board School', 'International School',
    'Residential School'
  ];

  const collegePurposeOptions = [
    'Arts & Science College', 'Engineering College', 'Medical College',
    'Nursing College', 'Polytechnic College', 'Law College',
    'Management Institute', 'Teacher Training Institute', 'University Campus'
  ];

  const institutionSuitabilityOptions = [
    ...schoolPurposeOptions,
    ...collegePurposeOptions
  ];

  const infrastructureUtilitiesOptions = [
    'Water Connection Available', 'Borewell Facility', 'Electricity Connection Available',
    '3-Phase Power Supply', 'Drainage Connection Available', 'Sewage System Available',
    'Internet / Fiber Connectivity', 'Internal Roads Available', 'Street Lighting Available'
  ];

  const campusFacilitiesOptions = [
    'Playground / Sports Ground', 'Auditorium Space', 'Laboratory Block Area',
    'Library Block Area', 'Administrative Block Area', 'Hostel Development Space',
    'Staff Quarters Space', 'Parking Area', 'Cafeteria / Food Court Area'
  ];

  const nearbyOptions = [
    'Residential Area', 'Bus Stand', 'Railway Station', 'Metro Station',
    'National Highway', 'State Highway', 'Airport', 'Hospital',
    'Commercial Area', 'Student Residential Area'
  ];

  const developmentPotentialOptions = [
    'Educational Hub', 'University Zone', 'Residential Growth Area',
    'Smart City Zone', 'Future Development Corridor', 'High Student Population Area'
  ];

  const tenantPreferencesOptions = [
    'School Management', 'College Management', 'Educational Trust',
    'University Administration', 'Corporate Education Group',
    'Religious / Charitable Trust', 'Educational Institution Operator',
    'Training Institute Operator'
  ];

  const buyerPreferencesOptions = [
    'School Management', 'College Management', 'Educational Trust',
    'University Administration', 'Corporate Education Group',
    'Religious / Charitable Trust', 'Educational Institution Investor'
  ];
  ...
  const mainSections = [
    ...
    { id: 'institution', label: '🎓 Institution', icon: <School className="w-3.5 h-3.5" /> },
    { id: 'infrastructure', label: '⚡ Utilities', icon: <Zap className="w-3.5 h-3.5" /> },
    { id: 'campus', label: '🏫 Campus', icon: <Building2 className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'School / College Land' };
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="School / College Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
    ...
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <YesNoRadioGroup label="Playground Space Available" name="playgroundSpace" value={filters.playgroundSpace} onChange={(val) => handleRadioChange('playgroundSpace', val)} />
        </div>
      </div>
    </div>
  );
  ...
  const renderInstitutionSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><School className="w-3.5 h-3.5" /> Institution Suitability</h3>
      <div className="grid grid-cols-2 gap-1.5">
        {institutionSuitabilityOptions.map(option => (
          <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
            <input
              type="checkbox"
              checked={filters.institutionSuitability.includes(option)}
              onChange={() => handleArrayToggle('institutionSuitability', option)}
              className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
            />
  ...
  const renderCampusSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Building2 className="w-3.5 h-3.5" /> Campus Facilities Potential</h3>
      <div className="grid grid-cols-2 gap-1.5">
        {campusFacilitiesOptions.map(option => (
          <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
            <input
              type="checkbox"
              checked={filters.campusFacilities.includes(option)}
              onChange={() => handleArrayToggle('campusFacilities', option)}
              className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
            />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">{option}</span>
          </label>
  ...
  const renderLegalSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <YesNoRadioGroup label="Educational Institution Zoning Approved" name="educationalZoningApproved" value={filters.educationalZoningApproved} onChange={(val) => handleRadioChange('educationalZoningApproved', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Developed Campus Land Preferred" name="developedCampusLand" value={filters.developedCampusLand} onChange={(val) => handleRadioChange('developedCampusLand', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Vacant Land Available" name="vacantLandAvailable" value={filters.vacantLandAvailable} onChange={(val) => handleRadioChange('vacantLandAvailable', val)} />
            <YesNoRadioGroup label="Developed Campus Land Available" name="developedCampusLand" value={filters.developedCampusLand} onChange={(val) => handleRadioChange('developedCampusLand', val)} />
            <YesNoRadioGroup label="Institution-Ready Land Available" name="institutionReadyLand" value={filters.institutionReadyLand} onChange={(val) => handleRadioChange('institutionReadyLand', val)} />
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <School className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-sm">Filter School / College Land</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'institution' && renderInstitutionSection()}
            {activeMainSection === 'infrastructure' && renderInfrastructureSection()}
            {activeMainSection === 'campus' && renderCampusSection()}
