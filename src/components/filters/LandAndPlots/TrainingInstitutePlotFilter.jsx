...
import { 
  ...
  Laptop, Monitor, Wifi as WifiIcon, Coffee, Users as UsersIcon
} from 'lucide-react';
...
const TrainingInstitutePlotFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    plotShape: '', plotLevel: '', facing: '', roadWidth: '', cornerPlotAvailable: '', boundaryWall: '',
    // Training Institute Suitability
    trainingSuitability: [],
    ...
    educationalUseApproved: '', landConversionApproved: '',
    ...
    vacantPlotAvailable: '', institutionReadyPlot: '',
    readyToRegister: '', immediatePossession: '', approvedEducationalPlot: '',
  ...
  const trainingSuitabilityOptions = [
    // Educational Training
    'Coaching Center', 'Tuition Center', 'Competitive Exam Academy',
    'Language Training Institute', 'Computer Training Center',
    'Skill Development Center', 'Vocational Training Institute',
    // Professional Training
    'IT Training Institute', 'Software Training Center',
    'Corporate Training Center', 'Technical Training Institute',
    'Industrial Training Institute (ITI)', 'Polytechnic Training Center',
    'Management Training Institute',
    // Specialized Training
    'Aviation Training Academy', 'Nursing Training Center',
    'Medical Training Institute', 'Teacher Training Institute',
    'Hospitality Training Institute', 'Fashion Design Institute',
    'Multimedia & Animation Institute'
  ];

  const infrastructureUtilitiesOptions = [
    'Water Connection Available', 'Borewell Facility', 'Electricity Connection Available',
    '3-Phase Power Supply', 'Internet / Fiber Connectivity', 'Drainage Connection Available',
    'Sewage System Available', 'Internal Roads Available', 'Street Lighting Available'
  ];

  const campusFacilitiesOptions = [
    'Classroom Development Area', 'Computer Lab Space', 'Workshop / Practical Lab Area',
    'Auditorium Space', 'Seminar Hall Area', 'Library Space',
    'Administrative Office Area', 'Hostel Development Space',
    'Cafeteria Area', 'Parking Area'
  ];

  const amenitiesOptions = [
    'Gated Property', 'Security Available', 'CCTV Surveillance',
    'Landscaped Area', 'Student Waiting Area', 'Visitor Parking',
    'Staff Parking', 'Open Ground Area'
  ];

  const nearbyOptions = [
    'Residential Area', 'School / College', 'University Campus',
    'Bus Stand', 'Railway Station', 'Metro Station',
    'National Highway', 'State Highway', 'Airport', 'Commercial Area'
  ];

  const developmentPotentialOptions = [
    'Educational Hub', 'Skill Development Zone', 'University Zone',
    'Residential Growth Area', 'Smart City Zone', 'High Student Population Area',
    'Future Development Corridor'
  ];

  const tenantPreferencesOptions = [
    'Educational Trust', 'Training Institute Operator', 'Coaching Center Management',
    'Corporate Training Provider', 'Skill Development Organization',
    'Vocational Training Center', 'Educational Franchise Operator',
    'Educational NGO'
  ];
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Training Institute Plot' };
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Training Institute Plot" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
    ...
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <CustomSelect label="Plot Level" options={plotLevelOptions} value={filters.plotLevel} onChange={(val) => handleInputChange('plotLevel', val)} placeholder="Select Level" />
  ...
  const renderTrainingSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><GraduationCap className="w-3.5 h-3.5" /> Training Institute Suitability</h3>
      <div className="grid grid-cols-2 gap-1.5">
        {trainingSuitabilityOptions.map(option => (
          <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
            <input
              type="checkbox"
              checked={filters.trainingSuitability.includes(option)}
              onChange={() => handleArrayToggle('trainingSuitability', option)}
              className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
            />
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <GraduationCap className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-sm">Filter Training Institute Plot</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'training' && renderTrainingSection()}
