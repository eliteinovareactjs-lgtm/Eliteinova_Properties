...
import {
  ...
  Server, Network, Database, Globe, Cpu, Package, Box, TruckIcon,
  Dock, Ruler as RulerIcon, Gauge, HardHat, HomeIcon
} from 'lucide-react';
...
// Warehouse Area Unit Select
const WarehouseAreaUnitSelect = ({ value, onChange }) => {
  const [isOpen, setIsOpen] = useState(false);
  const selectRef = useRef(null);
  const units = [
...
const WarehouseLandFilter = ({ activeTab = 'Lease', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    listingType: [],
    // Location
    city: '', industrialArea: '', locality: '', landmark: '', pincode: '', highwayAccess: '', mainRoadFacing: '', nearbyConnectivity: '',
    // Rent
    minRent: '', maxRent: '', securityDeposit: '', rentAdvanceAmount: '', maintenanceIncludedRent: '', rentNegotiable: '',
    // Buy
    minBudget: '', maxBudget: '', preferredPricePerSqft: '', preferredPricePerAcre: '', loanRequired: '', priceNegotiable: '',
    // Sell
    minSellPrice: '', maxSellPrice: '', pricePerSqft: '', pricePerAcre: '', sellPriceNegotiable: '', maintenanceCharges: '', propertyTax: '',
    // Lease
    ...
    // Warehouse Details
    warehouseArea: '', warehouseAreaUnit: 'sqft', builtupArea: '', openStorageArea: '', ceilingHeight: '', loadingBays: '', dockLevel: '', flooringType: '', roadWidthTruck: '',
    // For Buy specific
    builtupAreaRequired: '', openStorageAreaRequired: '', preferredCeilingHeight: '', requiredLoadingBays: '', dockLevelNeeded: '', flooringTypePreference: '', minRoadWidthRequired: '',
    // Warehouse Suitability
    warehouseSuitability: [],
    // Infrastructure & Utilities
    waterConnection: false, electricityConnection: false, threePhasePower: false, drainageFacility: false, internetBroadband: false, generatorBackup: false, fireSafetySystem: false, heavyVehicleAccess: false,
    // Amenities
    gatedComplex: '', security247: false, cctvSurveillance: false, parkingSpace: false, staffRooms: false, restroomFacility: false, weighbridgeAccess: false, internalRoads: false,
    // Legal & Approval
    dtcpApproved: '', cmdaApproved: '', industrialApproval: '', fireNOC: '', pollutionClearance: '', pattaAvailable: '', encumbranceFree: '',
    // Buy specific legal
    encumbranceFreePreferred: '', loanEligiblePropertyRequired: '', titleDeedVerified: '',
    // Sell specific legal
    loanEligible: '',
    // Rent/Lease specific
  ...
  const warehouseSuitabilityOptions = {
    rent: [
      'Logistics & Distribution', 'E-commerce Storage', 'Cold Storage',
      'FMCG Storage', 'Industrial Goods Storage', 'Automobile Storage',
      'Import / Export Goods', 'Bulk Inventory Management'
    ],
    buy: [
      'Logistics & Distribution', 'E-commerce Storage', 'Cold Storage',
      'FMCG Storage', 'Industrial Goods Storage', 'Automobile Storage',
  ...
  const nearbyOptions = {
    rent: [
      'Highway Access', 'Port / Harbour Access', 'Railway Cargo Access',
      'Airport Cargo Access', 'Industrial Area', 'Transport Hub'
    ],
    buy: [
      'Highway Access', 'Port / Harbour Access', 'Railway Cargo Access',
      'Airport Cargo Access', 'Industrial Area', 'Transport Hub'
  ...
  const mainSections = [
    ...
    { id: 'suitability', label: '📦 Suitability', icon: <Package className="w-3.5 h-3.5" /> },
    ...
    { id: 'nearby', label: '🚛 Nearby', icon: <Truck className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Warehouse' };
    ...
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          <div>
            ...
            <input type="text" value="Warehouse" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          ...
          <input type="text" placeholder="Industrial Area / Logistics Zone" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.industrialArea} onChange={(e) => handleInputChange('industrialArea', e.target.value)} />
          <input type="text" placeholder="Locality" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.locality} onChange={(e) => handleInputChange('locality', e.target.value)} />
  ...
  const renderWarehouseSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Warehouse className="w-3.5 h-3.5" /> Warehouse Details</h3>
      <div className="space-y-2">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div className="flex gap-2 items-center min-w-0 overflow-hidden">
            <input type="text" placeholder="Warehouse Area" className="flex-1 min-w-0 px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.warehouseArea} onChange={(e) => handleInputChange('warehouseArea', e.target.value)} />
            <WarehouseAreaUnitSelect value={filters.warehouseAreaUnit} onChange={(val) => handleInputChange('warehouseAreaUnit', val)} />
          </div>
          {currentTab === 'Buy' ? (
            <>
              <input type="text" placeholder="Built-up Area Requirement (sq.ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.builtupAreaRequired} onChange={(e) => handleInputChange('builtupAreaRequired', e.target.value)} />
              <input type="text" placeholder="Open Storage Area Requirement (sq.ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.openStorageAreaRequired} onChange={(e) => handleInputChange('openStorageAreaRequired', e.target.value)} />
            </>
          ) : (
            <>
              <input type="text" placeholder="Built-up Area (sq.ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.builtupArea} onChange={(e) => handleInputChange('builtupArea', e.target.value)} />
              <input type="text" placeholder="Open Storage Area (sq.ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.openStorageArea} onChange={(e) => handleInputChange('openStorageArea', e.target.value)} />
            </>
          )}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
          {currentTab === 'Buy' ? (
            <>
              <input type="text" placeholder="Preferred Ceiling Height (ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.preferredCeilingHeight} onChange={(e) => handleInputChange('preferredCeilingHeight', e.target.value)} />
              <input type="number" placeholder="Required Loading Bays" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.requiredLoadingBays} onChange={(e) => handleInputChange('requiredLoadingBays', e.target.value)} />
            </>
          ) : (
            <>
              <input type="text" placeholder="Ceiling Height (ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.ceilingHeight} onChange={(e) => handleInputChange('ceilingHeight', e.target.value)} />
              <input type="number" placeholder="Number of Loading Bays" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.loadingBays} onChange={(e) => handleInputChange('loadingBays', e.target.value)} />
            </>
          )}
          {currentTab === 'Buy' ? (
            <YesNoRadioGroup label="Dock Level Facility Needed" name="dockLevelNeeded" value={filters.dockLevelNeeded} onChange={(val) => handleRadioChange('dockLevelNeeded', val)} />
          ) : (
            <YesNoRadioGroup label="Dock Level Facility" name="dockLevel" value={filters.dockLevel} onChange={(val) => handleRadioChange('dockLevel', val)} />
          )}
          {currentTab === 'Buy' ? (
            <CustomSelect label="Flooring Type Preference" options={flooringTypeOptions} value={filters.flooringTypePreference} onChange={(val) => handleInputChange('flooringTypePreference', val)} placeholder="Select Flooring Type" />
          ) : (
            <CustomSelect label="Flooring Type" options={flooringTypeOptions} value={filters.flooringType} onChange={(val) => handleInputChange('flooringType', val)} placeholder="Select Flooring Type" />
          )}
          {currentTab === 'Buy' ? (
            <input type="text" placeholder="Min Road Width for Truck Access (ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.minRoadWidthRequired} onChange={(e) => handleInputChange('minRoadWidthRequired', e.target.value)} />
          ) : (
            <input type="text" placeholder="Road Width for Truck Access (ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.roadWidthTruck} onChange={(e) => handleInputChange('roadWidthTruck', e.target.value)} />
          )}
        </div>
      </div>
    </div>
  ...
  const renderSuitabilitySection = () => {
    const options = warehouseSuitabilityOptions[currentTab.toLowerCase()] || warehouseSuitabilityOptions.lease;
    return (
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Package className="w-3.5 h-3.5" /> Warehouse Suitability</h3>
        <CheckboxGroup options={options} values={filters.warehouseSuitability} onChange={(val) => handleInputChange('warehouseSuitability', val)} columns={2} />
      </div>
    );
  };
    ...
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        ...
        <label className="flex items-center gap-1.5 cursor-pointer group">
          ...
          <span className="text-xs text-gray-700 group-hover:text-teal-500">3-Phase Power Supply</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
        ...
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.generatorBackup} onChange={(e) => handleCheckboxChange('generatorBackup', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Generator / Power Backup</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.fireSafetySystem} onChange={(e) => handleCheckboxChange('fireSafetySystem', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Fire Safety System</span>
        </label>
        <label className="flex items-center gap-1.5 cursor-pointer group">
          <input type="checkbox" checked={filters.heavyVehicleAccess} onChange={(e) => handleCheckboxChange('heavyVehicleAccess', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
          <span className="text-xs text-gray-700 group-hover:text-teal-500">Heavy Vehicle Access</span>
        </label>
      </div>
    </div>
  );
  ...
  const renderAmenitiesSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        <YesNoRadioGroup label="Gated Warehouse Complex" name="gatedComplex" value={filters.gatedComplex} onChange={(val) => handleRadioChange('gatedComplex', val)} />
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          <label className="flex items-center gap-1.5 cursor-pointer group">
          ...
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <input type="checkbox" checked={filters.parkingSpace} onChange={(e) => handleCheckboxChange('parkingSpace', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">Parking Space Available</span>
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <input type="checkbox" checked={filters.staffRooms} onChange={(e) => handleCheckboxChange('staffRooms', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">Staff Rooms / Office Space</span>
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <input type="checkbox" checked={filters.restroomFacility} onChange={(e) => handleCheckboxChange('restroomFacility', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">Restroom Facility</span>
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer group">
            <input type="checkbox" checked={filters.weighbridgeAccess} onChange={(e) => handleCheckboxChange('weighbridgeAccess', e.target.checked)} className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500" />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">Weighbridge Access</span>
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer group">
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
            ...
            <YesNoRadioGroup label="Industrial Approval Available" name="industrialApproval" value={filters.industrialApproval} onChange={(val) => handleRadioChange('industrialApproval', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 min-w-0">
            <YesNoRadioGroup label="DTCP Approved" name="dtcpApproved" value={filters.dtcpApproved} onChange={(val) => handleRadioChange('dtcpApproved', val)} />
            <YesNoRadioGroup label="CMDA Approved" name="cmdaApproved" value={filters.cmdaApproved} onChange={(val) => handleRadioChange('cmdaApproved', val)} />
            <YesNoRadioGroup label="Industrial Approval Required" name="industrialApproval" value={filters.industrialApproval} onChange={(val) => handleRadioChange('industrialApproval', val)} />
            <YesNoRadioGroup label="Fire NOC Available" name="fireNOC" value={filters.fireNOC} onChange={(val) => handleRadioChange('fireNOC', val)} />
            <YesNoRadioGroup label="Pollution Clearance Available" name="pollutionClearance" value={filters.pollutionClearance} onChange={(val) => handleRadioChange('pollutionClearance', val)} />
            <YesNoRadioGroup label="Patta Available" name="pattaAvailable" value={filters.pattaAvailable} onChange={(val) => handleRadioChange('pattaAvailable', val)} />
            ...
            <YesNoRadioGroup label="Loan Eligible Property Required" name="loanEligiblePropertyRequired" value={filters.loanEligiblePropertyRequired} onChange={(val) => handleRadioChange('loanEligiblePropertyRequired', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Under Development Industrial Layout" name="underDevelopmentLayout" value={filters.underDevelopmentLayout} onChange={(val) => handleRadioChange('underDevelopmentLayout', val)} />
          </div>
        </div>
      );
    } else {
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <YesNoRadioGroup label="Ready to Register" name="readyToRegister" value={filters.readyToRegister} onChange={(val) => handleRadioChange('readyToRegister', val)} />
            <YesNoRadioGroup label="Immediate Possession" name="immediatePossession" value={filters.immediatePossession} onChange={(val) => handleRadioChange('immediatePossession', val)} />
            <YesNoRadioGroup label="Under Development Industrial Layout Acceptable" name="underDevelopmentAcceptable" value={filters.underDevelopmentAcceptable} onChange={(val) => handleRadioChange('underDevelopmentAcceptable', val)} />
          </div>
        </div>
      );
    }
  ...
  const renderNearbySection = () => {
    const options = nearbyOptions[currentTab.toLowerCase()] || nearbyOptions.lease;
    return (
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Truck className="w-3.5 h-3.5" /> Nearby Access</h3>
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <Warehouse className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-lg">Filter Warehouse</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'warehouse' && renderWarehouseSection()}
