...
import {
  ...
  Droplet as DropletIcon, Wind, Factory
} from 'lucide-react';
...
const DairyLandAreaUnitSelect = ({ value, onChange }) => {
  const [isOpen, setIsOpen] = useState(false);
  const selectRef = useRef(null);
  const units = [
...
const DairyFarmLandFilter = ({ activeTab = 'Lease', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    // Farm Details
    landArea: '', landAreaUnit: 'acres', cattleShedArea: '', cattleCapacity: '', dairyFarmType: '', milkProductionCapacity: '', fodderArea: '', waterSource: '', wasteManagement: '', facing: '', roadWidth: '', boundaryAvailable: '',
    ...
    pattaAvailable: '', dairyApprovalAvailable: '', pollutionClearanceAvailable: '', encumbranceFree: '', loanEligible: '', landSurveyCompleted: '', titleDeedVerified: '',
  ...
  const dairyFarmTypeOptions = [
    { value: 'Cow Farm', label: 'Cow Farm' },
    { value: 'Buffalo Farm', label: 'Buffalo Farm' },
    { value: 'Mixed Dairy Farm', label: 'Mixed Dairy Farm' }
  ];

  const farmSuitabilityOptions = [
    'Milk Production', 'Organic Dairy Farming', 'Cattle Breeding',
    'Buffalo Farming', 'Fodder Farming', 'Dairy Processing Unit',
    'Biogas Production'
  ];

  const infrastructureUtilitiesOptions = [
    'Water Connection Available', 'Borewell Facility', 'Electricity Connection Available',
    '3-Phase Power Supply', 'Generator / Power Backup', 'Drainage Facility',
    'Storage Room Available', 'Chilling Unit Available', 'Worker Accommodation',
    'Heavy Vehicle Access'
  ];

  const amenitiesOptions = [
    'Gated Farm Property', 'Security / Caretaker', 'CCTV Surveillance',
    'Internal Farm Roads', 'Parking Space Available'
  ];
  ...
  const nearbyOptions = [
    'Highway Access', 'Nearby Town / Market', 'Veterinary Hospital Nearby',
    'Milk Collection Center Nearby', 'Transport Access'
  ];
  ...
  const mainSections = [
    ...
    { id: 'farm', label: '🐄 Dairy Farm', icon: <Milk className="w-3.5 h-3.5" /> },
    { id: 'farmSuitability', label: '🌾 Suitability', icon: <Leaf className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Dairy Farm Land' };
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Dairy Farm Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
  ...
  const renderFarmSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Milk className="w-3.5 h-3.5" /> Dairy Farm Details</h3>
      <div className="space-y-2">
        <div className="grid grid-cols-4 gap-2 items-end">
          ...
          <DairyLandAreaUnitSelect value={filters.landAreaUnit} onChange={(val) => handleInputChange('landAreaUnit', val)} />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <input type="text" placeholder="Cattle Shed Area" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.cattleShedArea} onChange={(e) => handleInputChange('cattleShedArea', e.target.value)} />
          <input type="number" placeholder="Number of Cattle Capacity" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.cattleCapacity} onChange={(e) => handleInputChange('cattleCapacity', e.target.value)} />
          <CustomSelect label="Dairy Farm Type" options={dairyFarmTypeOptions} value={filters.dairyFarmType} onChange={(val) => handleInputChange('dairyFarmType', val)} placeholder="Select Type" />
          <input type="text" placeholder="Milk Production Capacity" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.milkProductionCapacity} onChange={(e) => handleInputChange('milkProductionCapacity', e.target.value)} />
          <input type="text" placeholder="Fodder Cultivation Area" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.fodderArea} onChange={(e) => handleInputChange('fodderArea', e.target.value)} />
          ...
          <YesNoRadioGroup label="Waste Management Facility Available" name="wasteManagement" value={filters.wasteManagement} onChange={(val) => handleRadioChange('wasteManagement', val)} />
  ...
  const renderFarmSuitabilitySection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Leaf className="w-3.5 h-3.5" /> Farm Suitability</h3>
      <CheckboxGroup options={farmSuitabilityOptions} values={filters.farmSuitability} onChange={(val) => handleInputChange('farmSuitability', val)} columns={2} />
    </div>
  );
  ...
  const renderLegalSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <YesNoRadioGroup label="Agricultural / Dairy Approval Available" name="dairyApprovalAvailable" value={filters.dairyApprovalAvailable} onChange={(val) => handleRadioChange('dairyApprovalAvailable', val)} />
        <YesNoRadioGroup label="Pollution Clearance Available" name="pollutionClearanceAvailable" value={filters.pollutionClearanceAvailable} onChange={(val) => handleRadioChange('pollutionClearanceAvailable', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Under Development Dairy Farm Layout" name="underDevelopmentLayout" value={filters.underDevelopmentLayout} onChange={(val) => handleRadioChange('underDevelopmentLayout', val)} />
          </div>
        </div>
      );
    } else { // Buy
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <YesNoRadioGroup label="Ready to Register" name="readyToRegister" value={filters.readyToRegister} onChange={(val) => handleRadioChange('readyToRegister', val)} />
            <YesNoRadioGroup label="Immediate Possession" name="immediatePossession" value={filters.immediatePossession} onChange={(val) => handleRadioChange('immediatePossession', val)} />
            <YesNoRadioGroup label="Under Development Dairy Farm Layout Acceptable" name="underDevelopmentLayout" value={filters.underDevelopmentLayout} onChange={(val) => handleRadioChange('underDevelopmentLayout', val)} />
          </div>
        </div>
      );
    }
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
        <div className="sticky top-0 z-10 bg-white border-b border-teal-100">
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-white font-semibold text-sm">Filter Dairy Farm Land</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'farmSuitability' && renderFarmSuitabilitySection()}
