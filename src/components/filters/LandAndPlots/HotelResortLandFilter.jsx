...
import { 
  ...
  Square, Home, Bus, CheckCircle, TrendingUp, Clock, Hotel, 
  Car, Wifi, Camera, Flame, Users, Train, ExternalLink, 
  Umbrella, Mountain, Trees, Coffee, Sparkles
} from 'lucide-react';
...
const HotelResortLandFilter = ({ activeTab = 'Rent', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    listingType: [],
    city: '', area: '', landmark: '', pincode: '', highwayFacing: '', scenicFacing: '',
    nearbyConnectivity: '',
    ...
    landArea: '', landAreaUnit: 'acres', plotLength: '', plotWidth: '', facing: '', cornerLand: '', roadWidth: '', boundaryWall: '', plotShape: '',
    minRoadWidthRequired: '', plotShapePreference: '',
    dtcpApproved: '', cmdaApproved: '', reraApproved: '', tourismApproval: '', pattaAvailable: '', encumbranceFree: '',
    rentalAgreementReady: '', leaseAgreementReady: '', loanEligible: '', titleDeedVerified: '',
    waterConnection: '', borewellFacility: '', electricityConnection: '', drainageFacility: '', internetBroadband: '', generatorBackup: '', parkingSpace: '', heavyVehicleAccess: '',
    gatedProperty: '', security24x7: '', cctvSurveillance: '', swimmingPoolProvision: '', gardenLandscaped: '', clubhouseRecreation: '', restaurantDiningProvision: '', outdoorActivityArea: '',
    suitableForTypes: [],
  ...
  const nearbyOptions = [
    'Airport Access', 'Railway Station', 'Tourist Attractions',
    'Highway Access', 'Beach / Hill Station', 'Market / Shopping Area'
  ];
  ...
  const mainSections = [
    ...
    { id: 'land', label: '🗺️ Land', icon: <SquareIcon className="w-3.5 h-3.5" /> },
    { id: 'legal', label: '⚖️ Legal', icon: <Shield className="w-3.5 h-3.5" /> },
    { id: 'utilities', label: '💡 Utilities', icon: <Zap className="w-3.5 h-3.5" /> },
    { id: 'amenities', label: '🏨 Amenities', icon: <Hotel className="w-3.5 h-3.5" /> },
    { id: 'suitable', label: '🏗️ Suitability', icon: <Umbrella className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Hotel / Resort Land' };
  ...
  const clearAllFilters = () => {
    setFilters({
      listingType: [], city: '', area: '', landmark: '', pincode: '', highwayFacing: '', scenicFacing: '',
      nearbyConnectivity: '',
      minRent: '', maxRent: '', securityDeposit: '', advanceAmount: '', maintenanceIncluded: '', rentNegotiable: '',
      minBudget: '', maxBudget: '', preferredPricePerUnit: '', loanRequired: '', priceNegotiable: '',
      minSellPrice: '', maxSellPrice: '', pricePerUnit: '', sellPriceNegotiable: '', maintenanceCharges: '', propertyTax: '',
  ...
  const renderBasicSection = () => (
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Hotel / Resort Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <input type="text" placeholder="City / Tourist Destination" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.city} onChange={(e) => handleInputChange('city', e.target.value)} />
          ...
          <div className="md:col-span-2 grid grid-cols-2 gap-2">
            ...
            <YesNoRadioGroup label="Beach / Hill / Lake Facing" name="scenicFacing" value={filters.scenicFacing} onChange={(val) => handleRadioChange('scenicFacing', val)} />
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <input type="text" placeholder="Road Width Facing the Land (ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.roadWidth} onChange={(e) => handleInputChange('roadWidth', e.target.value)} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        {(currentTab === 'Rent' || currentTab === 'Lease') ? (
          ...
          <>
            ...
            <YesNoRadioGroup label="RERA Approved Required" name="reraApproved" value={filters.reraApproved} onChange={(val) => handleRadioChange('reraApproved', val)} />
            <YesNoRadioGroup label="Commercial / Tourism Approval Required" name="tourismApproval" value={filters.tourismApproval} onChange={(val) => handleRadioChange('tourismApproval', val)} />
            ...
            <YesNoRadioGroup label="Loan Eligible Land Required" name="loanEligible" value={filters.loanEligible} onChange={(val) => handleRadioChange('loanEligible', val)} />
          </>
        ) : null}
      </div>
    </div>
  ...
  const renderUtilitiesSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        ...
        <YesNoRadioGroup label="Generator / Power Backup Provision" name="generatorBackup" value={filters.generatorBackup} onChange={(val) => handleRadioChange('generatorBackup', val)} />
  ...
  const renderAmenitiesSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Hotel className="w-3.5 h-3.5" /> Amenities</h3>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
        ...
        <YesNoRadioGroup label="Swimming Pool Provision" name="swimmingPoolProvision" value={filters.swimmingPoolProvision} onChange={(val) => handleRadioChange('swimmingPoolProvision', val)} />
        <YesNoRadioGroup label="Garden / Landscaped Area" name="gardenLandscaped" value={filters.gardenLandscaped} onChange={(val) => handleRadioChange('gardenLandscaped', val)} />
        <YesNoRadioGroup label="Clubhouse / Recreation Area" name="clubhouseRecreation" value={filters.clubhouseRecreation} onChange={(val) => handleRadioChange('clubhouseRecreation', val)} />
        <YesNoRadioGroup label="Restaurant / Dining Space Provision" name="restaurantDiningProvision" value={filters.restaurantDiningProvision} onChange={(val) => handleRadioChange('restaurantDiningProvision', val)} />
        <YesNoRadioGroup label="Outdoor Activity Area" name="outdoorActivityArea" value={filters.outdoorActivityArea} onChange={(val) => handleRadioChange('outdoorActivityArea', val)} />
      </div>
    </div>
  );
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <YesNoRadioGroup label="Ready to Register" name="readyToRegister" value={filters.readyToRegister} onChange={(val) => handleRadioChange('readyToRegister', val)} />
            <YesNoRadioGroup label="Immediate Possession" name="immediatePossession" value={filters.immediatePossession} onChange={(val) => handleRadioChange('immediatePossession', val)} />
            <YesNoRadioGroup label="Under Development Resort Layout Acceptable" name="underDevelopmentLayout" value={filters.underDevelopmentLayout} onChange={(val) => handleRadioChange('underDevelopmentLayout', val)} />
          </div>
        </div>
      );
    }
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <Umbrella className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-lg">Filter Hotel / Resort Land</h3>
            </div>
            {onClose && (
...
export default HotelResortLandFilter;