...
const SEZLandFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    listingType: [],
    // Location Details
    city: '', taluk: '', locality: '', sezName: '', landmark: '', pincode: '',
    industrialZoneType: [], mainRoadAccess: '', highwayConnectivity: '', nearbyTransport: '',
    // Price/Rent/Lease/Sell Details
    minPrice: '', maxPrice: '', priceNegotiable: '', loanRequired: '',
    minRent: '', maxRent: '', minAnnualRent: '', maxAnnualRent: '', securityDeposit: '', advanceAmount: '', rentNegotiable: '',
    minLeaseAmount: '', maxLeaseAmount: '', leaseDuration: '', leaseRenewable: '', leaseNegotiable: '',
    minSellPrice: '', maxSellPrice: '', sellPriceNegotiable: '', maintenanceCharges: '', propertyTax: '',
    pricePerUnit: '', preferredPricePerUnit: '',
    registrationChargesIncluded: '', camCharges: '', propertyTaxResponsibility: '',
    // Land Details
    landArea: '', landAreaUnit: 'acres', usableLandArea: '', developableArea: '', expandableLand: '',
    openYardArea: '', plotLength: '', plotWidth: '', facing: '', cornerPlot: '', 
    roadWidth: '', boundaryWall: '', levelledLand: '', waterLoggingFree: '', landShape: '', landLevel: '',
    ...
    sezApproval: '', dtcpApproved: '', cmdaApproved: '', industrialApproval: '',
    ...
    exportLicenseEligible: '', rentalAgreementAvailable: '', leaseAgreementAvailable: '',
    // Availability
    readyToRegister: '', immediatePossession: '', vacantLandAvailable: '', preDevelopedSiteAvailable: '',
    readyToOccupy: '', availableFrom: '',
    // Nearby Access
    nearbyAccess: [],
    // Business Benefits
    businessBenefits: [],
  ...
  const industrialZoneTypeOptions = [
    { value: 'IT / ITES SEZ', label: 'IT / ITES SEZ' },
    { value: 'Manufacturing SEZ', label: 'Manufacturing SEZ' },
    { value: 'Multi-Product SEZ', label: 'Multi-Product SEZ' },
    { value: 'Pharma SEZ', label: 'Pharma SEZ' },
    { value: 'Textile SEZ', label: 'Textile SEZ' },
    { value: 'Engineering SEZ', label: 'Engineering SEZ' },
    { value: 'Electronics SEZ', label: 'Electronics SEZ' },
    { value: 'Food Processing SEZ', label: 'Food Processing SEZ' }
  ];
  ...
  const sezSuitabilityOptions = [
    'IT / ITES Park', 'Export-Oriented Manufacturing', 'Electronics Manufacturing', 'Textile & Apparel Industry',
    'Automobile Industry', 'Pharmaceutical Industry', 'Food Processing Unit', 'Logistics & Warehouse Setup',
    'Engineering Industry', 'Multi-Product SEZ', 'Sector-Specific SEZ', 'Export-Oriented Unit (EOU)',
    'Research & Development Center', 'Data Center Development', 'Warehousing & Logistics'
  ];
  ...
  const infrastructureUtilitiesOptions = [
    ...
    'Dedicated Transformer', 'Transformer Nearby', 'Water Connection Available', 'Borewell Facility',
    'Drainage Facility', 'Sewage Connection', 'Gas Pipeline Available', 'Internet / Fiber Connectivity',
    'Heavy Vehicle Access', 'Container Truck Access', 'Internal SEZ Roads Available', 'Internal Roads Available',
    'Street Lighting Available'
  ];

  const transportationAccessOptions = [
    'National Highway Access', 'State Highway Access', 'Container Vehicle Access', 'Truck Access',
    'Railway Freight Terminal Nearby', 'Inland Container Depot (ICD) Nearby', 'Airport Cargo Terminal Nearby',
    'Seaport / Harbor Nearby'
  ];

  const amenitiesOptions = [
    'Gated SEZ Campus', 'Security / Watchman', 'Security Services', 'CCTV Surveillance', 'Administrative Office',
    'Street Lights', 'Parking Space Available', 'Truck Parking Area', 'Fire Safety System', 'Office Space Available',
    'Worker Accommodation', 'Employee Transport Facility', 'Staff Accommodation', 'Food Court / Cafeteria',
    'Medical Facility', 'Common Utilities Available', 'Customs Clearance Facility Nearby'
  ];

  const nearbyOptions = [
    'National Highway', 'State Highway', 'Industrial Corridor', 'Manufacturing Cluster',
    'Inland Container Depot (ICD)', 'Container Freight Station (CFS)', 'Airport Cargo Hub',
    'Seaport / Harbor', 'Logistics Park', 'Railway Freight Station', 'Export Hub Nearby'
  ];

  const businessBenefitsOptions = [
    'Export Incentive Eligible', 'Tax Benefit Zone', 'Customs Clearance Support',
    'Single Window Clearance Support', 'Government Incentive Zone', 'Plug-and-Play Infrastructure'
  ];

  const tenantPreferencesOptions = [
    'Manufacturing Company', 'Export-Oriented Unit', 'Logistics Company', 'IT / ITES Company',
    'Pharmaceutical Company', 'Electronics Manufacturer', 'Food Processing Company'
  ];
  ...
  const mainSections = [
    ...
    { id: 'sez', label: '🌍 SEZ', icon: <Globe className="w-3.5 h-3.5" /> },
    ...
    { id: 'benefits', label: '💰 Benefits', icon: <Award className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'SEZ Land (Special Economic Zone)' };
...
const renderBasicSection = () => (
  <div className="space-y-3">
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        <div>
          ...
          <input type="text" value="SEZ Land (Special Economic Zone)" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
        </div>
        <div>
        ...
        <div className="md:col-span-2">
          ...
          <div className="flex flex-wrap gap-3">
            {['Owner', 'Agent', 'Developer', 'Industrial Authority', 'SEZ Authority'].map(type => (
        ...
        <div className="md:col-span-2">
          <label className="text-xs text-teal-800 font-semibold block mb-1.5">Industrial Zone Type</label>
          <div className="flex flex-wrap gap-3">
            {industrialZoneTypeOptions.map(option => (
              <label key={option.value} className="flex items-center gap-1.5 cursor-pointer group">
                <input
                  type="checkbox"
                  checked={filters.industrialZoneType.includes(option.value)}
                  onChange={() => handleArrayToggle('industrialZoneType', option.value)}
                  className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
                />
                <span className="text-xs text-gray-700 group-hover:text-teal-500">{option.label}</span>
              </label>
    ...
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <input type="text" placeholder="SEZ Name / Industrial Corridor" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.sezName} onChange={(e) => handleInputChange('sezName', e.target.value)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <input type="number" placeholder="Rent Per Acre / Cent / Sq.ft (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.pricePerUnit} onChange={(e) => handleInputChange('pricePerUnit', e.target.value)} />
        ...
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <input type="number" placeholder="Preferred Price Per Acre / Cent / Sq.ft (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.preferredPricePerUnit} onChange={(e) => handleInputChange('preferredPricePerUnit', e.target.value)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <input type="number" placeholder="Lease Rent Per Acre / Cent / Sq.ft (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.pricePerUnit} onChange={(e) => handleInputChange('pricePerUnit', e.target.value)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <input type="number" placeholder="Security Deposit (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.securityDeposit} onChange={(e) => handleInputChange('securityDeposit', e.target.value)} />
              <input type="number" placeholder="Advance Amount (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.advanceAmount} onChange={(e) => handleInputChange('advanceAmount', e.target.value)} />
    ...
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <input type="text" placeholder="Developable Area" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.developableArea} onChange={(e) => handleInputChange('developableArea', e.target.value)} />
          ...
          <YesNoRadioGroup label="Expandable Land Available" name="expandableLand" value={filters.expandableLand} onChange={(val) => handleRadioChange('expandableLand', val)} />
  ...
  const renderSEZSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Globe className="w-3.5 h-3.5" /> SEZ Suitability</h3>
      <div className="grid grid-cols-2 gap-1.5">
        {sezSuitabilityOptions.map(option => (
          <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
            <input
              type="checkbox"
              checked={filters.sezSuitability.includes(option)}
              onChange={() => handleArrayToggle('sezSuitability', option)}
              className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
            />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">{option}</span>
          </label>
  ...
  const renderTransportSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Truck className="w-3.5 h-3.5" /> Transportation Access</h3>
      <div className="grid grid-cols-2 gap-1.5">
        {transportationAccessOptions.map(option => (
          <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
            <input
              type="checkbox"
              checked={filters.transportationAccess.includes(option)}
              onChange={() => handleArrayToggle('transportationAccess', option)}
              className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
            />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">{option}</span>
          </label>
  ...
  const renderLegalSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        <YesNoRadioGroup label="SEZ Approval Available" name="sezApproval" value={filters.sezApproval} onChange={(val) => handleRadioChange('sezApproval', val)} />
        ...
        <YesNoRadioGroup label="Export License Eligible" name="exportLicenseEligible" value={filters.exportLicenseEligible} onChange={(val) => handleRadioChange('exportLicenseEligible', val)} />
        {(currentTab === 'Rent') && (
    ...
    if (currentTab === 'Rent' || currentTab === 'Lease') {
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <YesNoRadioGroup label="Immediate Occupancy" name="readyToOccupy" value={filters.readyToOccupy} onChange={(val) => handleRadioChange('readyToOccupy', val)} />
            <YesNoRadioGroup label="Ready for Industrial Setup" name="readyToOccupy" value={filters.readyToOccupy} onChange={(val) => handleRadioChange('readyToOccupy', val)} />
            ...
            <YesNoRadioGroup label="Pre-Developed Industrial Plot Available" name="preDevelopedSiteAvailable" value={filters.preDevelopedSiteAvailable} onChange={(val) => handleRadioChange('preDevelopedSiteAvailable', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Vacant SEZ Land Available" name="vacantLandAvailable" value={filters.vacantLandAvailable} onChange={(val) => handleRadioChange('vacantLandAvailable', val)} />
            <YesNoRadioGroup label="Pre-Developed Industrial Site Available" name="preDevelopedSiteAvailable" value={filters.preDevelopedSiteAvailable} onChange={(val) => handleRadioChange('preDevelopedSiteAvailable', val)} />
          </div>
        </div>
      );
    } else {
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Pre-Developed Industrial Plot Preferred" name="preDevelopedSiteAvailable" value={filters.preDevelopedSiteAvailable} onChange={(val) => handleRadioChange('preDevelopedSiteAvailable', val)} />
          </div>
        </div>
      );
    }
  ...
  const renderBenefitsSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Award className="w-3.5 h-3.5" /> Business Benefits</h3>
      <div className="grid grid-cols-2 gap-1.5">
        {businessBenefitsOptions.map(option => (
          <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
            <input
              type="checkbox"
              checked={filters.businessBenefits.includes(option)}
              onChange={() => handleArrayToggle('businessBenefits', option)}
              className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
            />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">{option}</span>
          </label>
  ...
  const renderPreferencesSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Users className="w-3.5 h-3.5" /> Tenant / Buyer Preferences</h3>
      <div className="grid grid-cols-2 gap-1.5">
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <Globe className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-sm">Filter SEZ Land</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'sez' && renderSEZSection()}
            {activeMainSection === 'infrastructure' && renderInfrastructureSection()}
            {activeMainSection === 'transport' && renderTransportSection()}
            ...
            {activeMainSection === 'benefits' && renderBenefitsSection()}
