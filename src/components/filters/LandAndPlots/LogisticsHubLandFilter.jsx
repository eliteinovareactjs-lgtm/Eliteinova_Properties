...
import { 
  ...
  Square, Home, Bus, CheckCircle, TrendingUp, Clock, Truck,
  Warehouse, Box, Package, Anchor, Train, Plane, Ship,
  Battery, Droplets, Wifi, HardDrive, ShieldCheck,
  Trees, Fence, Car, Sparkles, Sun, Award, Landmark, FileCheck,
  Settings, Navigation, Compass, Globe, TrendingUp as Trending,
  DollarSign as DollarSignIcon, Smartphone, Users
} from 'lucide-react';
...
const LogisticsHubLandFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    minRent: '', maxRent: '', securityDeposit: '', advanceAmount: '', rentNegotiable: '',
    ...
    // Land Details
    landArea: '', landAreaUnit: 'acres', openYardArea: '', truckParkingCapacity: '',
    ...
    dtcpApproved: '', cmdaApproved: '', logisticsApproval: '',
    ...
    conversionApproval: '', rentalAgreementAvailable: '', leaseAgreementAvailable: '',
    // Availability
    readyToOccupy: '', immediatePossession: '', vacantLandAvailable: '', operationalHubAvailable: '',
    readyToRegister: '',
    // Nearby Access
    nearbyAccess: [],
  ...
  const logisticsSuitabilityOptions = [
    'Warehouse Setup', 'Distribution Center', 'Cold Storage Facility', 'E-Commerce Fulfillment Center',
    'Container Yard', 'Truck Terminal', 'Freight Forwarding Hub', 'Multi-Modal Logistics Park',
    'Packaging & Sorting Center', 'Bulk Storage Facility', 'Inland Container Depot (ICD)',
    'Transport Hub', 'Industrial Storage Facility'
  ];
  ...
  const infrastructureUtilitiesOptions = [
    ...
    'Transformer Nearby', 'Borewell Facility', 'Water Connection Available', 'Drainage Facility',
    'Sewage Connection', 'Internet / Fiber Connectivity', 'Heavy Vehicle Access',
    'Container Truck Access', 'Dock Levelers Available', 'Weighbridge Nearby'
  ];

  const amenitiesOptions = [
    'Gated Logistics Park', 'Security / Watchman', 'CCTV Surveillance', 'Internal Roads',
    'Street Lights', 'Parking Space', 'Fire Safety System', 'Driver Rest Area',
    'Worker Accommodation', 'Office Space Available', 'Common Utilities Available',
    'Security Cabin', 'Weighbridge Facility'
  ];

 
  const nearbyOptions = [
    'National Highway', 'State Highway', 'Industrial Park', 'Manufacturing Cluster',
    'Container Freight Station (CFS)', 'Railway Freight Station', 'Airport Cargo Hub',
    'Seaport / Harbor', 'Truck Terminal', 'Fuel Station'
  ];
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Logistics Hub Land' };
    ...
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Logistics Hub Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <input type="text" placeholder="Industrial Zone / Logistics Park / SEZ / Freight Corridor" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.industrialZone} onChange={(e) => handleInputChange('industrialZone', e.target.value)} />
        ...
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <input type="number" placeholder="Price Per Acre / Cent / Sq.ft (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.pricePerUnit} onChange={(e) => handleInputChange('pricePerUnit', e.target.value)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <input type="number" placeholder="Security Deposit / Advance Amount (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.refundableDeposit} onChange={(e) => handleInputChange('refundableDeposit', e.target.value)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
    ...
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <CustomSelect label="Land Shape" options={landShapeOptions} value={filters.plotShape} onChange={(val) => handleInputChange('plotShape', val)} placeholder="Select Shape" />
          <CustomSelect label="Land Level" options={landLevelOptions} value={filters.levelledLand} onChange={(val) => handleInputChange('levelledLand', val)} placeholder="Select Level" />
          <CustomSelect label="Soil Type" options={soilTypeOptions} value={filters.waterLoggingFree} onChange={(val) => handleInputChange('waterLoggingFree', val)} placeholder="Select Soil Type" />
  ...
  const renderLogisticsSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Truck className="w-3.5 h-3.5" /> Logistics Suitability</h3>
      <div className="grid grid-cols-2 gap-1.5">
        {logisticsSuitabilityOptions.map(option => (
          <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
            <input
              type="checkbox"
              checked={filters.logisticsSuitability.includes(option)}
              onChange={() => handleArrayToggle('logisticsSuitability', option)}
              className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
            />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">{option}</span>
          </label>
  ...
  const renderLegalSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <YesNoRadioGroup label="Industrial / Logistics Approval Available" name="logisticsApproval" value={filters.logisticsApproval} onChange={(val) => handleRadioChange('logisticsApproval', val)} />
        ...
        <YesNoRadioGroup label="Conversion Approval Available (Industrial Use)" name="conversionApproval" value={filters.conversionApproval} onChange={(val) => handleRadioChange('conversionApproval', val)} />
        {(currentTab === 'Rent') && (
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Vacant Logistics Land Available" name="vacantLandAvailable" value={filters.vacantLandAvailable} onChange={(val) => handleRadioChange('vacantLandAvailable', val)} />
            <YesNoRadioGroup label="Operational Logistics Hub Available" name="operationalHubAvailable" value={filters.operationalHubAvailable} onChange={(val) => handleRadioChange('operationalHubAvailable', val)} />
          </div>
        </div>
      );
    } else if (currentTab === 'Sell') {
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Operational Logistics Facility Available" name="operationalHubAvailable" value={filters.operationalHubAvailable} onChange={(val) => handleRadioChange('operationalHubAvailable', val)} />
          </div>
        </div>
      );
    } else {
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <YesNoRadioGroup label="Ready to Register" name="readyToRegister" value={filters.readyToRegister} onChange={(val) => handleRadioChange('readyToRegister', val)} />
            <YesNoRadioGroup label="Immediate Possession" name="immediatePossession" value={filters.immediatePossession} onChange={(val) => handleRadioChange('immediatePossession', val)} />
            <YesNoRadioGroup label="Vacant Logistics Land Preferred" name="vacantLandAvailable" value={filters.vacantLandAvailable} onChange={(val) => handleRadioChange('vacantLandAvailable', val)} />
            <YesNoRadioGroup label="Operational Logistics Hub Preferred" name="operationalHubAvailable" value={filters.operationalHubAvailable} onChange={(val) => handleRadioChange('operationalHubAvailable', val)} />
          </div>
        </div>
      );
    }
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <Anchor className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-sm">Filter Logistics Hub Land</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'logistics' && renderLogisticsSection()}
...
export default LogisticsHubLandFilter;