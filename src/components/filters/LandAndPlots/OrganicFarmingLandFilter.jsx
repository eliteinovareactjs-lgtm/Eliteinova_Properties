...
import {
  ...
  Warehouse, PawPrint, Flower2, Recycle
} from 'lucide-react';
...
const OrganicLandAreaUnitSelect = ({ value, onChange }) => {
  const [isOpen, setIsOpen] = useState(false);
  const selectRef = useRef(null);
  const units = [
...
const OrganicFarmingLandFilter = ({ activeTab = 'Rent', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    // Land Details
    landArea: '', landAreaUnit: 'acres', soilType: '', landType: '', certifiedOrganicLand: '', yearsOfOrganicCultivation: '', waterSource: '', facing: '', cornerLand: '', roadWidth: '', boundaryAvailable: '', landShape: '',
    minRoadWidthRequired: '', landShapePreference: '',
    // Organic Farming Suitability
    organicFarmingSuitability: [],
    // Infrastructure & Utilities
    infrastructureUtilities: [],
    // Amenities
    amenities: [],
    // Approval & Legal Details
    organicCertificationAvailable: '', pattaAvailable: '', chittaAdangalAvailable: '', encumbranceFree: '', agriculturalApprovalAvailable: '', loanEligible: '', landSurveyCompleted: '', titleDeedVerified: '',
  ...
  const soilTypeOptions = [
    { value: 'Organic Soil', label: 'Organic Soil' },
  ...
  const organicFarmingSuitabilityOptions = [
    'Vegetable Farming', 'Fruit Orchard', 'Herbal Farming', 'Millet Cultivation',
    'Coconut / Banana Farm', 'Greenhouse Farming', 'Dairy Farming', 'Poultry Farming',
    'Natural Farming', 'Permaculture Farming'
  ];

  const infrastructureUtilitiesOptions = [
    'Water Connection Available', 'Borewell Facility', 'Electricity Connection Available',
    'Solar Power Facility', 'Irrigation Facility', 'Drip Irrigation Setup',
    'Compost Unit Available', 'Storage Shed Available', 'Tractor / Heavy Vehicle Access'
  ];

  const amenitiesOptions = [
    'Farm House Available', 'Gated Farm Land', 'Security / Caretaker',
    'Rainwater Harvesting', 'Internal Farm Roads', 'Worker Accommodation'
  ];
  ...
  const nearbyOptions = [
    'Highway Access', 'Nearby Village / Town', 'Organic Market Access',
    'Warehouse / Cold Storage Facility', 'Transport Access'
  ];
  ...
  const mainSections = [
    ...
    { id: 'organicSuitability', label: '🌱 Organic', icon: <Flower2 className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Organic Farming Land' };
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Organic Farming Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
    ...
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        <div className="grid grid-cols-4 gap-2 items-end">
          ...
          <OrganicLandAreaUnitSelect value={filters.landAreaUnit} onChange={(val) => handleInputChange('landAreaUnit', val)} />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <YesNoRadioGroup label="Certified Organic Land" name="certifiedOrganicLand" value={filters.certifiedOrganicLand} onChange={(val) => handleRadioChange('certifiedOrganicLand', val)} />
          <input type="number" placeholder="Years of Organic Cultivation" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.yearsOfOrganicCultivation} onChange={(e) => handleInputChange('yearsOfOrganicCultivation', e.target.value)} />
  ...
  const renderOrganicSuitabilitySection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Flower2 className="w-3.5 h-3.5" /> Organic Farming Suitability</h3>
      <CheckboxGroup options={organicFarmingSuitabilityOptions} values={filters.organicFarmingSuitability} onChange={(val) => handleInputChange('organicFarmingSuitability', val)} columns={2} />
    </div>
  );
  ...
  const renderLegalSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        <YesNoRadioGroup label="Organic Certification Available" name="organicCertificationAvailable" value={filters.organicCertificationAvailable} onChange={(val) => handleRadioChange('organicCertificationAvailable', val)} />
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <YesNoRadioGroup label="Under Development Organic Farm Layout" name="underDevelopmentLayout" value={filters.underDevelopmentLayout} onChange={(val) => handleRadioChange('underDevelopmentLayout', val)} />
          </div>
        </div>
      );
    } else { // Buy
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <YesNoRadioGroup label="Ready to Register" name="readyToRegister" value={filters.readyToRegister} onChange={(val) => handleRadioChange('readyToRegister', val)} />
            <YesNoRadioGroup label="Immediate Possession" name="immediatePossession" value={filters.immediatePossession} onChange={(val) => handleRadioChange('immediatePossession', val)} />
            <YesNoRadioGroup label="Under Development Organic Farm Layout Acceptable" name="underDevelopmentLayout" value={filters.underDevelopmentLayout} onChange={(val) => handleRadioChange('underDevelopmentLayout', val)} />
          </div>
        </div>
      );
    }
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
        <div className="sticky top-0 z-10 bg-white border-b border-teal-100">
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-white font-semibold text-sm">Filter Organic Farming Land</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'organicSuitability' && renderOrganicSuitabilitySection()}
