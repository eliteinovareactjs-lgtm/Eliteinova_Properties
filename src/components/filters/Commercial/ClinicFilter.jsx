...
// Activity Icon Component - MUST BE DEFINED BEFORE businessTypeOptions
...
const ClinicFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    numberOfConsultationRooms: '', numberOfFloors: '',
    icuOperationTheatre: '', receptionArea: '', waitingHallCapacity: '',
    parkingAvailability: '', propertyAge: '', ownershipType: '',
    // Medical Facility Features
    furnishingType: '',
    consultationRooms: false, operationTheatre: false, pharmacySpace: false,
    laboratorySpace: false, diagnosticCenter: false, patientWaitingArea: false,
    doctorCabin: false, washrooms: false, liftWheelchairAccess: false,
    centralizedAc: false,
    // Amenities
    ...
    internetBroadband: false, ambulanceAccess: false, visitorParking: false,
    ...
    leaseRenewalOption: '',
    // Legal Details
    medicalLicenseEligible: '', fireSafetyApproved: '', tradeLicenseAvailable: '',
  ...
  const businessTypeOptions = [
    { icon: <Stethoscope className="w-3 h-3" />, label: 'Clinic' },
    { icon: <Building2 className="w-3 h-3" />, label: 'Multi-speciality Hospital' },
    { icon: <Heart className="w-3 h-3" />, label: 'Dental Clinic' },
    { icon: <Syringe className="w-3 h-3" />, label: 'Diagnostic Center' },
    { icon: <Activity className="w-3 h-3" />, label: 'Physiotherapy Center' },
    { icon: <Eye className="w-3 h-3" />, label: 'Eye Clinic' },
    { icon: <Heart className="w-3 h-3" />, label: 'Veterinary Clinic' }
  ];

  const nearbyOptions = [
    'Residential Area', 'Pharmacy Nearby', 'Bus Stop / Metro',
    'Highway Access', 'Emergency Service Access', 'Medical Hub Nearby'
  ];
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Clinic / Hospital Space' };
      ...
      {expandedSections.basic && (
        <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          <div>
            ...
            <input type="text" value="Clinic / Hospital Space" disabled className="w-full px-2 py-1 rounded-lg border-2 border-teal-300 bg-gray-50 text-sm text-gray-600" />
          </div>
          <div>
    ...
    <div className="mb-3">
      ...
      {expandedSections.property && (
        <div className="mt-2 space-y-3 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-1">Number of Consultation Rooms</label>
              <input type="number" placeholder="Enter number" className="w-full px-2 py-1.5 rounded-lg border-2 border-teal-300 bg-white text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500" value={filters.numberOfConsultationRooms} onChange={(e) => handleInputChange('numberOfConsultationRooms', e.target.value)} />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-2">ICU / Operation Theatre Availability</label>
              <div className="flex gap-3">
                <label className="flex items-center gap-1.5 cursor-pointer group">
                  <input type="radio" name="icuOperationTheatre" value="Yes" checked={filters.icuOperationTheatre === 'Yes'} onChange={(e) => handleRadioChange('icuOperationTheatre', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                  <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                </label>
                <label className="flex items-center gap-1.5 cursor-pointer group">
                  <input type="radio" name="icuOperationTheatre" value="No" checked={filters.icuOperationTheatre === 'No'} onChange={(e) => handleRadioChange('icuOperationTheatre', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                  <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                </label>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-1">Waiting Hall Capacity</label>
              <input type="number" placeholder="Number of seats" className="w-full px-2 py-1.5 rounded-lg border-2 border-teal-300 bg-white text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500" value={filters.waitingHallCapacity} onChange={(e) => handleInputChange('waitingHallCapacity', e.target.value)} />
            </div>
            <div>
  ...
  const renderMedicalFacilityFeatures = () => (
    <div className="mb-3">
      <SectionHeader emoji="🩺" title="Medical Facility Features" section="medicalFeatures" />
      {expandedSections.medicalFeatures && (
        <div className="mt-2 space-y-3 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          <div>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.consultationRooms} onChange={(e) => handleCheckboxChange('consultationRooms', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Consultation Rooms</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.operationTheatre} onChange={(e) => handleCheckboxChange('operationTheatre', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Operation Theatre</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.pharmacySpace} onChange={(e) => handleCheckboxChange('pharmacySpace', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Pharmacy Space</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.laboratorySpace} onChange={(e) => handleCheckboxChange('laboratorySpace', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Laboratory Space</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.diagnosticCenter} onChange={(e) => handleCheckboxChange('diagnosticCenter', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Diagnostic Center Setup</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.patientWaitingArea} onChange={(e) => handleCheckboxChange('patientWaitingArea', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Patient Waiting Area</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.doctorCabin} onChange={(e) => handleCheckboxChange('doctorCabin', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Doctor Cabin</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.centralizedAc} onChange={(e) => handleCheckboxChange('centralizedAc', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
      ...
      {expandedSections.amenities && (
        <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          <div className="grid grid-cols-2 gap-1.5">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.ambulanceAccess} onChange={(e) => handleCheckboxChange('ambulanceAccess', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Ambulance className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Ambulance Access</span>
            </label>
      ...
      return (
        <div className="mb-3">
          ...
          {expandedSections.legal && (
            <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
              <div>
                <label className="text-sm text-teal-800 font-medium block mb-2">Medical License Eligible</label>
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="medicalLicenseEligible" value="Yes" checked={filters.medicalLicenseEligible === 'Yes'} onChange={(e) => handleRadioChange('medicalLicenseEligible', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="medicalLicenseEligible" value="No" checked={filters.medicalLicenseEligible === 'No'} onChange={(e) => handleRadioChange('medicalLicenseEligible', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
          ...
          {expandedSections.legal && (
            <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
              <div>
                ...
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="medicalLicenseEligibleLease" value="Yes" checked={filters.medicalLicenseEligible === 'Yes'} onChange={(e) => handleRadioChange('medicalLicenseEligible', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="medicalLicenseEligibleLease" value="No" checked={filters.medicalLicenseEligible === 'No'} onChange={(e) => handleRadioChange('medicalLicenseEligible', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
          ...
          {expandedSections.legal && (
            <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
              <div>
                ...
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="medicalLicenseEligibleRent" value="Yes" checked={filters.medicalLicenseEligible === 'Yes'} onChange={(e) => handleRadioChange('medicalLicenseEligible', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="medicalLicenseEligibleRent" value="No" checked={filters.medicalLicenseEligible === 'No'} onChange={(e) => handleRadioChange('medicalLicenseEligible', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
        ...
        <div className="flex justify-between items-center px-4 py-2.5 bg-gradient-to-r from-teal-600 to-emerald-600">
          <div className="flex items-center gap-2">
            <div className="p-1 bg-white/20 rounded-lg">
              ...
              `}</style>
              <Stethoscope className="w-5 h-5 text-white slow-rotate" />
            </div>
            <h3 className="text-white font-semibold text-sm md:text-base">Filter Clinic / Hospital Space</h3>
          </div>
