...
import { 
  ...
  Radio, Building as BuildingIcon, ShoppingCart
} from 'lucide-react';
...
const CommercialComplexFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    numberOfUnits: '', frontageWidth: '', ceilingHeight: '', parkingCapacity: '',
    propertyAge: '', ownershipType: '',
    // Complex Features
    furnishingType: '',
    receptionArea: false, officeSpace: false, retailSpace: false,
    foodCourtArea: false, washrooms: false, pantry: false,
    airConditioning: false, signageSpace: false, loadingUnloadingAccess: false,
    // Amenities
    twentyFourSevenSecurity: false, cctvSurveillance: false, powerBackup: false,
    fireSafetySystem: false, liftEscalator: false, visitorParking: false,
    centralizedAc: false, internetBroadband: false, housekeepingServices: false,
    ...
    leaseRenewalOption: '',
    // Legal Details
    titleDeedVerified: '', commercialLicense: '', reraApproved: '', loanEligible: '',
    // Nearby Access
    nearbyAccess: [],
    // Contact Preference
  ...
  const businessTypeOptions = [
    { icon: <Store className="w-3 h-3" />, label: 'Retail Shops' },
    { icon: <Building className="w-3 h-3" />, label: 'Corporate Offices' },
    { icon: <Utensils className="w-3 h-3" />, label: 'Restaurants / Cafés' },
    { icon: <Stethoscope className="w-3 h-3" />, label: 'Clinics / Medical Centers' },
    { icon: <Laptop className="w-3 h-3" />, label: 'IT Companies' },
    { icon: <GraduationCap className="w-3 h-3" />, label: 'Training Institutes' },
    { icon: <Store className="w-3 h-3" />, label: 'Franchise Outlets' }
  ];

  const nearbyOptions = [
    'Metro / Bus Stop', 'Railway Station', 'Residential Area',
    'Commercial Hub', 'Highway Access', 'Bank / ATM Nearby'
  ];
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Commercial Complex' };
      ...
      {expandedSections.basic && (
        <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          <div>
            ...
            <input type="text" value="Commercial Complex" disabled className="w-full px-2 py-1 rounded-lg border-2 border-teal-300 bg-gray-50 text-sm text-gray-600" />
          </div>
          <div>
        ...
        <div className="mt-2 space-y-3 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="number" placeholder="Total floors in complex" className="w-full px-2 py-1.5 rounded-lg border-2 border-teal-300 bg-white text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500" value={filters.totalFloors} onChange={(e) => handleInputChange('totalFloors', e.target.value)} />
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-1">Number of Units / Shops</label>
              <input type="number" placeholder="Enter number" className="w-full px-2 py-1.5 rounded-lg border-2 border-teal-300 bg-white text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500" value={filters.numberOfUnits} onChange={(e) => handleInputChange('numberOfUnits', e.target.value)} />
            </div>
            <div>
  ...
  const renderComplexFeatures = () => (
    <div className="mb-3">
      <SectionHeader emoji="🏢" title="Complex Features" section="complexFeatures" />
      {expandedSections.complexFeatures && (
        <div className="mt-2 space-y-3 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          <div>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.retailSpace} onChange={(e) => handleCheckboxChange('retailSpace', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Retail Space</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.foodCourtArea} onChange={(e) => handleCheckboxChange('foodCourtArea', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Utensils className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Food Court Area</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
  ...
  return (
    <div className="bg-white rounded-2xl shadow-2xl border-2 border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '90vh' }}>
      <div className="sticky top-0 z-10 bg-white border-b-2 border-teal-100">
        <div className="flex justify-between items-center px-4 py-2.5 bg-gradient-to-r from-teal-600 to-emerald-600">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-white font-semibold text-sm md:text-base">Filter Commercial Complex</h3>
          </div>
      ...
      <div className="overflow-y-auto flex-1 p-3 custom-scroll" style={{ maxHeight: 'calc(90vh - 100px)' }}>
        ...
        {renderComplexFeatures()}
        {renderAmenities()}
        {renderBusinessSuitability()}
        {renderAvailability()}
        {(currentTab === 'Buy' || currentTab === 'Sell' || currentTab === 'Lease') && renderLegal()}
        {renderNearbyAccess()}
...
export default CommercialComplexFilter;