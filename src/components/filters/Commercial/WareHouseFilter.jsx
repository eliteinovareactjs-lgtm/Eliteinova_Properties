...
import { 
  ...
  Warehouse, Boxes, Scale, Lock, Globe, Wind, Camera,
  Battery, Droplets, Wrench, Gauge, DoorOpen, ParkingCircle,
  HardDrive, Thermometer, Briefcase
} from 'lucide-react';
...
const WareHouseFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    ceilingHeight: '', floorLoadCapacity: '', numberOfShutters: '', dockLeveler: '', parkingTruckCapacity: '',
    propertyAge: '', ownershipType: '',
    // Warehouse Features
    storageType: '',
    loadingUnloadingBay: false, mezzanineFloor: false, officeCabinSpace: false,
    staffRestArea: false, washrooms: false, pantry: false,
    ventilationSystem: false, fireSafetySystem: false, cctvSurveillance: false,
    securityCabin: false,
    // Infrastructure
    electricityLoadCapacity: '', waterConnection: false, drainageFacility: false,
    internetBroadband: false, generatorBackup: false, compoundWall: false,
    wideInternalRoads: false, truckTurningRadius: false,
    // Business Suitability
    businessTypes: [],
    // Legal Details
    titleDeedVerified: '', loanEligible: '', reraApproved: '',
    industrialLicense: '', leaseAgreementAvailable: '',
    // Availability
  ...
  const businessTypeOptions = [
    { icon: <Boxes className="w-3 h-3" />, label: 'Logistics Hub' },
    { icon: <Truck className="w-3 h-3" />, label: 'E-commerce Storage' },
    { icon: <ShoppingBag className="w-3 h-3" />, label: 'FMCG Distribution' },
    { icon: <Factory className="w-3 h-3" />, label: 'Manufacturing Storage' },
    { icon: <Thermometer className="w-3 h-3" />, label: 'Cold Chain Storage' },
    { icon: <Scale className="w-3 h-3" />, label: 'Wholesale Distribution' },
    { icon: <Briefcase className="w-3 h-3" />, label: 'Industrial Goods Storage' }
  ];

  const nearbyOptions = [
    'Highway Access', 'Railway Cargo Terminal', 'Airport Cargo Access',
    'Port Access', 'Industrial Zone', 'Market / Commercial Hub'
  ];
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Warehouse / Godown' };
      ...
      {expandedSections.basic && (
        <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          <div>
            ...
            <input type="text" value="Warehouse / Godown" disabled className="w-full px-2 py-1 rounded-lg border-2 border-teal-300 bg-gray-50 text-sm text-gray-600" />
          </div>
          <div>
      ...
      {expandedSections.location && (
        <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          ...
          <div>
            ...
            <input type="text" placeholder="Metro, Bus, Highway, Port" className="w-full px-2 py-1.5 rounded-lg border-2 border-teal-300 bg-white text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500" value={filters.nearbyConnectivity} onChange={(e) => handleInputChange('nearbyConnectivity', e.target.value)} />
          </div>
        </div>
      )}
    </div>
    ...
    <div className="mb-3">
      ...
      {expandedSections.property && (
        <div className="mt-2 space-y-3 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-1">Number of Shutters / Entry Gates</label>
              <input type="number" placeholder="Enter number" className="w-full px-2 py-1.5 rounded-lg border-2 border-teal-300 bg-white text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500" value={filters.numberOfShutters} onChange={(e) => handleInputChange('numberOfShutters', e.target.value)} />
            </div>
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-2">Dock Leveler Available</label>
              <div className="flex gap-3">
                <label className="flex items-center gap-1.5 cursor-pointer group">
                  <input type="radio" name="dockLeveler" value="Yes" checked={filters.dockLeveler === 'Yes'} onChange={(e) => handleRadioChange('dockLeveler', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                  <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                </label>
                <label className="flex items-center gap-1.5 cursor-pointer group">
                  <input type="radio" name="dockLeveler" value="No" checked={filters.dockLeveler === 'No'} onChange={(e) => handleRadioChange('dockLeveler', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                  <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                </label>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-1">Parking / Truck Bay Capacity</label>
              <input type="number" placeholder="Number of trucks" className="w-full px-2 py-1.5 rounded-lg border-2 border-teal-300 bg-white text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500" value={filters.parkingTruckCapacity} onChange={(e) => handleInputChange('parkingTruckCapacity', e.target.value)} />
            </div>
  ...
  const renderWarehouseFeatures = () => (
    <div className="mb-3">
      <SectionHeader emoji="🏗️" title="Warehouse Features" section="warehouseFeatures" />
      {expandedSections.warehouseFeatures && (
        <div className="mt-2 space-y-3 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          <CustomSelect label="Storage Type" options={storageTypeOptions} value={filters.storageType} onChange={(val) => handleInputChange('storageType', val)} placeholder="Select Storage Type" />
          
          <div className="grid grid-cols-2 gap-1.5">
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.loadingUnloadingBay} onChange={(e) => handleCheckboxChange('loadingUnloadingBay', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Loading / Unloading Bay</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.mezzanineFloor} onChange={(e) => handleCheckboxChange('mezzanineFloor', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Mezzanine Floor</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.staffRestArea} onChange={(e) => handleCheckboxChange('staffRestArea', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Staff Rest Area</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
  ...
  const renderInfrastructure = () => (
    <div className="mb-3">
      ...
      {expandedSections.infrastructure && (
        <div className="mt-2 space-y-3 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          <div>
            <label className="text-sm text-teal-800 font-medium block mb-1">Electricity Load Capacity (kVA/HP)</label>
            <input type="text" placeholder="Enter load capacity" className="w-full px-2 py-1.5 rounded-lg border-2 border-teal-300 bg-white text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500" value={filters.electricityLoadCapacity} onChange={(e) => handleInputChange('electricityLoadCapacity', e.target.value)} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.drainageFacility} onChange={(e) => handleCheckboxChange('drainageFacility', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Drainage Facility</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.compoundWall} onChange={(e) => handleCheckboxChange('compoundWall', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Fence className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Compound Wall</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.wideInternalRoads} onChange={(e) => handleCheckboxChange('wideInternalRoads', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Road className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Wide Internal Roads</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group col-span-2">
              <input type="checkbox" checked={filters.truckTurningRadius} onChange={(e) => handleCheckboxChange('truckTurningRadius', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Truck className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Adequate Truck Turning Radius</span>
            </label>
          </div>
        </div>
      )}
        ...
        <div className="mb-3">
          ...
          {expandedSections.legal && (
            <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
              ...
              <div>
                <label className="text-sm text-teal-800 font-medium block mb-2">Industrial / Commercial License Eligible</label>
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
        ...
        <div className="mb-3">
          ...
          {expandedSections.legal && (
            <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
              ...
              <div>
                <label className="text-sm text-teal-800 font-medium block mb-2">Lease Agreement Available</label>
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="leaseAgreementAvailable" value="Yes" checked={filters.leaseAgreementAvailable === 'Yes'} onChange={(e) => handleRadioChange('leaseAgreementAvailable', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="leaseAgreementAvailable" value="No" checked={filters.leaseAgreementAvailable === 'No'} onChange={(e) => handleRadioChange('leaseAgreementAvailable', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="reraApprovedLease" value="Yes" checked={filters.reraApproved === 'Yes'} onChange={(e) => handleRadioChange('reraApproved', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="reraApprovedLease" value="No" checked={filters.reraApproved === 'No'} onChange={(e) => handleRadioChange('reraApproved', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
        ...
        <div className="flex justify-between items-center px-4 py-2.5 bg-gradient-to-r from-teal-600 to-emerald-600">
          <div className="flex items-center gap-2">
            <div className="p-1 bg-white/20 rounded-lg">
              ...
              `}</style>
              <Warehouse className="w-5 h-5 text-white slow-rotate" />
            </div>
            <h3 className="text-white font-semibold text-sm md:text-base">Filter Warehouse / Godown</h3>
          </div>
