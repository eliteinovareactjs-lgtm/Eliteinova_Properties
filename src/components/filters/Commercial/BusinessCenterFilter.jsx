...
import { 
  ...
  Globe, Headphones, BarChart, Target, Mail
} from 'lucide-react';
...
const BusinessCenterFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    meetingRooms: '', conferenceHall: '', receptionArea: '',
    privateOfficeSpace: '', sharedWorkspace: '',
    ownershipType: '',
    // Workspace Features
    furnishingType: '',
    highSpeedInternet: false, airConditioning: false, pantryCafeteria: false,
    printingScanning: false, phoneBooths: false, loungeArea: false,
    visitorWaitingArea: false, lockersStorage: false, smartOfficeSetup: false,
    // Amenities
  ...
  const businessTypeOptions = [
    { icon: <Building className="w-3 h-3" />, label: 'Corporate Offices' },
    { icon: <Rocket className="w-3 h-3" />, label: 'Startups' },
    { icon: <Laptop className="w-3 h-3" />, label: 'IT Companies' },
    { icon: <Briefcase className="w-3 h-3" />, label: 'Consulting Firms' },
    { icon: <Users className="w-3 h-3" />, label: 'Training Institutes' },
    { icon: <Headphones className="w-3 h-3" />, label: 'BPO / KPO' },
    { icon: <UsersIcon className="w-3 h-3" />, label: 'Remote Teams' }
  ];

  const nearbyOptions = [
    'Metro / Bus Stop', 'Railway Station', 'Airport Access',
    'IT Park / Business Hub', 'Restaurants / Hotels', 'Bank / ATM Nearby'
  ];
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Business Centre' };
      ...
      {expandedSections.basic && (
        <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          <div>
            ...
            <input type="text" value="Business Centre" disabled className="w-full px-2 py-1 rounded-lg border-2 border-teal-300 bg-gray-50 text-sm text-gray-600" />
          </div>
          <div>
    ...
    <div className="mb-3">
      ...
      {expandedSections.space && (
        <div className="mt-2 space-y-3 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-2">Meeting Rooms</label>
              <div className="flex gap-3">
                <label className="flex items-center gap-1.5 cursor-pointer group">
            ...
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-2">Conference Hall</label>
              <div className="flex gap-3">
                <label className="flex items-center gap-1.5 cursor-pointer group">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-1">Private Office Space</label>
              <input type="number" placeholder="Area for private offices" className="w-full px-2 py-1.5 rounded-lg border-2 border-teal-300 bg-white text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500" value={filters.privateOfficeSpace} onChange={(e) => handleInputChange('privateOfficeSpace', e.target.value)} />
            </div>
          </div>

          <div>
            <label className="text-sm text-teal-800 font-medium block mb-2">Shared Workspace Availability</label>
            <div className="flex gap-3">
              <label className="flex items-center gap-1.5 cursor-pointer group">
                <input type="radio" name="sharedWorkspace" value="Yes" checked={filters.sharedWorkspace === 'Yes'} onChange={(e) => handleRadioChange('sharedWorkspace', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
              </label>
              <label className="flex items-center gap-1.5 cursor-pointer group">
                <input type="radio" name="sharedWorkspace" value="No" checked={filters.sharedWorkspace === 'No'} onChange={(e) => handleRadioChange('sharedWorkspace', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
              </label>
            </div>
          </div>
      ...
      {expandedSections.workspaceFeatures && (
        <div className="mt-2 space-y-3 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.visitorWaitingArea} onChange={(e) => handleCheckboxChange('visitorWaitingArea', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Users className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Visitor Waiting Area</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer group col-span-2">
              <input type="checkbox" checked={filters.smartOfficeSetup} onChange={(e) => handleCheckboxChange('smartOfficeSetup', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Globe className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Smart Office Setup</span>
            </label>
          </div>
        </div>
      )}
        ...
        <div className="flex justify-between items-center px-4 py-2.5 bg-gradient-to-r from-teal-600 to-emerald-600">
          <div className="flex items-center gap-2">
            <div className="p-1 bg-white/20 rounded-lg">
              ...
              `}</style>
              <Building className="w-5 h-5 text-white slow-rotate" />
            </div>
            <h3 className="text-white font-semibold text-sm md:text-base">Filter Business Centre</h3>
          </div>
...
export default BusinessCenterFilter;