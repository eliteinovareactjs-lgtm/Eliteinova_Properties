...
const Activity = ({ className }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <path d="M22 12h-4l-3 9L9 3l-3 9H2" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);
...
const EducationalFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    numberOfClassrooms: '', numberOfFloors: '',
    auditoriumSeminarHall: '', laboratoryRooms: '', librarySpace: '',
    playgroundAvailability: '', parkingCapacity: '',
    propertyAge: '', ownershipType: '',
    // Institution Features
    furnishingType: '',
    smartClassrooms: false, computerLab: false, scienceLaboratory: false,
    staffRooms: false, administrativeOffice: false, studentWaitingArea: false,
    cafeteriaCanteen: false, washrooms: false, liftWheelchairAccess: false,
    // Amenities
    ...
    internetBroadband: false, schoolBusParking: false, visitorParking: false,
    ...
    leaseRenewalOption: '',
    // Legal Details
    educationalLicenseEligible: '', fireSafetyApproved: '', tradeLicenseAvailable: '',
    titleDeedVerified: '', commercialInstitutionalApproval: '', loanEligible: '',
    leaseAgreementReady: '',
    // Nearby Access
    nearbyAccess: [],
    // Contact Preference
  ...
  const businessTypeOptions = [
    { icon: <School className="w-3 h-3" />, label: 'School' },
    { icon: <Graduation className="w-3 h-3" />, label: 'College' },
    { icon: <BookOpen className="w-3 h-3" />, label: 'Coaching Center' },
    { icon: <Users className="w-3 h-3" />, label: 'Training Institute' },
    { icon: <BookOpen className="w-3 h-3" />, label: 'Tuition Center' },
    { icon: <School className="w-3 h-3" />, label: 'Montessori / Kindergarten' },
    { icon: <Laptop className="w-3 h-3" />, label: 'Computer Training Center' }
  ];

  const nearbyOptions = [
    'Residential Area', 'Bus Stop / Metro', 'Highway Access',
    'Student Residential Area', 'Commercial Hub', 'Bank / ATM Nearby'
  ];
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Educational Institution Property' };
      ...
      {expandedSections.basic && (
        <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          <div>
            ...
            <input type="text" value="Educational Institution Property" disabled className="w-full px-2 py-1 rounded-lg border-2 border-teal-300 bg-gray-50 text-sm text-gray-600" />
          </div>
          <div>
    ...
    <div className="mb-3">
      ...
      {expandedSections.property && (
        <div className="mt-2 space-y-3 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-1">Number of Classrooms</label>
              <input type="number" placeholder="Enter number" className="w-full px-2 py-1.5 rounded-lg border-2 border-teal-300 bg-white text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500" value={filters.numberOfClassrooms} onChange={(e) => handleInputChange('numberOfClassrooms', e.target.value)} />
            </div>
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-1">Number of Floors</label>
              <input type="number" placeholder="Total floors" className="w-full px-2 py-1.5 rounded-lg border-2 border-teal-300 bg-white text-sm focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500" value={filters.numberOfFloors} onChange={(e) => handleInputChange('numberOfFloors', e.target.value)} />
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-2">Auditorium / Seminar Hall</label>
              <div className="flex gap-3">
                <label className="flex items-center gap-1.5 cursor-pointer group">
                  <input type="radio" name="auditoriumSeminarHall" value="Yes" checked={filters.auditoriumSeminarHall === 'Yes'} onChange={(e) => handleRadioChange('auditoriumSeminarHall', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                  <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                </label>
                <label className="flex items-center gap-1.5 cursor-pointer group">
                  <input type="radio" name="auditoriumSeminarHall" value="No" checked={filters.auditoriumSeminarHall === 'No'} onChange={(e) => handleRadioChange('auditoriumSeminarHall', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                  <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                </label>
              </div>
            </div>
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-2">Laboratory Rooms</label>
              <div className="flex gap-3">
                <label className="flex items-center gap-1.5 cursor-pointer group">
                  <input type="radio" name="laboratoryRooms" value="Yes" checked={filters.laboratoryRooms === 'Yes'} onChange={(e) => handleRadioChange('laboratoryRooms', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                  <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                </label>
                <label className="flex items-center gap-1.5 cursor-pointer group">
                  <input type="radio" name="laboratoryRooms" value="No" checked={filters.laboratoryRooms === 'No'} onChange={(e) => handleRadioChange('laboratoryRooms', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                  <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                </label>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-2">Library Space</label>
              <div className="flex gap-3">
                <label className="flex items-center gap-1.5 cursor-pointer group">
                  <input type="radio" name="librarySpace" value="Yes" checked={filters.librarySpace === 'Yes'} onChange={(e) => handleRadioChange('librarySpace', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                  <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                </label>
                <label className="flex items-center gap-1.5 cursor-pointer group">
                  <input type="radio" name="librarySpace" value="No" checked={filters.librarySpace === 'No'} onChange={(e) => handleRadioChange('librarySpace', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                  <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                </label>
              </div>
            </div>
            <div>
              <label className="text-sm text-teal-800 font-medium block mb-2">Playground Availability</label>
              <div className="flex gap-3">
                <label className="flex items-center gap-1.5 cursor-pointer group">
                  <input type="radio" name="playgroundAvailability" value="Yes" checked={filters.playgroundAvailability === 'Yes'} onChange={(e) => handleRadioChange('playgroundAvailability', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                  <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                </label>
                <label className="flex items-center gap-1.5 cursor-pointer group">
                  <input type="radio" name="playgroundAvailability" value="No" checked={filters.playgroundAvailability === 'No'} onChange={(e) => handleRadioChange('playgroundAvailability', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                  <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                </label>
              </div>
            </div>
  ...
  const renderInstitutionFeatures = () => (
    <div className="mb-3">
      <SectionHeader emoji="📚" title="Institution Features" section="institutionFeatures" />
      {expandedSections.institutionFeatures && (
        <div className="mt-2 space-y-3 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          <div>
          ...
          <div className="grid grid-cols-2 gap-1.5">
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.smartClassrooms} onChange={(e) => handleCheckboxChange('smartClassrooms', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Smart Classrooms</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.computerLab} onChange={(e) => handleCheckboxChange('computerLab', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Laptop className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Computer Lab</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.scienceLaboratory} onChange={(e) => handleCheckboxChange('scienceLaboratory', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Microscope className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Science Laboratory</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.staffRooms} onChange={(e) => handleCheckboxChange('staffRooms', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Users className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Staff Rooms</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.administrativeOffice} onChange={(e) => handleCheckboxChange('administrativeOffice', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Building className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Administrative Office</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.studentWaitingArea} onChange={(e) => handleCheckboxChange('studentWaitingArea', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Users className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Student Waiting Area</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.cafeteriaCanteen} onChange={(e) => handleCheckboxChange('cafeteriaCanteen', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Coffee className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Cafeteria / Canteen</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer group col-span-2">
              <input type="checkbox" checked={filters.liftWheelchairAccess} onChange={(e) => handleCheckboxChange('liftWheelchairAccess', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Accessibility className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">Lift / Wheelchair Accessibility</span>
            </label>
          </div>
        </div>
      )}
      ...
      {expandedSections.amenities && (
        <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
          <div className="grid grid-cols-2 gap-1.5">
            ...
            <label className="flex items-center gap-1.5 cursor-pointer group">
              <input type="checkbox" checked={filters.schoolBusParking} onChange={(e) => handleCheckboxChange('schoolBusParking', e.target.checked)} className="w-3.5 h-3.5 rounded border-2 border-teal-300 bg-white checked:bg-teal-600 checked:border-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
              <Bus className="w-3 h-3 text-teal-600" />
              <span className="text-xs text-teal-700 group-hover:text-teal-600">School Bus Parking</span>
            </label>
            <label className="flex items-center gap-1.5 cursor-pointer group col-span-2">
      ...
      return (
        <div className="mb-3">
          ...
          {expandedSections.legal && (
            <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
              <div>
                <label className="text-sm text-teal-800 font-medium block mb-2">Educational License Eligible</label>
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="educationalLicenseEligible" value="Yes" checked={filters.educationalLicenseEligible === 'Yes'} onChange={(e) => handleRadioChange('educationalLicenseEligible', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="educationalLicenseEligible" value="No" checked={filters.educationalLicenseEligible === 'No'} onChange={(e) => handleRadioChange('educationalLicenseEligible', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="tradeLicenseAvailable" value="Yes" checked={filters.tradeLicenseAvailable === 'Yes'} onChange={(e) => handleRadioChange('tradeLicenseAvailable', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="tradeLicenseAvailable" value="No" checked={filters.tradeLicenseAvailable === 'No'} onChange={(e) => handleRadioChange('tradeLicenseAvailable', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
              ...
              <div>
                <label className="text-sm text-teal-800 font-medium block mb-2">Commercial / Institutional Approval</label>
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="commercialInstitutionalApproval" value="Yes" checked={filters.commercialInstitutionalApproval === 'Yes'} onChange={(e) => handleRadioChange('commercialInstitutionalApproval', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="commercialInstitutionalApproval" value="No" checked={filters.commercialInstitutionalApproval === 'No'} onChange={(e) => handleRadioChange('commercialInstitutionalApproval', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
          ...
          {expandedSections.legal && (
            <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
              <div>
                ...
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="educationalLicenseEligibleLease" value="Yes" checked={filters.educationalLicenseEligible === 'Yes'} onChange={(e) => handleRadioChange('educationalLicenseEligible', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="educationalLicenseEligibleLease" value="No" checked={filters.educationalLicenseEligible === 'No'} onChange={(e) => handleRadioChange('educationalLicenseEligible', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="tradeLicenseAvailableLease" value="Yes" checked={filters.tradeLicenseAvailable === 'Yes'} onChange={(e) => handleRadioChange('tradeLicenseAvailable', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="tradeLicenseAvailableLease" value="No" checked={filters.tradeLicenseAvailable === 'No'} onChange={(e) => handleRadioChange('tradeLicenseAvailable', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
              <div>
                ...
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="commercialInstitutionalApprovalLease" value="Yes" checked={filters.commercialInstitutionalApproval === 'Yes'} onChange={(e) => handleRadioChange('commercialInstitutionalApproval', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="commercialInstitutionalApprovalLease" value="No" checked={filters.commercialInstitutionalApproval === 'No'} onChange={(e) => handleRadioChange('commercialInstitutionalApproval', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
          ...
          {expandedSections.legal && (
            <div className="mt-2 space-y-2 bg-teal-50 rounded-lg border-2 border-teal-200 p-3">
              <div>
                ...
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="educationalLicenseEligibleRent" value="Yes" checked={filters.educationalLicenseEligible === 'Yes'} onChange={(e) => handleRadioChange('educationalLicenseEligible', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="educationalLicenseEligibleRent" value="No" checked={filters.educationalLicenseEligible === 'No'} onChange={(e) => handleRadioChange('educationalLicenseEligible', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="tradeLicenseAvailableRent" value="Yes" checked={filters.tradeLicenseAvailable === 'Yes'} onChange={(e) => handleRadioChange('tradeLicenseAvailable', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="tradeLicenseAvailableRent" value="No" checked={filters.tradeLicenseAvailable === 'No'} onChange={(e) => handleRadioChange('tradeLicenseAvailable', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
              <div>
                ...
                <div className="flex gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="commercialInstitutionalApprovalRent" value="Yes" checked={filters.commercialInstitutionalApproval === 'Yes'} onChange={(e) => handleRadioChange('commercialInstitutionalApproval', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">Yes</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="radio" name="commercialInstitutionalApprovalRent" value="No" checked={filters.commercialInstitutionalApproval === 'No'} onChange={(e) => handleRadioChange('commercialInstitutionalApproval', e.target.value)} className="w-3.5 h-3.5 border-2 border-teal-300 text-teal-600 focus:ring-0 focus:ring-offset-0 focus:outline-none accent-teal-600" />
                    <span className="text-xs text-teal-700 group-hover:text-teal-600">No</span>
                  </label>
                </div>
              </div>
        ...
        <div className="flex justify-between items-center px-4 py-2.5 bg-gradient-to-r from-teal-600 to-emerald-600">
          <div className="flex items-center gap-2">
            <div className="p-1 bg-white/20 rounded-lg">
              ...
              `}</style>
              <School className="w-5 h-5 text-white slow-rotate" />
            </div>
            <h3 className="text-white font-semibold text-sm md:text-base">Filter Educational Institution Property</h3>
          </div>
