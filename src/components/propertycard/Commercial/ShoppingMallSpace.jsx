...
import { ShoppingMallSpaceData } from "../../data/Commercial/ShoppingMallSpaceData";

const PAGE_NAME = "ShoppingMall Space";
...
const ShoppingMallSpace = () => {
  const [showLoginModal, setShowLoginModal] = useState(false);
  const [selectedProperty, setSelectedProperty] = useState(null);
  const [showContactInfo, setShowContactInfo] = useState(false);
  const [filteredProperties, setFilteredProperties] = useState(ShoppingMallSpaceData);
          ...
          <div className="flex flex-col gap-5">
            {filteredProperties.length > 0 ? (
              ...
              <div className="w-full bg-white rounded-2xl shadow-2xl border border-teal-100 p-8 text-center">
                ...
                <p className="text-xs text-slate-500">No ShoppingMallSpace apartments available at the moment.</p>
              </div>
            )}
          </div>
        </div>
...
export default ShoppingMallSpace;