...
import { IndependentHousePlotData } from "../../data/LandAndPlots/IndependentHousePlotData";

const PAGE_NAME = "Independent House Plot";
...
const PropertyCard = ({ property, onContactClick }) => {
  ...
  return (
    <>
      ...
      {/* MAP CONFIRM MODAL (Same as Duplex) */}
      {mapConfirm.show && (
        <div
        ...
        >
          ...
          >
            ...
            <div className="flex gap-3">
              <button onClick={() => setMapConfirm({ show: false, location: '' })} className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold py-2.5 rounded-xl text-sm transition-all duration-200">Cancel</button>
              <button onClick={() => { const query = encodeURIComponent(mapConfirm.location); window.open(`https://www.google.com/maps/search/?api=1&query=${query}`, '_blank'); setMapConfirm({ show: false, location: '' }); }} className="flex-1 bg-gradient-to-r from-[#00695C] to-[#26A69A] hover:opacity-90 text-white font-bold py-2.5 rounded-xl text-sm transition-all duration-200 shadow-lg">Open</button>
            </div>
          </div>
        </div>
      )}

      {/* AGENT DETAILS MODAL - Similar to Duplex but with Independent House specific fields */}
      {showAgentModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[200] flex items-start justify-center p-4 pt-16 md:pt-20 animate-fadeIn" onClick={() => setShowAgentModal(false)}>
          <div className="bg-white rounded-2xl max-w-[95%] sm:max-w-lg md:max-w-2xl w-full max-h-[85vh] overflow-y-auto shadow-2xl animate-scale-in mt-16 md:mt-20" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5">
              ...
              <div className="mb-4">
                <h4 className="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2"><span className="w-3 h-3 bg-teal-500 rounded-full"></span>Property Information</h4>
                <div className="bg-teal-50/50 rounded-xl p-4 space-y-2">
                  ...
                  <p className="text-sm flex items-center gap-1 cursor-pointer hover:text-[#00695C] hover:underline transition-colors duration-200 w-fit" onClick={() => openInMaps(property.location)}><strong>📍 Location:</strong> {property.location}</p>
                </div>
              </div>

              <div className="mb-4">
                <h4 className="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2"><span className="w-3 h-3 bg-teal-500 rounded-full"></span>Property Highlights</h4>
                <div className="bg-teal-50/50 rounded-xl p-4">
                  <div className="flex flex-wrap gap-2">
                    {property.highlights.split('|').map((h, i) => (<span key={i} className="bg-white px-3 py-1.5 rounded-lg text-xs font-medium text-slate-700 border border-teal-100 shadow-sm">✨ {h.trim()}</span>))}
                  </div>
                </div>
              </div>

              <div className="mb-4">
                <h4 className="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2"><span className="w-3 h-3 bg-teal-500 rounded-full"></span>Plot Details</h4>
                <div className="bg-teal-50/50 rounded-xl p-4">
                  <div className="grid grid-cols-2 gap-3">
                    <p className="text-sm"><strong>Facing:</strong> {property.facing || 'North'}</p>
                    <p className="text-sm"><strong>Road Width:</strong> {property.roadWidth || '30 ft'}</p>
                    <p className="text-sm"><strong>commercial Plot:</strong> {property.commercialPlot || 'Yes'}</p>
                    <p className="text-sm"><strong>Boundary Wall:</strong> {property.boundaryWall || 'Yes'}</p>
                  </div>
                </div>
              </div>

              <div className="mb-4">
                <h4 className="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2"><span className="w-3 h-3 bg-teal-500 rounded-full"></span>About {property.postedAs === 'Agent' ? 'Agent' : property.postedAs === 'Builder' ? 'Builder' : property.postedAs === 'Seller' ? 'Seller' : 'Owner'}</h4>
              ...
              <div className="mb-4">
                <h4 className="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2"><span className="w-3 h-3 bg-teal-500 rounded-full"></span>Contact Information</h4>
              ...
              <div className="mb-4">
                <h4 className="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2"><span className="w-3 h-3 bg-teal-500 rounded-full"></span>Photos Uploaded ({property.images.length})</h4>
                <div className="bg-teal-50/40 rounded-xl p-4">
                  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
                    {property.images.map((img, idx) => (
                      <div key={idx} className="relative aspect-square rounded-lg overflow-hidden cursor-pointer group shadow-md hover:shadow-xl transition-all hover:scale-105" onClick={() => { setGalleryActiveImg(idx); setShowFullGallery(true); setShowAgentModal(false); }}>
                        <img src={img} alt="thumb" className="w-full h-full object-cover" onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1564013799919-ab600027ffc6?w=200&h=200&fit=crop'; }} />
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {/* FULL GALLERY MODAL (Same as Duplex) */}
      {showFullGallery && (
        <div className="fixed inset-0 bg-black/95 z-[150] flex flex-col animate-fadeIn" onClick={() => setShowFullGallery(false)}>
          <div className="bg-gradient-to-r from-[#00695C] to-[#26A69A] p-3 md:p-4 flex justify-between items-center px-4 md:px-6">
            <div className="pr-2"><h3 className="text-white font-bold text-sm md:text-lg truncate max-w-[180px] md:max-w-none">{property.location.split(',')[0]}</h3><p className="text-white/80 text-[10px] md:text-xs mt-0.5">Click outside or press ESC to close</p></div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-3 md:p-6 pb-2" onClick={(e) => e.stopPropagation()}>
            <div className="relative w-full max-w-4xl">
              <div className="relative rounded-xl overflow-hidden shadow-2xl bg-black/50">
                <img src={property.images[galleryActiveImg]} alt="Gallery main" className="w-full h-auto max-h-[50vh] md:max-h-[60vh] object-contain" />
              </div>
...
const IndependentHousePlot = () => {
  const [showLoginModal, setShowLoginModal] = useState(false);
  const [selectedProperty, setSelectedProperty] = useState(null);
  const [showContactInfo, setShowContactInfo] = useState(false);
  const [filteredProperties, setFilteredProperties] = useState(IndependentHousePlotData);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 via-gray-200 to-teal-100/50 p-4 md:p-6">
          ...
          <div className="flex flex-col gap-5">
            {filteredProperties.length > 0 ? (
              ...
              <div className="w-full bg-white rounded-2xl shadow-2xl border border-teal-100 p-8 text-center">
                ...
                <p className="text-xs text-slate-500">No independent house plots available at the moment.</p>
              </div>
            )}
          </div>
        </div>
      ...
      <style>{`
        ...
        @keyframes tagJump { 0%, 100% { transform: translateY(0px) scale(1); box-shadow: 0 0 20px rgba(0,0,0,0.4), 0 0 10px rgba(0,105,92,0.8); } 50% { transform: translateY(-7px) scale(1.05); box-shadow: 0 0 30px rgba(0,0,0,0.6), 0 0 20px rgba(0,105,92,1), 0 5px 15px rgba(0,0,0,0.5); } }
...
export default IndependentHousePlot;