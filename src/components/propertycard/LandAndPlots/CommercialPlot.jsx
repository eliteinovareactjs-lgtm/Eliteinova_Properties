...
import { CommercialPlotData } from "../../data/LandAndPlots/CommercialPlotData";

const PAGE_NAME = "Commercial Plot";
...
// Helper: extract BHK from highlights (modified for commercial)
const extractPropertyDetails = (highlights) => {
  if (!highlights) return { type: '', details: '' };
  return { type: highlights.split('|')[0]?.trim() || '', details: highlights };
};
...
const PropertyCard = ({ property, onContactClick }) => {
  ...
  const { type: propertyType } = extractPropertyDetails(property.highlights);

  const getRoleTitle = () => {
    if (property.postedAs === 'Agent') return 'Commercial Real Estate Agent';
    if (property.postedAs === 'Builder') return 'Commercial Developer';
  ...
  return (
    <>
        ...
        <div className="p-4 md:p-5">
          <div className="flex flex-col lg:flex-row gap-5 items-stretch">
            ...
            <div className="flex-1 flex flex-col gap-2">
              ...
              <div className="flex flex-wrap justify-between items-start gap-2">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1">
                    ...
                    <span className="text-[#00695C] font-bold bg-teal-50 px-2 py-1 rounded-md text-xs md:text-sm shadow-sm">🏢 {property.builtUp}</span>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-1 shrink-0">
                  <div className="flex items-center gap-2">
                    <span className="font-black text-[#00695C] uppercase tracking-wide blink-text" style={{ fontSize: '13px', letterSpacing: '0.7px', whiteSpace: 'nowrap' }}>{PAGE_NAME}</span>
                  </div>
                  <div className="bg-gradient-to-r from-teal-600 to-emerald-600 text-white font-black tracking-wider uppercase flex items-center justify-center gap-1 whitespace-nowrap text-[10px] md:text-xs tag-animation" style={{ clipPath: 'polygon(0% 0%, 100% 0%, 92% 50%, 100% 100%, 0% 100%, 8% 50%)', padding: '2px 10px', minWidth: '40px' }}>
                    <span className="text-xs md:text-sm">{property.tag === 'BUY' && '💰'}{property.tag === 'SELL' && '🏷️'}{property.tag === 'RENT' && '🔑'}{property.tag === 'LEASE' && '📄'}</span>
              ...
              {/* LOCATION */}
              <div className="flex items-start gap-2 cursor-pointer group" onClick={() => openInMaps(property.location)} title="View on Google Maps">
                ...
                <p className="text-slate-800 font-bold text-sm md:text-base leading-tight group-hover:text-[#00695C] group-hover:underline transition-colors duration-200">{property.location}</p>
              </div>

              {/* HIGHLIGHTS */}
              <div>
                <p className="font-black text-[#004D40] uppercase tracking-wider mb-1.5 flex items-center gap-3 text-[10px] md:text-[11px]"><span className="w-5 h-px bg-[#004D40]"></span>Property Highlights</p>
              ...
              <div className="pt-2 border-t border-gray-100">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-3 flex-1 min-w-[180px]">
                    <div className="rounded-xl bg-gradient-to-br from-[#00695C] to-[#26A69A] flex items-center justify-center text-white font-black shadow-lg overflow-hidden shrink-0 w-10 h-10 md:w-12 md:h-12 text-base md:text-lg cursor-pointer hover:scale-110 hover:shadow-xl transition-all duration-200" onClick={() => openInMaps(property.location)}>
                    ...
                    <div className="flex flex-col min-w-0">
                      ...
                      <div className="flex items-baseline gap-4.5 flex-wrap">
                        ...
                        <button onClick={() => setShowAgentModal(true)} className="text-teal-500 hover:text-teal-700 underline flex items-center gap-0.7 transition-all duration-300 hover:translate-x-1 text-sm font-medium whitespace-nowrap">📖 View Details →</button>
                      </div>
                      <span className="text-teal-600 font-medium text-[9px] md:text-[10px] leading-tight">{getRoleTitle()}</span>
                    </div>
                  </div>
                  <button onClick={onContactClick} onMouseEnter={() => setIsContactHovered(true)} onMouseLeave={() => setIsContactHovered(false)} className="bg-gradient-to-r from-[#00695C] to-[#26A69A] text-white font-bold rounded-lg flex items-center gap-1 whitespace-nowrap transition-all duration-300 shrink-0 px-4 md:px-5 py-1.5 md:py-2 text-xs md:text-sm contact-button" style={{ transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', transform: isContactHovered ? 'translateY(-2px) scale(1.02)' : 'translateY(0) scale(1)', boxShadow: isContactHovered ? '0 12px 30px rgba(0,105,92,0.5), 0 0 0 3px rgba(38,166,154,0.3)' : '0 8px 20px rgba(0,105,92,0.3)' }}>
                    <span className="text-xs md:text-sm transition-transform duration-300" style={{ transform: isContactHovered ? 'scale(1.1) rotate(-5deg)' : 'scale(1) rotate(0)' }}>📞</span> Contact
                  </button>
                </div>
              </div>
            </div>
      ...
      {/* MODALS - Same as previous components (Map Confirm, Agent Details, Full Gallery) */}
      {/* Map Confirm Modal */}
      {mapConfirm.show && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[300] flex items-center justify-center p-4 animate-fadeIn" onClick={() => setMapConfirm({ show: false, location: '' })}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 animate-scale-in" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={() => setMapConfirm({ show: false, location: '' })} className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold py-2.5 rounded-xl text-sm">Cancel</button>
              <button onClick={() => { const query = encodeURIComponent(mapConfirm.location); window.open(`https://www.google.com/maps/search/?api=1&query=${query}`, '_blank'); setMapConfirm({ show: false, location: '' }); }} className="flex-1 bg-gradient-to-r from-[#00695C] to-[#26A69A] hover:opacity-90 text-white font-bold py-2.5 rounded-xl text-sm shadow-lg">Open</button>
            </div>
          </div>
        </div>
      )}

      {/* Agent Details Modal - Simplified for brevity, similar structure as previous */}
      {showAgentModal && (
          ...
          <div className="bg-white rounded-2xl max-w-[95%] sm:max-w-lg md:max-w-2xl w-full max-h-[85vh] overflow-y-auto shadow-2xl animate-scale-in mt-16 md:mt-20" onClick={(e) => e.stopPropagation()}>
            <div className="bg-gradient-to-r from-[#00695C] to-[#26A69A] p-5 rounded-t-2xl flex justify-between items-center sticky top-0 z-10">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center text-2xl">{property.tag === 'BUY' && '💰'}{property.tag === 'SELL' && '🏷️'}{property.tag === 'RENT' && '🔑'}{property.tag === 'LEASE' && '📄'}</div>
                <div><h3 className="text-white font-bold text-lg md:text-xl">Complete Details</h3><p className="text-white/80 text-xs md:text-sm">{property.postedAs || getRoleTitle()}</p></div>
              </div>
            ...
            <div className="p-5">
              <div className="flex items-center gap-4 pb-4 border-b border-teal-100 mb-4">
                <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-[#00695C] to-[#26A69A] flex items-center justify-center text-white font-black shadow-lg text-xl overflow-hidden">
                  {property.logo && !logoError ? <img src={property.logo} alt="logo" className="w-full h-full object-cover" /> : <span>{property.postedBy.charAt(0)}</span>}
                </div>
                <div><p className="text-[10px] text-[#00695C] font-bold uppercase tracking-wider">{getListedByText()}</p><p className="text-xl md:text-2xl font-black text-slate-800">{property.postedBy}</p><p className="text-xs md:text-sm text-teal-600 font-medium mt-0.5">{getRoleTitle()}</p></div>
              </div>
              <div className="mb-4"><h4 className="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2"><span className="w-3 h-3 bg-teal-500 rounded-full"></span>Property Information</h4><div className="bg-teal-50/50 rounded-xl p-4 space-y-2"><p className="text-sm"><strong>Property ID:</strong> {property.id}</p><p className="text-sm"><strong>Listed Price:</strong> {property.price}</p><p className="text-sm"><strong>{property.sqftPrice}</strong> • 📐 {property.totalSqft} • 🏢 {property.builtUp}</p><p className="text-sm flex items-center gap-1 cursor-pointer hover:text-[#00695C] hover:underline" onClick={() => openInMaps(property.location)}><strong>📍 Location:</strong> {property.location}</p></div></div>
              <div className="mb-4"><h4 className="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2"><span className="w-3 h-3 bg-teal-500 rounded-full"></span>About {property.postedAs === 'Agent' ? 'Agent' : property.postedAs === 'Builder' ? 'Builder' : property.postedAs === 'Seller' ? 'Seller' : 'Owner'}</h4><div className="bg-teal-50/50 rounded-xl p-4"><p className="text-sm text-gray-700 leading-relaxed">{property.agentDetails || "No additional details provided."}</p></div></div>
              <div className="mb-4"><h4 className="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2"><span className="w-3 h-3 bg-teal-500 rounded-full"></span>Contact Information</h4><div className="bg-gray-50 rounded-xl p-4 space-y-2"><p className="text-sm flex items-center gap-2 break-all"><span className="text-teal-600">📧</span><span>{property.contactEmail || property.postedBy.toLowerCase().replace(/\s/g, '') + '@example.com'}</span></p><p className="text-sm flex items-center gap-2"><span className="text-teal-600">📞</span><span>{property.contactPhone || '+91 98765 43210'}</span></p></div></div>
              <div className="flex gap-3 mt-5 pt-4 border-t border-teal-100">
                <button onClick={() => { setShowAgentModal(false); onContactClick(); }} className="flex-1 bg-gradient-to-r from-[#00695C] to-[#26A69A] text-white py-2.5 rounded-xl font-bold text-sm">📞 Contact Now</button>
                <button onClick={() => setShowAgentModal(false)} className="flex-1 bg-gray-100 text-gray-600 py-2.5 rounded-xl font-bold text-sm">Close</button>
              </div>
            </div>
          </div>
        </div>
...
const CommercialPlot = () => {
  const [showLoginModal, setShowLoginModal] = useState(false);
  const [selectedProperty, setSelectedProperty] = useState(null);
  const [showContactInfo, setShowContactInfo] = useState(false);
  const [filteredProperties, setFilteredProperties] = useState(CommercialPlotData);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 via-gray-200 to-teal-100/50 p-4 md:p-6">
          ...
          <div className="flex flex-col gap-5">
            {filteredProperties.length > 0 ? (
              ...
              <div className="w-full bg-white rounded-2xl shadow-2xl border border-teal-100 p-8 text-center">
                ...
                <p className="text-xs text-slate-500">No commercial plots available at the moment.</p>
              </div>
            )}
          </div>
        </div>
      ...
      {showLoginModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[200] flex items-center justify-center animate-fadeIn">
          <div className="bg-white rounded-2xl max-w-md w-full p-5 shadow-2xl">
            <div className="w-14 h-14 bg-gradient-to-br from-[#00695C] to-[#26A69A] rounded-full flex items-center justify-center mx-auto mb-3 shadow-lg"><span className="text-xl text-white">🔒</span></div>
            ...
            <button onClick={handleLogin} className="w-full bg-gradient-to-r from-[#00695C] to-[#26A69A] text-white py-2 rounded-lg font-bold text-sm shadow-lg">Continue to Login</button>
      ...
      <style>{`
        ...
        @keyframes tagJump { 0%, 100% { transform: translateY(0px) scale(1); } 50% { transform: translateY(-7px) scale(1.05); } }
        @keyframes contactPulse { 0%, 100% { box-shadow: 0 8px 20px rgba(0,105,92,0.3); } 50% { box-shadow: 0 8px 25px rgba(0,105,92,0.5), 0 0 0 3px rgba(38,166,154,0.2); } }
