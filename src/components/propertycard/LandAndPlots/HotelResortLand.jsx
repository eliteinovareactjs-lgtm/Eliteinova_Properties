// HotelResortLand.jsx (Property Card Component)
import React, { useState, useEffect } from 'react';
import { HotelResortLandData } from "../../data/LandAndPlots/HotelResortLandData";

const PAGE_NAME = "HotelResort";
...
// Helper: extract property type from highlights
const extractPropertyType = (highlights) => {
  if (!highlights) return '';
  const propertyTypes = ['Hotel Development', 'Resort Development', 'Beach Resort', 'Hill Station Resort', 'Heritage Hotel', 'Boutique Hotel', 'Luxury Resort', 'Eco Resort'];
  for (const type of propertyTypes) {
    if (highlights.includes(type)) return type;
  }
  return '';
};
...
const PropertyCard = ({ property, onContactClick }) => {
  ...
  const propertyType = extractPropertyType(property.highlights);
              ...
              >
                ...
                >
                  <img
                    ...
                    onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=600&h=450&fit=crop'; }}
                  />
                  ...
                  {property.images.map((img, idx) => (
                    ...
                    >
                      <img
                        ...
                        alt="thumb"
                        onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=100&h=100&fit=crop'; }}
                      />
                      {imageCount > 5 && idx === 3 && (
              ...
              <div className="flex flex-wrap justify-between items-start gap-2">
                <div className="flex-1">
                  <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0">
                    ...
                    {propertyType && <span className="font-bold text-[#00695C] text-base md:text-lg ml-1">({propertyType})</span>}
                  </div>
            ...
            <div className="p-5">
              ...
              <div className="mb-4">
                ...
                <div className="bg-teal-50/50 rounded-xl p-4 space-y-2">
                  ...
                  <p className="text-sm"><strong>Listed Price:</strong> {property.price} {propertyType && `(${propertyType})`}</p>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-3 md:p-6 pb-2" onClick={(e) => e.stopPropagation()}>
            <div className="relative w-full max-w-4xl">
              <div className="relative rounded-xl overflow-hidden shadow-2xl bg-black/50">
                <img src={property.images[galleryActiveImg]} alt="Gallery main" className="w-full h-auto max-h-[50vh] md:max-h-[60vh] object-contain" onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800&h=600&fit=crop'; }} />
              </div>
              ...
              <div className="flex gap-2 md:gap-3 overflow-x-auto pb-2 justify-center flex-wrap">
                {property.images.map((img, idx) => (
                  <div key={idx} className={`w-12 h-12 md:w-16 md:h-16 flex-shrink-0 rounded-lg overflow-hidden cursor-pointer transition-all ${galleryActiveImg === idx ? 'ring-2 ring-[#26A69A] shadow-xl scale-105' : 'opacity-70 hover:opacity-100 hover:scale-105 shadow-md'}`} onClick={() => handleGalleryThumbnailClick(idx)}>
                    <img src={img} alt="thumb" className="w-full h-full object-cover" onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=100&h=100&fit=crop'; }} />
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-3 md:p-8" onClick={(e) => e.stopPropagation()}>
            <div className="relative w-full max-w-4xl">
              <div className="relative overflow-hidden rounded-2xl shadow-2xl bg-black/50">
                <img src={property.images[activeImg]} alt="Gallery" className="w-full h-auto max-h-[50vh] md:max-h-[65vh] object-contain" onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=800&h=600&fit=crop'; }} />
              </div>
          ...
          <div className="flex flex-col gap-5">
            {filteredProperties.length > 0 ? (
              ...
              <div className="w-full bg-white rounded-2xl shadow-2xl border border-teal-100 p-8 text-center">
                ...
                <p className="text-xs text-slate-500">No Hotel/Resort Land properties available at the moment.</p>
              </div>
            )}
          </div>
        </div>
