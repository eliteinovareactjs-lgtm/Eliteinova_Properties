...
import { DuplexHousePlotData } from "../../data/LandAndPlots/DuplexHousePlotData";

const PAGE_NAME = "Duplex House Plot";
...
const DuplexHousePlot = () => {
  const [showLoginModal, setShowLoginModal] = useState(false);
  const [selectedProperty, setSelectedProperty] = useState(null);
  const [showContactInfo, setShowContactInfo] = useState(false);
  const [filteredProperties, setFilteredProperties] = useState(DuplexHousePlotData);
          ...
          <div className="flex flex-col gap-5">
            {filteredProperties.length > 0 ? (
              ...
              <div className="w-full bg-white rounded-2xl shadow-2xl border border-teal-100 p-8 text-center">
                ...
                <p className="text-xs text-slate-500">No duplex house plots available at the moment.</p>
              </div>
            )}
          </div>
        </div>
...
export default DuplexHousePlot;