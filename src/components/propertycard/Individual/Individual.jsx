...
import { IndividualData } from '../../data/Individual/IndividualData';

const PAGE_NAME = "Individual";
...
const PropertyCard = ({ property, onContactClick }) => {
  ...
  // ============================================
  // NOW do all computations AFTER all hooks
  // ============================================
  
  // Guard against missing property
  if (!property) {
    return null;
  }
  ...
  const nextImg = (e) => {
    ...
    setActiveImg((prev) => (prev + 1) % imageCount);
  };

  const prevImg = (e) => {
    e.stopPropagation();
    if (imageCount === 0) return;
    setActiveImg((prev) => (prev - 1 + imageCount) % imageCount);
  };

  const nextGalleryImg = (e) => {
    e.stopPropagation();
    if (imageCount === 0) return;
    setGalleryActiveImg((prev) => (prev + 1) % imageCount);
  };

  const prevGalleryImg = (e) => {
    e.stopPropagation();
    if (imageCount === 0) return;
    setGalleryActiveImg((prev) => (prev - 1 + imageCount) % imageCount);
  };
  ...
  const postedBy = property.postedBy || 'Unknown';
          ...
          <div className="flex flex-col lg:flex-row gap-5">
            ...
            <div className="w-full lg:w-[35%] xl:w-[32%]">
              <div className="flex flex-row bg-gray-100 rounded-xl overflow-hidden shadow-lg" style={{ height: '260px', minHeight: '260px', flexShrink: 0 }}>
                ...
                <div className="flex-1 h-full overflow-hidden relative cursor-pointer" onDoubleClick={(e) => handleImageDoubleClick(activeImg, e)}>
                  <img
                    src={images[activeImg] || 'https://images.unsplash.com/photo-1564013799919-ab600027ffc6?w=600&h=450&fit=crop'}
                    alt="Property"
            ...
            <div className="flex-1 flex flex-col gap-2">
              ...
              <div className="flex items-start gap-2">
                ...
                <p className="text-slate-800 font-bold text-sm md:text-base leading-tight">{location}</p>
              </div>
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-[180px]">
                    ...
                    <div className="flex flex-col min-w-0">
                      ...
                      <div className="flex items-baseline gap-4.5 flex-wrap">
                        <p className="font-black text-slate-800 text-sm md:text-base leading-snug">{postedBy}</p>
                        <button 
            ...
            <div className="p-5">
              ...
              <div className="flex items-center gap-4 pb-4 border-b border-teal-100 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xl md:text-2xl font-black text-slate-800">{postedBy}</p>
              ...
              <div className="mb-4">
                ...
                <div className="bg-gray-50 rounded-xl p-4 space-y-2">
                  <p className="text-sm flex items-center gap-2 break-all"><span className="text-teal-600">📧</span><span>{property.contactEmail || postedBy.toLowerCase().replace(/\s/g, '') + '@example.com'}</span></p>
                  ...
                  <p className="text-sm flex items-center gap-2"><span className="text-teal-600">📍</span><span>{location}</span></p>
                </div>
              </div>
              ...
              <div className="mb-4">
                <h4 className="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                  <span className="w-3 h-3 bg-teal-500 rounded-full"></span>
                  Photos Uploaded ({imageCount} {imageCount === 1 ? 'Photo' : 'Photos'})
                </h4>
      ...
      {showFullGallery && (
        ...
        >
          <div className="bg-gradient-to-r from-[#00695C] to-[#26A69A] p-3 md:p-4 flex justify-between items-center px-4 md:px-6">
            <div className="pr-2">
              <h3 className="text-white font-bold text-sm md:text-lg truncate max-w-[180px] md:max-w-none">{location.split(',')[0]}</h3>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-3 md:p-6 pb-2" onClick={(e) => e.stopPropagation()}>
            <div className="relative w-full max-w-4xl">
              <div className="relative rounded-xl overflow-hidden shadow-2xl bg-black/50">
                <img src={images[galleryActiveImg]} alt="Gallery main" className="w-full h-auto max-h-[50vh] md:max-h-[60vh] object-contain" onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1564013799919-ab600027ffc6?w=800&h=600&fit=crop'; }} />
              </div>
            ...
            <div className="mt-2 text-white/80 text-xs md:text-sm">{galleryActiveImg + 1} / {imageCount}</div>
      ...
      {showImageModal && (
        ...
        >
          <div className="bg-gradient-to-r from-[#00695C] to-[#26A69A] p-3 md:p-5 flex justify-between items-center px-4 md:px-8">
            <div>
              <h3 className="text-white font-bold text-sm md:text-xl truncate max-w-[150px] sm:max-w-[300px]">{location.split(',')[0]}</h3>
              <p className="text-white/80 text-xs md:text-sm mt-1">{imageCount} Photos</p>
            </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-3 md:p-8" onClick={(e) => e.stopPropagation()}>
            <div className="relative w-full max-w-4xl">
              <div className="relative overflow-hidden rounded-2xl shadow-2xl bg-black/50">
                <img src={images[activeImg]} alt="Gallery" className="w-full h-auto max-h-[50vh] md:max-h-[65vh] object-contain" onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1564013799919-ab600027ffc6?w=800&h=600&fit=crop'; }} />
              </div>
              ...
              <div className="absolute -bottom-8 md:-bottom-12 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-lg text-white text-[10px] md:text-sm px-2 md:px-4 py-1 rounded-full shadow-lg">{activeImg + 1} / {imageCount}</div>
            </div>
...
const Individual = () => {
  ...
  const [filteredProperties, setFilteredProperties] = useState(IndividualData || []);
          ...
          <div className="flex flex-col gap-5">
            {filteredProperties.length > 0 ? (
              ...
              <div className="w-full bg-white rounded-2xl shadow-2xl border border-teal-100 p-8 text-center">
                ...
                <p className="text-xs text-slate-500">No properties available at the moment.</p>
              </div>
            )}
          </div>
        </div>
