...
const nearbyPlacesList = ["Highway Access", "Industrial Zone", "Metro / Bus Stop", "Residential Area", "IT Park / Business Hub", "Airport Access"];
...
export default function SellAgentComForm({ isOpen, onClose }) {
  ...
  const [formData, setFormData] = useState({
    ...
    securityDeposit: "", priceNegotiable: "Fixed",
  ...
  const validateStep = (s) => {
    ...
    if (s === 2) {
      ...
      else if (!isAlphanumericWithSpaces(formData.area)) e.area = "Only letters, numbers and spaces allowed";
      if (formData.pinCode && !isValidPincode(formData.pinCode)) e.pinCode = "Enter a valid 6-digit PIN code";
      if (formData.state && !isOnlyLettersAndSpaces(formData.state)) e.state = "Only letters and spaces allowed";
      if (formData.district && !isOnlyLettersAndSpaces(formData.district)) e.district = "Only letters and spaces allowed";
      if (!formData.commercialType) e.commercialType = "Please select a commercial type";
      if (formData.builtUpArea && !isOnlyDigits(formData.builtUpArea)) e.builtUpArea = "Only digits allowed";
      if (formData.carpetArea && !isOnlyDigits(formData.carpetArea)) e.carpetArea = "Only digits allowed";
      if (formData.floorNumber && !isOnlyDigits(formData.floorNumber)) e.floorNumber = "Only digits allowed";
      if (formData.totalFloors && !isOnlyDigits(formData.totalFloors)) e.totalFloors = "Only digits allowed";
      if (formData.propertyAge && !isOnlyDigits(formData.propertyAge)) e.propertyAge = "Only digits allowed";
      if (formData.frontageWidth && !isOnlyDigits(formData.frontageWidth)) e.frontageWidth = "Only digits allowed";
      if (formData.ceilingHeight && !isOnlyDigits(formData.ceilingHeight)) e.ceilingHeight = "Only digits allowed";
      if (formData.parkingCapacity && !isOnlyDigits(formData.parkingCapacity)) e.parkingCapacity = "Only digits allowed";
    ...
    if (s === 3) {
      ...
      else if (parseFloat(formData.sellPrice) < 0) e.sellPrice = "Selling price cannot be negative";
      else if (!isOnlyDigits(formData.sellPrice)) e.sellPrice = "Only digits allowed";
    ...
    if (s === 5) {
      ...
      if (!formData.saleDeed) e.saleDeed = "Sale deed is required";
      if (!formData.encumbranceCertificate) e.encumbranceCertificate = "Encumbrance certificate is required";
      if (!formData.tradeLicense) e.tradeLicense = "Trade license is required";
      if (!formData.buildingApprovalPlan) e.buildingApprovalPlan = "Building approval plan is required";
      if (!formData.occupancyCertificate) e.occupancyCertificate = "Occupancy certificate is required";
    ...
    if (s === 8) {
      ...
      if (!formData.declaration1) e.declaration1 = "You must confirm this to proceed";
      if (!formData.declaration2) e.declaration2 = "You must confirm this to proceed";
      if (!formData.declaration3) e.declaration3 = "You must agree to proceed";
    }
    return e;
  };
...
function MobContentSellAgentCom({ 
  ...
  // STEP 2: Property Details - Updated with validation
  if (step === 2) return (
    <>
      <div className="flex items-center gap-1.5 mb-2 pb-1.5 border-b-2 border-green-50">
        <div className="w-1 h-3 bg-[#00695C] rounded" />
      ...
      <Field label="PIN Code" hint="Exactly 6 digits">
      ...
      <Field label="Built-up Area" hint="In square feet">
        <input className={inp} type="number" min="0" placeholder="Enter built-up area in sq.ft" value={formData.builtUpArea} onChange={(e) => updateForm("builtUpArea", handleNumericFieldChange(e.target.value))} />
      ...
      <Field label="Carpet Area" hint="In square feet">
        <input className={inp} type="number" min="0" placeholder="Enter carpet area in sq.ft" value={formData.carpetArea} onChange={(e) => updateForm("carpetArea", handleNumericFieldChange(e.target.value))} />
      ...
      <Field label="Floor Number">
        ...
        {errors.floorNumber && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.floorNumber}</p>}
      </Field>
      <Field label="Total Floors">
        ...
        {errors.totalFloors && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.totalFloors}</p>}
      </Field>
      ...
      <Field label="Frontage Width (ft)">
        ...
        {errors.frontageWidth && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.frontageWidth}</p>}
      </Field>
      <Field label="Ceiling Height (ft)">
        ...
        {errors.ceilingHeight && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.ceilingHeight}</p>}
      </Field>
      ...
      <Field label="Parking Capacity">
        ...
        {errors.parkingCapacity && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.parkingCapacity}</p>}
      </Field>
      <Field label="Business Type Suitable" required>
        ...
        {errors.businessType && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.businessType}</p>}
      </Field>
  ...
  if (step === 3) return (
    <>
      ...
      <Field label="Selling Price (₹)" required>
        <input className={inp} type="number" min="0" placeholder="Enter selling price" value={formData.sellPrice} onChange={(e) => updateForm("sellPrice", handleNumericFieldChange(e.target.value))} />
      ...
      <Field label="Budget Range (₹)" hint="Set a range for negotiation">
        <div className="flex gap-1">
          <input className={`${inp} w-1/2`} type="number" min="0" placeholder="Min" value={formData.budgetRange.min} onChange={(e) => updateForm("budgetRange", { ...formData.budgetRange, min: handleNumericFieldChange(e.target.value) })} />
          <input className={`${inp} w-1/2`} type="number" min="0" placeholder="Max" value={formData.budgetRange.max} onChange={(e) => updateForm("budgetRange", { ...formData.budgetRange, max: handleNumericFieldChange(e.target.value) })} />
        </div>
      </Field>
  ...
  if (step === 5) return (
    <>
      ...
      <Field label="Agency Logo" hint="Optional (JPG, PNG max 2MB)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept="image/*" className="hidden" id="m-logo-sell-com" onChange={(e) => {
            const file = e.target.files[0];
            if (file) {
              if (file.size > 2 * 1024 * 1024) {
      ...
      <Field label="Sale Deed" required>
        ...
        {errors.saleDeed && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.saleDeed}</p>}
      </Field>
      ...
      <Field label="Encumbrance Certificate (EC)" required>
        ...
        {errors.encumbranceCertificate && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.encumbranceCertificate}</p>}
      </Field>
      ...
      <Field label="Trade License" required>
        ...
        {errors.tradeLicense && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.tradeLicense}</p>}
      </Field>
      ...
      <Field label="Building Approval Plan" required>
        ...
        {errors.buildingApprovalPlan && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.buildingApprovalPlan}</p>}
      </Field>
      ...
      <Field label="Occupancy Certificate" required>
        ...
        {errors.occupancyCertificate && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.occupancyCertificate}</p>}
      </Field>
      ...
      <Field label="Fire Safety Certificate" required>
        ...
        {errors.fireSafetyCertificate && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.fireSafetyCertificate}</p>}
      </Field>
    </>
  );
  ...
  if (step === 8) return (
    <>
      ...
      <div className="space-y-1.5">
        ...
        {errors.declaration1 && <p className="text-[10px] text-red-500 font-medium ml-5">{errors.declaration1}</p>}
        <label className="flex items-start gap-1.5 text-[10px] cursor-pointer">
          <input 
            type="checkbox" 
            className="accent-[#00695C] w-3.5 h-3.5 mt-0.5 cursor-pointer" 
        ...
        {errors.declaration2 && <p className="text-[10px] text-red-500 font-medium ml-5">{errors.declaration2}</p>}
        <label className="flex items-start gap-1.5 text-[10px] cursor-pointer">
          <input 
            type="checkbox" 
            className="accent-[#00695C] w-3.5 h-3.5 mt-0.5 cursor-pointer" 
        ...
        {errors.declaration3 && <p className="text-[10px] text-red-500 font-medium ml-5">{errors.declaration3}</p>}
      </div>
    </>
  );
...
function DtContentSellAgentCom({ 
  ...
  // STEP 2: Property Details (Desktop) - Updated with validation
  if (step === 2) return (
    <>
      <div className="flex items-center gap-2 mb-3 pb-2 border-b-2 border-green-50">
        <div className="w-1 h-4 bg-[#00695C] rounded" />
      ...
      <FieldDt label="PIN Code" hint="Exactly 6 digits">
        <input className={inp} type="tel" inputMode="numeric" maxLength={6} placeholder="Enter 6-digit PIN code" value={formData.pinCode} onChange={(e) => updateForm("pinCode", handleNumericFieldChange(e.target.value).slice(0, 6))} />
        {errors.pinCode && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.pinCode}</p>}
      </FieldDt>
  ...
  if (step === 5) return (
    <>
      ...
      <FieldDt label="Agency Logo" hint="Optional (JPG, PNG max 2MB)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept="image/*" className="hidden" id="dt-logo-sell-com" onChange={(e) => {
            const file = e.target.files[0];
            if (file) {
              if (file.size > 2 * 1024 * 1024) {
                alert("Logo must be less than 2MB");
  ...
  if (step === 8) return (
    <>
      ...
      <div className="space-y-2">
        ...
        {errors.declaration1 && <p className="text-[10px] text-red-500 font-medium ml-6">{errors.declaration1}</p>}
        <label className="flex items-start gap-2 text-[13px] cursor-pointer">
          <input 
            type="checkbox" 
            className="accent-[#00695C] w-4 h-4 mt-0.5 cursor-pointer" 
        ...
        {errors.declaration2 && <p className="text-[10px] text-red-500 font-medium ml-6">{errors.declaration2}</p>}
        <label className="flex items-start gap-2 text-[13px] cursor-pointer">
          <input 
            type="checkbox" 
            className="accent-[#00695C] w-4 h-4 mt-0.5 cursor-pointer" 
        ...
        {errors.declaration3 && <p className="text-[10px] text-red-500 font-medium ml-6">{errors.declaration3}</p>}
      </div>
    </>
  );
