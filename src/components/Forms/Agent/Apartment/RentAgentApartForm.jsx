...
export default function RentAgentApartForm({ isOpen, onClose }) {
  ...
  const validateStep = (s) => {
    ...
    if (s === 3) {
      ...
      else if (!isOnlyNumbers(formData.rentPrice)) e.rentPrice = "Only numbers allowed";
    }
    if (s === 4) {
  ...
  if (step === 3) return (
    <>
      ...
      <Field label="Monthly Rent (₹)" required>
        <input className={inp} type="text" inputMode="numeric" placeholder="Enter monthly rent amount" value={formData.rentPrice} onChange={(e) => handleNumericFieldChange("rentPrice", e.target.value)} />
