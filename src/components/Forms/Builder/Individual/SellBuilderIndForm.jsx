      ...
      <div className="fixed inset-0 z-50 flex flex-col sm:hidden">
        ...
        <div className="flex-1 bg-white rounded-3xl flex flex-col overflow-hidden shadow-2xl mx-5 mb-5">
          <div className="relative flex flex-col items-center justify-center px-4 pt-3 pb-3 overflow-hidden shrink-0 rounded-t-3xl"
            ...
            <h1 className="text-[13px] font-extrabold text-white tracking-wide relative z-10 text-center">Sell Property - Builder</h1>
      ...
      <div className="fixed inset-0 bg-black/60 z-50 hidden sm:flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
          <div className="relative flex flex-col items-center justify-center min-h-[65px] px-4 pt-2.5 pb-2.5 overflow-hidden shrink-0 rounded-3xl"
            ...
            <button onClick={onClose} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/25 hover:bg-white/40 flex items-center justify-center z-10 text-white font-bold text-[11px]">✕</button>
            <div className="text-xl mb-0.5 relative z-10">🏗️</div>
            <h1 className="text-[14px] font-extrabold text-white tracking-wide relative z-10">Sell Property - Builder</h1>
...
function MobContentSellBuilder({ step, inp, formData, updateForm, imagePreviews, handleImageUpload, removeImage, handleVideoUpload, videoPreview, removeVideo, handleDocumentUpload, removeFile, removeBrochureImage, reraPreview, floorPlanPreview, builderIdPreview, brochurePreview, toggleAmenity, toggleSellAmenity, availableAmenities, customAmenitiesList, addCustomAmenity, removeCustomAmenity, bedroomOptions, bathroomOptions, furnishingOptions, parkingOptions, yesNoOptions, sellAmenities }) {
    ...
    <>
      ...
      <Field label="Property Type" required>
        {["Residential", "Commercial", "Mill / Industrial"].map(t => (
          <label key={t} className="flex items-center gap-2 text-[11px] mb-1 cursor-pointer">
            <input type="radio" name="mob-ptype-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.propertyType === t} onChange={() => updateForm("propertyType", t)} readOnly={false} />
            {t}
          </label>
        ))}
      </Field>
      ...
      <Field label="Furnishing Status" required>
        {["Full Furnish", "Semi Furnish", "Unfurnished"].map(f => (
          <label key={f} className="flex items-center gap-2 text-[11px] mb-1 cursor-pointer">
            <input type="radio" name="mob-furnish-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.furnishingStatus === f} onChange={() => updateForm("furnishingStatus", f)} readOnly={false} />
            {f}
          </label>
        ))}
      </Field>
      <Field label="Parking">
        <div className="flex gap-4">
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-parking-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.parking === "yes"} onChange={() => updateForm("parking", "yes")} readOnly={false} />
            Yes
          </label>
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-parking-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.parking === "no"} onChange={() => updateForm("parking", "no")} readOnly={false} />
            No
          </label>
        </div>
      </Field>
        ...
        <div className="flex gap-2">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-negotiable-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.negotiable === option} onChange={() => updateForm("negotiable", option)} />
              {option}
            </label>
          ))}
        </div>
      ...
      <Field label="Property Condition">
        {["New", "Good", "Renovated", "Needs Renovation"].map(condition => (
          <label key={condition} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
            <input type="radio" name="mob-condition-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.propertyCondition === condition} onChange={() => updateForm("propertyCondition", condition)} />
            {condition}
          </label>
        ))}
      </Field>
      <Field label="Ownership Type">
        {["Freehold", "Leasehold"].map(type => (
          <label key={type} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
            <input type="radio" name="mob-ownership-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.ownershipType === type} onChange={() => updateForm("ownershipType", type)} />
            {type}
          </label>
        ))}
      </Field>
      <Field label="Loan Outstanding">
        <div className="flex gap-2">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-loan-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.loanOutstanding === option} onChange={() => updateForm("loanOutstanding", option)} />
              {option}
            </label>
          ))}
        </div>
      ...
      <Field label="Garden Space">
        <div className="flex gap-2">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-garden-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.gardenSpace === option} onChange={() => updateForm("gardenSpace", option)} />
              {option}
            </label>
          ))}
        </div>
      ...
      <Field label="Terrace / Balcony">
        <div className="flex gap-2">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-terrace-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.terrace === option} onChange={() => updateForm("terrace", option)} />
              {option}
            </label>
          ))}
        </div>
  ...
  if (step === 2) return (
    <>
      ...
      <Field label="Listing Purpose" required>
        <div className="flex gap-4">
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-purpose-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.listingPurpose === "sale"} onChange={() => updateForm("listingPurpose", "sale")} readOnly={false} />
            For Sale
          </label>
        </div>
      </Field>
      ...
      <Field label="Price Type">
        <div className="flex gap-4">
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-pt-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "fixed"} onChange={() => updateForm("priceType", "fixed")} readOnly={false} />
            Fixed
          </label>
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-pt-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "negotiable"} onChange={() => updateForm("priceType", "negotiable")} readOnly={false} />
            Negotiable
          </label>
        </div>
      </Field>
    ...
    <>
      ...
      <Field label="Property Images" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept="image/*" multiple className="hidden" id="m-imgs-sellbuilder" onChange={handleImageUpload} />
          <label htmlFor="m-imgs-sellbuilder" className="cursor-pointer flex flex-col items-center">
      ...
      <Field label="Property Video">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept="video/mp4,video/mov" className="hidden" id="m-vid-sellbuilder" onChange={handleVideoUpload} />
          <label htmlFor="m-vid-sellbuilder" className="cursor-pointer flex flex-col items-center">
  ...
  if (step === 4) return (
    <>
      ...
      <Field label="RERA Registration Certificate" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="m-rera-sell" onChange={(e) => handleDocumentUpload("reraCert", e)} />
          <label htmlFor="m-rera-sell" className="cursor-pointer flex flex-col items-center">
      ...
      <Field label="Property Layout/Floor Plan" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="m-floorplan-sell" onChange={(e) => handleDocumentUpload("floorPlan", e)} />
      ...
      <Field label="Builder ID Proof/Company Registration" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="m-builderid-sell" onChange={(e) => handleDocumentUpload("builderIdProof", e)} />
          <label htmlFor="m-builderid-sell" className="cursor-pointer flex flex-col items-center">
            <div className="w-8 h-8 rounded-full bg-[#00695C] flex items-center justify-center mb-1">
              <Upload className="w-4 h-4 text-white" />
            </div>
      ...
      <Field label="Project Brochure/Images">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" multiple className="hidden" id="m-brochure-sell" onChange={(e) => handleDocumentUpload("brochure", e)} />
          <label htmlFor="m-brochure-sell" className="cursor-pointer flex flex-col items-center">
            <div className="w-8 h-8 rounded-full bg-[#00695C] flex items-center justify-center mb-1">
              <Upload className="w-4 h-4 text-white" />
            </div>
    ...
    <>
      ...
      <FieldDt label="Property Type" required>
        {["Residential", "Commercial", "Mill / Industrial"].map(t => (
          <label key={t} className="flex items-center gap-2 text-[13px] mb-2 cursor-pointer">
            <input type="radio" name="dt-ptype-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.propertyType === t} onChange={() => updateForm("propertyType", t)} readOnly={false} />
            {t}
          </label>
        ))}
      </FieldDt>
      ...
      <FieldDt label="Furnishing Status" required>
        {["Full Furnish", "Semi Furnish", "Unfurnished"].map(f => (
          <label key={f} className="flex items-center gap-2 text-[13px] mb-2 cursor-pointer">
            <input type="radio" name="dt-furnish-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.furnishingStatus === f} onChange={() => updateForm("furnishingStatus", f)} readOnly={false} />
            {f}
          </label>
        ))}
      </FieldDt>
      <FieldDt label="Parking Facility">
        <div className="flex gap-5">
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-parking-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.parking === "yes"} onChange={() => updateForm("parking", "yes")} readOnly={false} />
            Yes, available
          </label>
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-parking-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.parking === "no"} onChange={() => updateForm("parking", "no")} readOnly={false} />
            No parking
          </label>
        </div>
      </FieldDt>
        ...
        <div className="flex gap-2">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-2 p-2 rounded-lg border-2 border-teal-200 bg-white hover:border-teal-500 cursor-pointer transition-all duration-300 flex-1 text-center justify-center">
              <input type="radio" name="dt-negotiable-sellbuilder" className="w-3.5 h-3.5 text-teal-600 focus:ring-teal-500/30" checked={formData.negotiable === option} onChange={() => updateForm("negotiable", option)} />
        ...
        <div className="grid grid-cols-2 gap-2">
          {["New", "Good", "Renovated", "Needs Renovation"].map(condition => (
            <label key={condition} className="flex items-center gap-2 p-2 rounded-lg border-2 border-teal-200 bg-white hover:border-teal-500 cursor-pointer transition-all duration-300">
              <input type="radio" name="dt-condition-sellbuilder" className="w-3.5 h-3.5 text-teal-600 focus:ring-teal-500/30" checked={formData.propertyCondition === condition} onChange={() => updateForm("propertyCondition", condition)} />
        ...
        <div className="flex gap-2">
          {["Freehold", "Leasehold"].map(type => (
            <label key={type} className="flex items-center gap-2 p-2 rounded-lg border-2 border-teal-200 bg-white hover:border-teal-500 cursor-pointer transition-all duration-300 flex-1 text-center justify-center">
              <input type="radio" name="dt-ownership-sellbuilder" className="w-3.5 h-3.5 text-teal-600 focus:ring-teal-500/30" checked={formData.ownershipType === type} onChange={() => updateForm("ownershipType", type)} />
      ...
      <FieldDt label="Loan Outstanding">
        <div className="flex gap-2">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-2 p-2 rounded-lg border-2 border-teal-200 bg-white hover:border-teal-500 cursor-pointer transition-all duration-300 flex-1 text-center justify-center">
              <input type="radio" name="dt-loan-sellbuilder" className="w-3.5 h-3.5 text-teal-600 focus:ring-teal-500/30" checked={formData.loanOutstanding === option} onChange={() => updateForm("loanOutstanding", option)} />
              <span className="text-sm font-semibold text-teal-900">{option}</span>
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Garden Space">
        <div className="flex gap-2">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-2 p-2 rounded-lg border-2 border-teal-200 bg-white hover:border-teal-500 cursor-pointer transition-all duration-300 flex-1 text-center justify-center">
              <input type="radio" name="dt-garden-sellbuilder" className="w-3.5 h-3.5 text-teal-600 focus:ring-teal-500/30" checked={formData.gardenSpace === option} onChange={() => updateForm("gardenSpace", option)} />
              <span className="text-sm font-semibold text-teal-900">{option}</span>
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Terrace / Balcony">
        <div className="flex gap-2">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-2 p-2 rounded-lg border-2 border-teal-200 bg-white hover:border-teal-500 cursor-pointer transition-all duration-300 flex-1 text-center justify-center">
              <input type="radio" name="dt-terrace-sellbuilder" className="w-3.5 h-3.5 text-teal-600 focus:ring-teal-500/30" checked={formData.terrace === option} onChange={() => updateForm("terrace", option)} />
              <span className="text-sm font-semibold text-teal-900">{option}</span>
            </label>
          ))}
        </div>
  ...
  if (step === 2) return (
    <>
      ...
      <FieldDt label="Listing Purpose" required>
        <div className="flex gap-5">
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-purpose-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.listingPurpose === "sale"} onChange={() => updateForm("listingPurpose", "sale")} readOnly={false} />
            For Sale
          </label>
        </div>
      </FieldDt>
      ...
      <FieldDt label="Price Type">
        <div className="flex gap-5">
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-priceType-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "fixed"} onChange={() => updateForm("priceType", "fixed")} readOnly={false} />
            Fixed Price
          </label>
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-priceType-sellbuilder" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "negotiable"} onChange={() => updateForm("priceType", "negotiable")} readOnly={false} />
            Negotiable
          </label>
        </div>
      </FieldDt>
    ...
    <>
      ...
      <FieldDt label="Property Images" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept="image/*" multiple className="hidden" id="property-images-sellbuilder" onChange={handleImageUpload} />
          <label htmlFor="property-images-sellbuilder" className="cursor-pointer flex flex-col items-center">
      ...
      <FieldDt label="Property Video">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept="video/mp4,video/mov" className="hidden" id="property-video-sellbuilder" onChange={handleVideoUpload} />
          <label htmlFor="property-video-sellbuilder" className="cursor-pointer flex flex-col items-center">
    ...
    <>
      ...
      <FieldDt label="RERA Registration Certificate" required>
        <div className="upload-box cursor-pointer hover:bg-green-50 border-2 border-dashed border-teal-300 rounded-xl p-4 text-center">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="dt-rera-sell" onChange={(e) => handleDocumentUpload("reraCert", e)} />
          <label htmlFor="dt-rera-sell" className="cursor-pointer flex flex-col items-center">
      ...
      <FieldDt label="Property Layout/Floor Plan" required>
        <div className="upload-box cursor-pointer hover:bg-green-50 border-2 border-dashed border-teal-300 rounded-xl p-4 text-center">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="dt-floorplan-sell" onChange={(e) => handleDocumentUpload("floorPlan", e)} />
      ...
      <FieldDt label="Builder ID Proof/Company Registration" required>
        <div className="upload-box cursor-pointer hover:bg-green-50 border-2 border-dashed border-teal-300 rounded-xl p-4 text-center">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="dt-builderid-sell" onChange={(e) => handleDocumentUpload("builderIdProof", e)} />
          <label htmlFor="dt-builderid-sell" className="cursor-pointer flex flex-col items-center">
            <div className="w-10 h-10 rounded-full bg-[#00695C] flex items-center justify-center mb-2">
              <Upload className="w-5 h-5 text-white" />
            </div>
      ...
      <FieldDt label="Project Brochure/Images">
        <div className="upload-box cursor-pointer hover:bg-green-50 border-2 border-dashed border-teal-300 rounded-xl p-4 text-center">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" multiple className="hidden" id="dt-brochure-sell" onChange={(e) => handleDocumentUpload("brochure", e)} />
          <label htmlFor="dt-brochure-sell" className="cursor-pointer flex flex-col items-center">
            <div className="w-10 h-10 rounded-full bg-[#00695C] flex items-center justify-center mb-2">
              <Upload className="w-5 h-5 text-white" />
            </div>
