...
function MobContentSell({ step, inp, formData, updateForm, imagePreviews, handleImageUpload, removeImage, handleVideoUpload, videoPreview, removeVideo, handleDocumentUpload, toggleAmenity, toggleSellAmenity, availableAmenities, customAmenitiesList, addCustomAmenity, removeCustomAmenity, bedroomOptions, bathroomOptions, furnishingOptions, parkingOptions, yesNoOptions, sellAmenities }) {
    ...
    <>
      ...
      <Field label="Property Images" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept="image/*" multiple className="hidden" id="m-imgs-sell" onChange={handleImageUpload} />
          <label htmlFor="m-imgs-sell" className="cursor-pointer flex flex-col items-center">
  ...
  if (step === 4) return (
    <>
      ...
      {[
        { id: "m-own-sell", e: "📄", t: "Upload Ownership Document", s: "Sale deed, registry (PDF/JPG/PNG)", l: "Ownership Proof", r: true, key: "ownershipDoc" },
        { id: "m-id-sell", e: "🪪", t: "Upload ID Proof", s: "Aadhaar, PAN or Passport", l: "Owner ID Proof", r: true, key: "idProofDoc" },
        { id: "m-add-sell", e: "📎", t: "+ Add More Documents", s: "Tax receipts, NOC, floor plans", l: "Additional Documents", r: false, key: "additionalDocs" }
      ].map(d => (
...
// DESKTOP CONTENT - SELL (Updated to match Buy form styling)
function DtContentSell({ step, inp, formData, updateForm, imagePreviews, handleImageUpload, removeImage, handleVideoUpload, videoPreview, removeVideo, handleDocumentUpload, toggleAmenity, toggleSellAmenity, availableAmenities, customAmenitiesList, addCustomAmenity, removeCustomAmenity, bedroomOptions, bathroomOptions, furnishingOptions, parkingOptions, yesNoOptions, sellAmenities }) {
    ...
    <>
        ...
        <div className="flex gap-2">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-2 p-2 rounded-lg border-2 border-teal-200 bg-white hover:border-teal-500 cursor-pointer transition-all duration-300 flex-1 text-center justify-center">
              <input type="radio" name="dt-negotiable-sell" className="w-3.5 h-3.5 text-teal-600 focus:ring-teal-500/30" checked={formData.negotiable === option} onChange={() => updateForm("negotiable", option)} />
        ...
        <div className="grid grid-cols-2 gap-2">
          {["New", "Good", "Renovated", "Needs Renovation"].map(condition => (
            <label key={condition} className="flex items-center gap-2 p-2 rounded-lg border-2 border-teal-200 bg-white hover:border-teal-500 cursor-pointer transition-all duration-300">
              <input type="radio" name="dt-condition-sell" className="w-3.5 h-3.5 text-teal-600 focus:ring-teal-500/30" checked={formData.propertyCondition === condition} onChange={() => updateForm("propertyCondition", condition)} />
        ...
        <div className="flex gap-2">
          {["Freehold", "Leasehold"].map(type => (
            <label key={type} className="flex items-center gap-2 p-2 rounded-lg border-2 border-teal-200 bg-white hover:border-teal-500 cursor-pointer transition-all duration-300 flex-1 text-center justify-center">
              <input type="radio" name="dt-ownership-sell" className="w-3.5 h-3.5 text-teal-600 focus:ring-teal-500/30" checked={formData.ownershipType === type} onChange={() => updateForm("ownershipType", type)} />
      ...
      <FieldDt label="Loan Outstanding">
        <div className="flex gap-2">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-2 p-2 rounded-lg border-2 border-teal-200 bg-white hover:border-teal-500 cursor-pointer transition-all duration-300 flex-1 text-center justify-center">
              <input type="radio" name="dt-loan-sell" className="w-3.5 h-3.5 text-teal-600 focus:ring-teal-500/30" checked={formData.loanOutstanding === option} onChange={() => updateForm("loanOutstanding", option)} />
              <span className="text-sm font-semibold text-teal-900">{option}</span>
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Garden Space">
        <div className="flex gap-2">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-2 p-2 rounded-lg border-2 border-teal-200 bg-white hover:border-teal-500 cursor-pointer transition-all duration-300 flex-1 text-center justify-center">
              <input type="radio" name="dt-garden-sell" className="w-3.5 h-3.5 text-teal-600 focus:ring-teal-500/30" checked={formData.gardenSpace === option} onChange={() => updateForm("gardenSpace", option)} />
              <span className="text-sm font-semibold text-teal-900">{option}</span>
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Terrace / Balcony">
        <div className="flex gap-2">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-2 p-2 rounded-lg border-2 border-teal-200 bg-white hover:border-teal-500 cursor-pointer transition-all duration-300 flex-1 text-center justify-center">
              <input type="radio" name="dt-terrace-sell" className="w-3.5 h-3.5 text-teal-600 focus:ring-teal-500/30" checked={formData.terrace === option} onChange={() => updateForm("terrace", option)} />
              <span className="text-sm font-semibold text-teal-900">{option}</span>
            </label>
          ))}
        </div>
    ...
    <>
      ...
      <FieldDt label="Property Images" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept="image/*" multiple className="hidden" id="property-images-sell" onChange={handleImageUpload} />
          <label htmlFor="property-images-sell" className="cursor-pointer flex flex-col items-center">
      ...
      <FieldDt label="Property Video">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept="video/mp4,video/mov" className="hidden" id="property-video-sell" onChange={handleVideoUpload} />
          <label htmlFor="property-video-sell" className="cursor-pointer flex flex-col items-center">
  ...
  if (step === 4) return (
    <>
      ...
      {[
        { id: "ownership-proof-sell", e: "📄", t: "Upload Ownership Document", s: "Sale deed, registry (PDF/JPG/PNG)", l: "Property Ownership Proof", r: true, key: "ownershipDoc" },
        { id: "id-proof-sell", e: "🪪", t: "Upload ID Proof", s: "Aadhaar, PAN, or Passport", l: "Owner ID Proof", r: true, key: "idProofDoc" },
        { id: "additional-docs-sell", e: "📎", t: "+ Add More Documents", s: "Tax receipts, NOC, floor plans", l: "Additional Documents", r: false, key: "additionalDocs" }
      ].map(d => (
