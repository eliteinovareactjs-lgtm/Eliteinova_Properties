...
// Buy filter options - integrated as form fields
...
export default function IndBuyForm({ isOpen, onClose }) {
      ...
      <div className="fixed inset-0 z-50 flex flex-col sm:hidden">
        ...
        <div className="flex-1 bg-white rounded-3xl flex flex-col overflow-hidden shadow-2xl mx-5 mb-5">
          <div className="relative flex flex-col items-center justify-center px-4 pt-3 pb-3 overflow-hidden shrink-0 rounded-t-3xl"
            ...
            <h1 className="text-[13px] font-extrabold text-white tracking-wide relative z-10 text-center">Buy Property - Individual Owner</h1>
      ...
      <div className="fixed inset-0 bg-black/60 z-50 hidden sm:flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
          <div className="relative flex flex-col items-center justify-center min-h-[65px] px-4 pt-2.5 pb-2.5 overflow-hidden shrink-0 rounded-3xl"
            ...
            <button onClick={onClose} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/25 hover:bg-white/40 flex items-center justify-center z-10 text-white font-bold text-[11px]">✕</button>
            <div className="text-xl mb-0.5 relative z-10">🏠</div>
            <h1 className="text-[14px] font-extrabold text-white tracking-wide relative z-10">Buy Property - Individual Owner</h1>
...
// Mobile Content - All steps with integrated fields
function MobContent({ step, inp, formData, updateForm, imagePreviews, handleImageUpload, removeImage, handleVideoUpload, videoPreview, removeVideo, handleDocumentUpload, toggleAmenity, toggleBuyAmenity, availableAmenities, customAmenitiesList, addCustomAmenity, removeCustomAmenity, bedroomOptions, bathroomOptions, furnishingOptions, parkingOptions, facingOptions, yesNoOptions, buyAmenities }) {
      ...
      <Field label="Property Type" required>
        {["Residential", "Commercial", "Mill / Industrial"].map(t => (
          <label key={t} className="flex items-center gap-2 text-[11px] mb-1 cursor-pointer">
            <input type="radio" name="mob-ptype" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.propertyType === t} onChange={() => updateForm("propertyType", t)} readOnly={false} />
            {t}
          </label>
        ))}
      </Field>
      ...
      <Field label="Parking">
        <div className="flex gap-4">
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-parking" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.parking === "yes"} onChange={() => updateForm("parking", "yes")} readOnly={false} />
            Yes
          </label>
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-parking" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.parking === "no"} onChange={() => updateForm("parking", "no")} readOnly={false} />
            No
          </label>
        </div>
      </Field>

      {/* Integrated Buy Preferences */}
      ...
      <Field label="Purchase Time">
        {["Immediately", "Within 3 Months", "Within 6 Months", "Just Exploring"].map(time => (
          <label key={time} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
            <input type="radio" name="mob-purchase" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.purchaseTime === time} onChange={() => updateForm("purchaseTime", time)} />
            {time}
          </label>
        ))}
      </Field>
      <Field label="Buying Purpose">
        {["Self Use", "Investment", "Vacation Home"].map(purpose => (
          <label key={purpose} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
            <input type="radio" name="mob-purpose-buy" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.buyingPurpose === purpose} onChange={() => updateForm("buyingPurpose", purpose)} />
            {purpose}
          </label>
        ))}
      </Field>
        ...
        <div className="flex gap-2">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-loan" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.homeLoanRequired === option} onChange={() => updateForm("homeLoanRequired", option)} />
              {option}
            </label>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-1">
          {facingOptions.map(facing => (
            <label key={facing} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-facing" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.facingPreference === facing} onChange={() => updateForm("facingPreference", facing)} />
              {facing}
            </label>
          ))}
        </div>
  ...
  if (step === 2) return (
    <>
      ...
      <Field label="Listing Purpose" required>
        <div className="flex gap-4">
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-purpose" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.listingPurpose === "sale"} onChange={() => updateForm("listingPurpose", "sale")} readOnly={false} />
            For Sale
          </label>
        </div>
      </Field>
  ...
  if (step === 4) return (
    <>
      ...
      {[
        { id: "m-own", e: "📄", t: "Upload Ownership Document", s: "Sale deed, registry (PDF/JPG/PNG)", l: "Ownership Proof", r: true, key: "ownershipDoc" },
        { id: "m-id", e: "🪪", t: "Upload ID Proof", s: "Aadhaar, PAN or Passport", l: "Owner ID Proof", r: true, key: "idProofDoc" },
        { id: "m-add", e: "📎", t: "+ Add More Documents", s: "Tax receipts, NOC, floor plans", l: "Additional Documents", r: false, key: "additionalDocs" }
      ].map(d => (
...
// Desktop Content - All steps with integrated fields
function DtContent({ step, inp, formData, updateForm, imagePreviews, handleImageUpload, removeImage, handleVideoUpload, videoPreview, removeVideo, handleDocumentUpload, toggleAmenity, toggleBuyAmenity, availableAmenities, customAmenitiesList, addCustomAmenity, removeCustomAmenity, bedroomOptions, bathroomOptions, furnishingOptions, parkingOptions, facingOptions, yesNoOptions, buyAmenities }) {
  const ta = `${inp} resize-y`;
      ...
      <FieldDt label="Property Type" required>
        {["Residential", "Commercial", "Mill / Industrial"].map(t => (
          <label key={t} className="flex items-center gap-2 text-[13px] mb-2 cursor-pointer">
            <input type="radio" name="dt-ptype" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.propertyType === t} onChange={() => updateForm("propertyType", t)} readOnly={false} />
            {t}
          </label>
        ))}
      </FieldDt>
      ...
      <FieldDt label="Parking Facility">
        <div className="flex gap-5">
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-parking" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.parking === "yes"} onChange={() => updateForm("parking", "yes")} readOnly={false} />
            Yes, available
          </label>
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-parking" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.parking === "no"} onChange={() => updateForm("parking", "no")} readOnly={false} />
            No parking
          </label>
        </div>
      </FieldDt>
      ...
      <FieldDt label="Purchase Time">
        {["Immediately", "Within 3 Months", "Within 6 Months", "Just Exploring"].map(time => (
          <label key={time} className="flex items-center gap-2 text-[13px] mb-1.5 cursor-pointer">
            <input type="radio" name="dt-purchase" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.purchaseTime === time} onChange={() => updateForm("purchaseTime", time)} />
            {time}
          </label>
        ))}
      </FieldDt>
      <FieldDt label="Buying Purpose">
        {["Self Use", "Investment", "Vacation Home"].map(purpose => (
          <label key={purpose} className="flex items-center gap-2 text-[13px] mb-1.5 cursor-pointer">
            <input type="radio" name="dt-purpose-buy" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.buyingPurpose === purpose} onChange={() => updateForm("buyingPurpose", purpose)} />
            {purpose}
          </label>
        ))}
      </FieldDt>
        ...
        <div className="flex gap-5">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-loan" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.homeLoanRequired === option} onChange={() => updateForm("homeLoanRequired", option)} />
              {option}
            </label>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-x-5 gap-y-1.5">
          {facingOptions.map(facing => (
            <label key={facing} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-facing" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.facingPreference === facing} onChange={() => updateForm("facingPreference", facing)} />
              {facing}
            </label>
          ))}
        </div>
  ...
  if (step === 2) return (
    <>
      ...
      <FieldDt label="Listing Purpose" required>
        <div className="flex gap-5">
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-purpose" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.listingPurpose === "sale"} onChange={() => updateForm("listingPurpose", "sale")} readOnly={false} />
            For Sale
          </label>
        </div>
      </FieldDt>
      ...
      <FieldDt label="Price Type">
        <div className="flex gap-5">
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-priceType" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "fixed"} onChange={() => updateForm("priceType", "fixed")} readOnly={false} />
            Fixed Price
          </label>
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-priceType" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "negotiable"} onChange={() => updateForm("priceType", "negotiable")} readOnly={false} />
            Negotiable
          </label>
        </div>
      </FieldDt>
  ...
  if (step === 4) return (
    <>
      ...
      {[
        { id: "ownership-proof", e: "📄", t: "Upload Ownership Document", s: "Sale deed, registry, or title deed (PDF/JPG/PNG)", l: "Property Ownership Proof", r: true, key: "ownershipDoc" },
        { id: "id-proof", e: "🪪", t: "Upload ID Proof", s: "Aadhaar, PAN, or Passport (PDF/JPG/PNG)", l: "Owner ID Proof", r: true, key: "idProofDoc" },
        { id: "additional-docs", e: "📎", t: "+ Add More Documents", s: "Tax receipts, NOC, floor plans (Multiple allowed)", l: "Additional Documents", r: false, key: "additionalDocs" }
      ].map(d => (
