...
// Commercial options - integrated as form fields
const commercialTypeOptions = ["Retail Shop", "Office Space", "Showroom", "Warehouse", "Commercial Complex", "Shopping Mall", "Restaurant", "Hotel", "Co-working Space", "Educational Institution", "Clinic", "Petrol Bunk"];
const furnishingOptions = ["Fully Furnished", "Semi Furnished", "Unfurnished"];
const parkingOptions = ["1 Car", "2 Cars", "3+ Cars", "4+ Cars", "Truck Parking"];
...
export default function ComRentForm({ isOpen, onClose }) {
  ...
  const [formData, setFormData] = useState({
    ...
    propertyCity: "", builtUpArea: "", carpetArea: "", commercialType: "",
    furnishingStatus: "", parking: "",
    // Property Category & Posted By (Step 2)
    propertyCategory: "commercial",
    postedBy: "owner",
    // Pricing & Amenities (Step 3)
    listingPurpose: "rent", expectedPrice: "", budgetRange: { min: "", max: "" }, priceType: "", maintenance: "", securityDeposit: "",
    availableFrom: "", selectedAmenities: [], otherAmenities: "",
    // Commercial Rent Preferences (integrated in step 2)
    leaseTerm: "",
    businessType: "",
    footfall: "",
    operatingHours: "",
  ...
  const handleAlphaFieldChange = (field, value) => {
    if (isOnlyLettersAndSpaces(value) || value === '') {
  ...
  const handleAlphaNumericFieldChange = (field, value) => {
    if (isAlphaNumericWithSpaces(value) || value === '') {
      updateForm(field, value);
    }
  };

  const handlePinCodeChange = (field, value) => {
    if (isOnlyNumbers(value) && value.length <= 6) {
      updateForm(field, value);
    }
  };
  ...
  const validateStep = (s) => {
    ...
    if (s === 0) {
      ...
      else if (!isOnlyLettersAndSpaces(formData.ownerName)) e.ownerName = "Only letters and spaces allowed";
      if (!formData.contactNumber || formData.contactNumber.length !== 10) e.contactNumber = "Enter a valid 10-digit mobile number";
      else if (!isOnlyNumbers(formData.contactNumber)) e.contactNumber = "Mobile number must contain only digits";
    ...
    if (s === 1) {
      ...
      else if (!isOnlyNumbers(formData.aadhaarNumber)) e.aadhaarNumber = "Aadhaar number must contain only numbers";
      ...
      else if (formData.pinCode.length !== 6) e.pinCode = "PIN code must be exactly 6 digits";
    }
    if (s === 2) {
      ...
      if (!formData.commercialType) e.commercialType = "Please select a commercial type";
      ...
      else if (!isOnlyLettersAndSpaces(formData.propertyCity)) e.propertyCity = "Only letters and spaces allowed";
      ...
      else if (!isOnlyNumbers(formData.carpetArea)) e.carpetArea = "Only numbers allowed";
    ...
    if (s === 3) {
      ...
      else if (!isOnlyNumbers(formData.expectedPrice)) e.expectedPrice = "Only numbers allowed";
    }
    if (s === 4) {
    ...
    if (s === 6) {
      ...
      else if (!isOnlyNumbers(formData.accountNumber)) e.accountNumber = "Only numbers allowed";
      else if (formData.accountNumber.length < 9 || formData.accountNumber.length > 18) e.accountNumber = "Account number must be between 9-18 digits";
  ...
  const handleSubmit = () => {
    try {
      console.log("Commercial Rent Form submitted:", formData);
      onClose();
    } catch (err) {
      ...
      <div className="fixed inset-0 z-50 flex flex-col sm:hidden">
        ...
        <div className="flex-1 bg-white rounded-3xl flex flex-col overflow-hidden shadow-2xl mx-5 mb-5">
          <div className="relative flex flex-col items-center justify-center px-4 pt-3 pb-3 overflow-hidden shrink-0 rounded-t-3xl"
            ...
            <h1 className="text-[13px] font-extrabold text-white tracking-wide relative z-10 text-center">Rent Commercial Property - Owner</h1>
            <p className="text-[10px] text-white/80 relative z-10 mt-0.5 text-center">List your commercial property for rent</p>
          </div>
      ...
      <div className="fixed inset-0 bg-black/60 z-50 hidden sm:flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
          <div className="relative flex flex-col items-center justify-center min-h-[65px] px-4 pt-2.5 pb-2.5 overflow-hidden shrink-0 rounded-3xl"
            ...
            <button onClick={onClose} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/25 hover:bg-white/40 flex items-center justify-center z-10 text-white font-bold text-[11px]">✕</button>
            <div className="text-xl mb-0.5 relative z-10">🏢</div>
            <h1 className="text-[14px] font-extrabold text-white tracking-wide relative z-10">Rent Commercial Property - Owner</h1>
            <p className="text-[10px] text-white/80 relative z-10 mt-0.5">List your commercial property for rent</p>
          </div>
...
// MOBILE CONTENT - COMMERCIAL RENT
function MobContentComRent({ step, inp, formData, updateForm, handleAlphaFieldChange, handleNumericFieldChange, handleAlphaNumericFieldChange, handlePinCodeChange, imagePreviews, handleImageUpload, removeImage, handleVideoUpload, videoPreview, removeVideo, handleDocumentUpload, handlePassportUpload, toggleAmenity, availableAmenities, customAmenitiesList, addCustomAmenity, removeCustomAmenity, commercialTypeOptions, furnishingOptions, parkingOptions, yesNoOptions, bankOptions, handleCoverImageUpload, handleFloorPlanUpload, coverPreview, floorPlanPreview, removeCoverImage, removeFloorPlan, toggleContactMethod, isValidEmail, errors, startDrawing, draw, stopDrawing, clearSignature, signaturePoints, allSignaturePoints, setAllSignaturePoints }) {
  ...
  if (step === 0) return (
    <>
      ...
      <Field label="Mobile Number" required>
        <input className={inp} type="tel" inputMode="numeric" maxLength={10} placeholder="Enter your 10-digit mobile number" value={formData.contactNumber} onChange={(e) => handleNumericFieldChange("contactNumber", e.target.value.slice(0, 10))} />
  ...
  if (step === 1) return (
    <>
      ...
      <Field label="Aadhaar Number" required>
        <input className={inp} inputMode="numeric" maxLength={12} placeholder="Enter 12-digit Aadhaar number" value={formData.aadhaarNumber} onChange={(e) => handleNumericFieldChange("aadhaarNumber", e.target.value.slice(0, 12))} />
      ...
      <Field label="Upload Aadhaar Card" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-aadhaar-com-rent" onChange={(e) => handleDocumentUpload("aadhaarCard", e)} />
          <label htmlFor="m-aadhaar-com-rent" className="cursor-pointer flex flex-col items-center">
      ...
      <Field label="Upload PAN Card (Optional)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-pan-com-rent" onChange={(e) => handleDocumentUpload("panCard", e)} />
          <label htmlFor="m-pan-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
      ...
      <Field label="Upload Passport-size Photo" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".jpg,.jpeg,.png" className="hidden" id="m-passport-com-rent" onChange={(e) => handlePassportUpload("passportPhoto", e)} />
          <label htmlFor="m-passport-com-rent" className="cursor-pointer flex flex-col items-center">
  ...
  // STEP 2: Property Details + Commercial Rent Preferences
  if (step === 2) return (
    <>
      <div className="flex items-center gap-1.5 mb-2 pb-1.5 border-b-2 border-green-50">
        <div className="w-1 h-3 bg-[#00695C] rounded" />
      ...
      <Field label="Property Title / Name" required>
        <input className={inp} placeholder="e.g. Prime Retail Space, Office Complex" value={formData.propertyTitle} onChange={(e) => handleAlphaNumericFieldChange("propertyTitle", e.target.value)} />
        {errors.propertyTitle && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.propertyTitle}</p>}
      </Field>
      <Field label="Property Type" required>
        <div className="grid grid-cols-2 gap-1">
          {["Commercial Complex", "Retail Shop", "Office Space", "Showroom", "Warehouse", "Restaurant", "Hotel"].map(t => (
            <label key={t} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-ptype-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.propertyType === t} onChange={() => updateForm("propertyType", t)} />
              {t}
            </label>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-1">
          {commercialTypeOptions.map(type => (
            <label key={type} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-com-type-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.commercialType === type} onChange={() => updateForm("commercialType", type)} />
              {type}
            </label>
          ))}
        </div>
        {errors.commercialType && <p className="text-[10px] text-red-500 font-medium mt-0.5">{errors.commercialType}</p>}
      </Field>
      ...
      <Field label="Area Details" required hint="In square feet">
        <div className="grid grid-cols-2 gap-1.5">
          <input className={inp} type="text" inputMode="numeric" placeholder="Built-up Area" value={formData.builtUpArea} onChange={(e) => handleNumericFieldChange("builtUpArea", e.target.value)} />
        ...
        <div className="grid grid-cols-2 gap-1">
          {furnishingOptions.map(f => (
            <label key={f} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-furnish-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.furnishingStatus === f} onChange={() => updateForm("furnishingStatus", f)} />
              {f}
            </label>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {parkingOptions.map(option => (
            <label key={option} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-parking-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.parking === option} onChange={() => updateForm("parking", option)} />
              {option}
            </label>
          ))}
        </div>
      ...
      <div className="flex items-center gap-1.5 mt-3 mb-2 pb-1.5 border-b-2 border-green-50">
        <div className="w-1 h-3 bg-[#00695C] rounded" />
        <h3 className="text-[11px] font-bold text-[#00695C]">Commercial Rent Preferences</h3>
      </div>
      <Field label="Preferred Lease Term">
        <div className="space-y-0.5">
          {["Short Term (1-3 months)", "Medium Term (3-12 months)", "Long Term (12+ months)"].map(term => (
            <label key={term} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-lease-term-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.leaseTerm === term} onChange={() => updateForm("leaseTerm", term)} />
              {term}
            </label>
          ))}
        </div>
      </Field>
      <Field label="Business Type Suitable">
        <div className="grid grid-cols-2 gap-1">
          {["Retail", "Office", "Food & Beverage", "Warehouse", "Service"].map(type => (
            <label key={type} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-business-type-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.businessType === type} onChange={() => updateForm("businessType", type)} />
              {type}
            </label>
          ))}
        </div>
      </Field>
      <Field label="Estimated Footfall">
        <div className="space-y-0.5">
          {["Low (0-50/day)", "Medium (50-200/day)", "High (200-500/day)", "Very High (500+/day)"].map(level => (
            <label key={level} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-footfall-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.footfall === level} onChange={() => updateForm("footfall", level)} />
              {level}
            </label>
          ))}
        </div>
      </Field>
      <Field label="Operating Hours Flexibility">
        <div className="flex flex-wrap gap-2">
          {["Flexible", "Fixed 9-9", "Fixed 9-6", "Custom"].map(hours => (
            <label key={hours} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-hours-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.operatingHours === hours} onChange={() => updateForm("operatingHours", hours)} />
              {hours}
            </label>
          ))}
        </div>
  ...
  if (step === 3) return (
    <>
      ...
      <Field label="Expected Rent (₹/month)" required>
        <input className={inp} type="text" inputMode="numeric" placeholder="e.g. 25,000" value={formData.expectedPrice} onChange={(e) => handleNumericFieldChange("expectedPrice", e.target.value)} />
      ...
      <Field label="Budget Range (₹/month)" hint="Set a range for negotiation">
        <div className="flex gap-1">
          <input className={inp} type="text" inputMode="numeric" placeholder="Min" value={formData.budgetRange.min} onChange={(e) => updateForm("budgetRange", { ...formData.budgetRange, min: e.target.value })} />
          <input className={inp} type="text" inputMode="numeric" placeholder="Max" value={formData.budgetRange.max} onChange={(e) => updateForm("budgetRange", { ...formData.budgetRange, max: e.target.value })} />
        </div>
      </Field>
      ...
      <Field label="Price Type">
        <div className="flex gap-4">
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-pt-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "fixed"} onChange={() => updateForm("priceType", "fixed")} />
            Fixed
          </label>
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-pt-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "negotiable"} onChange={() => updateForm("priceType", "negotiable")} />
            Negotiable
          </label>
        </div>
      </Field>
    ...
    <>
      ...
      <Field label="Upload Cover Image" required hint="Max 2MB">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept="image/*" className="hidden" id="m-cover-com-rent" onChange={handleCoverImageUpload} />
          <label htmlFor="m-cover-com-rent" className="cursor-pointer flex flex-col items-center">
      ...
      <Field label="Upload Property Photos (Max 3)" required hint={`${formData.propertyImages.length}/3 images uploaded`}>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept="image/*" multiple className="hidden" id="m-imgs-com-rent" onChange={handleImageUpload} disabled={formData.propertyImages.length >= 3} />
          <label htmlFor="m-imgs-com-rent" className={`cursor-pointer flex flex-col items-center ${formData.propertyImages.length >= 3 ? 'opacity-50 cursor-not-allowed' : ''}`}>
            <ImagePlus className="mb-1 w-7 h-7 text-[#00695C]" />
      ...
      <Field label="Upload Property Video (Optional)" hint="Max 10MB">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept="video/mp4,video/mov" className="hidden" id="m-vid-com-rent" onChange={handleVideoUpload} />
          <label htmlFor="m-vid-com-rent" className="cursor-pointer flex flex-col items-center">
  ...
  if (step === 5) return (
    <>
      ...
      <Field label="Upload Floor Plan" required hint="PDF only (Max 5MB)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-floorplan-com-rent" onChange={handleFloorPlanUpload} />
          <label htmlFor="m-floorplan-com-rent" className="cursor-pointer flex flex-col items-center">
      ...
      <Field label="Sale Deed">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-saleDeed-com-rent" onChange={(e) => handleDocumentUpload("saleDeed", e)} />
          <label htmlFor="m-saleDeed-com-rent" className="cursor-pointer flex flex-col items-center">
      ...
      <Field label="Patta / Chitta">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-patta-com-rent" onChange={(e) => handleDocumentUpload("pattaChitta", e)} />
          <label htmlFor="m-patta-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
            <span className="text-[9px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <Field label="Encumbrance Certificate (EC)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-ec-com-rent" onChange={(e) => handleDocumentUpload("encumbranceCertificate", e)} />
          <label htmlFor="m-ec-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
            <span className="text-[9px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <Field label="Property Tax Receipt">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-tax-com-rent" onChange={(e) => handleDocumentUpload("propertyTaxReceipt", e)} />
          <label htmlFor="m-tax-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
            <span className="text-[9px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <Field label="Building Approval Plan">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-building-com-rent" onChange={(e) => handleDocumentUpload("buildingApprovalPlan", e)} />
          <label htmlFor="m-building-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
            <span className="text-[9px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <Field label="Completion Certificate">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-completion-com-rent" onChange={(e) => handleDocumentUpload("completionCertificate", e)} />
          <label htmlFor="m-completion-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
            <span className="text-[9px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <Field label="Occupancy Certificate">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-occupancy-com-rent" onChange={(e) => handleDocumentUpload("occupancyCertificate", e)} />
          <label htmlFor="m-occupancy-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
            <span className="text-[9px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <Field label="Rental Agreement">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-rentalAgreement-com-rent" onChange={(e) => handleDocumentUpload("rentalAgreement", e)} />
          <label htmlFor="m-rentalAgreement-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
            <span className="text-[9px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <Field label="Other Supporting Documents">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" multiple className="hidden" id="m-otherDocs-com-rent" onChange={(e) => {
            const files = Array.from(e.target.files);
          ...
          <label htmlFor="m-otherDocs-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
  ...
  if (step === 6) return (
    <>
      ...
      <Field label="Account Number" required>
        <input className={inp} type="text" inputMode="numeric" placeholder="Enter account number" value={formData.accountNumber} onChange={(e) => handleNumericFieldChange("accountNumber", e.target.value)} />
...
// DESKTOP CONTENT - COMMERCIAL RENT
function DtContentComRent({ step, inp, formData, updateForm, handleAlphaFieldChange, handleNumericFieldChange, handleAlphaNumericFieldChange, handlePinCodeChange, imagePreviews, handleImageUpload, removeImage, handleVideoUpload, videoPreview, removeVideo, handleDocumentUpload, handlePassportUpload, toggleAmenity, availableAmenities, customAmenitiesList, addCustomAmenity, removeCustomAmenity, commercialTypeOptions, furnishingOptions, parkingOptions, yesNoOptions, bankOptions, handleCoverImageUpload, handleFloorPlanUpload, coverPreview, floorPlanPreview, removeCoverImage, removeFloorPlan, toggleContactMethod, isValidEmail, errors, startDrawing, draw, stopDrawing, clearSignature, signaturePoints, allSignaturePoints, setAllSignaturePoints }) {
  const ta = `${inp} resize-y`;
  const signatureCanvasRef = useRef(null);
  ...
  if (step === 1) return (
    <>
      ...
      <FieldDt label="Upload Aadhaar Card" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-aadhaar-com-rent" onChange={(e) => handleDocumentUpload("aadhaarCard", e)} />
          <label htmlFor="dt-aadhaar-com-rent" className="cursor-pointer flex flex-col items-center">
      ...
      <FieldDt label="Upload PAN Card (Optional)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-pan-com-rent" onChange={(e) => handleDocumentUpload("panCard", e)} />
          <label htmlFor="dt-pan-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-7 h-7 text-[#00695C]" />
      ...
      <FieldDt label="Upload Passport-size Photo" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".jpg,.jpeg,.png" className="hidden" id="dt-passport-com-rent" onChange={(e) => handlePassportUpload("passportPhoto", e)} />
          <label htmlFor="dt-passport-com-rent" className="cursor-pointer flex flex-col items-center">
    ...
    <>
        ...
        <div className="grid grid-cols-2 gap-2">
          {["Commercial Complex", "Retail Shop", "Office Space", "Showroom", "Warehouse", "Restaurant", "Hotel"].map(t => (
            <label key={t} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-ptype-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.propertyType === t} onChange={() => updateForm("propertyType", t)} />
              {t}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Commercial Type" required>
        <div className="grid grid-cols-3 gap-1">
          {commercialTypeOptions.map(type => (
            <label key={type} className="flex items-center gap-1.5 text-[12px] cursor-pointer">
              <input type="radio" name="dt-com-type-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.commercialType === type} onChange={() => updateForm("commercialType", type)} />
              {type}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Area Details" required hint="Enter values in square feet">
        <div className="grid grid-cols-2 gap-2">
          <input className={inp} type="text" inputMode="numeric" placeholder="Built-up Area (sq ft)" value={formData.builtUpArea} onChange={(e) => handleNumericFieldChange("builtUpArea", e.target.value)} />
        ...
        <div className="flex flex-wrap gap-3">
          {furnishingOptions.map(f => (
            <label key={f} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-furnish-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.furnishingStatus === f} onChange={() => updateForm("furnishingStatus", f)} />
              {f}
            </label>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          {parkingOptions.map(option => (
            <label key={option} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-parking-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.parking === option} onChange={() => updateForm("parking", option)} />
              {option}
            </label>
          ))}
        </div>
      ...
      <div className="flex items-center gap-2 mt-4 mb-3 pb-2 border-b-2 border-green-50">
        <div className="w-1 h-4 bg-[#00695C] rounded" />
        <h3 className="text-[14px] font-bold text-[#00695C]">Commercial Rent Preferences</h3>
      </div>
      <FieldDt label="Preferred Lease Term">
        <div className="space-y-1">
          {["Short Term (1-3 months)", "Medium Term (3-12 months)", "Long Term (12+ months)"].map(term => (
            <label key={term} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-lease-term-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.leaseTerm === term} onChange={() => updateForm("leaseTerm", term)} />
              {term}
            </label>
          ))}
        </div>
      </FieldDt>
      <FieldDt label="Business Type Suitable">
        <div className="grid grid-cols-2 gap-2">
          {["Retail", "Office", "Food & Beverage", "Warehouse", "Service"].map(type => (
            <label key={type} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-business-type-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.businessType === type} onChange={() => updateForm("businessType", type)} />
              {type}
            </label>
          ))}
        </div>
      </FieldDt>
      <FieldDt label="Estimated Footfall">
        <div className="space-y-1">
          {["Low (0-50/day)", "Medium (50-200/day)", "High (200-500/day)", "Very High (500+/day)"].map(level => (
            <label key={level} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-footfall-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.footfall === level} onChange={() => updateForm("footfall", level)} />
              {level}
            </label>
          ))}
        </div>
      </FieldDt>
      <FieldDt label="Operating Hours Flexibility">
        <div className="flex flex-wrap gap-3">
          {["Flexible", "Fixed 9-9", "Fixed 9-6", "Custom"].map(hours => (
            <label key={hours} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-hours-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.operatingHours === hours} onChange={() => updateForm("operatingHours", hours)} />
              {hours}
            </label>
          ))}
        </div>
  ...
  // STEP 3: Pricing & Amenities (Desktop version - same as mobile but with dt styling)
  if (step === 3) return (
    <>
      <div className="flex items-center gap-2 mb-3 pb-2 border-b-2 border-green-50">
        <div className="w-1 h-4 bg-[#00695C] rounded" />
      ...
      <FieldDt label="Price Type">
        <div className="flex gap-5">
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-priceType-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "fixed"} onChange={() => updateForm("priceType", "fixed")} />
            Fixed Price
          </label>
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-priceType-com-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "negotiable"} onChange={() => updateForm("priceType", "negotiable")} />
            Negotiable
          </label>
        </div>
      </FieldDt>
    ...
    <>
      ...
      <FieldDt label="Upload Cover Image" required hint="Max 2MB">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept="image/*" className="hidden" id="dt-cover-com-rent" onChange={handleCoverImageUpload} />
          <label htmlFor="dt-cover-com-rent" className="cursor-pointer flex flex-col items-center">
      ...
      <FieldDt label="Upload Property Photos (Max 3)" required hint={`${formData.propertyImages.length}/3 images uploaded`}>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept="image/*" multiple className="hidden" id="dt-imgs-com-rent" onChange={handleImageUpload} disabled={formData.propertyImages.length >= 3} />
          <label htmlFor="dt-imgs-com-rent" className={`cursor-pointer flex flex-col items-center ${formData.propertyImages.length >= 3 ? 'opacity-50 cursor-not-allowed' : ''}`}>
            <ImagePlus className="mx-auto mb-2 w-8 h-8 sm:w-10 sm:h-10 text-[#00695C]" />
      ...
      <FieldDt label="Upload Property Video (Optional)" hint="Max 10MB">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept="video/mp4,video/mov" className="hidden" id="dt-vid-com-rent" onChange={handleVideoUpload} />
          <label htmlFor="dt-vid-com-rent" className="cursor-pointer flex flex-col items-center">
  ...
  if (step === 5) return (
    <>
      ...
      <FieldDt label="Upload Floor Plan" required hint="PDF only (Max 5MB)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-floorplan-com-rent" onChange={handleFloorPlanUpload} />
          <label htmlFor="dt-floorplan-com-rent" className="cursor-pointer flex flex-col items-center">
      ...
      <FieldDt label="Sale Deed">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-saleDeed-com-rent" onChange={(e) => handleDocumentUpload("saleDeed", e)} />
          <label htmlFor="dt-saleDeed-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
      ...
      <FieldDt label="Patta / Chitta">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-patta-com-rent" onChange={(e) => handleDocumentUpload("pattaChitta", e)} />
          <label htmlFor="dt-patta-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
            <span className="text-[10px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <FieldDt label="Encumbrance Certificate (EC)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-ec-com-rent" onChange={(e) => handleDocumentUpload("encumbranceCertificate", e)} />
          <label htmlFor="dt-ec-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
            <span className="text-[10px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <FieldDt label="Property Tax Receipt">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-tax-com-rent" onChange={(e) => handleDocumentUpload("propertyTaxReceipt", e)} />
          <label htmlFor="dt-tax-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
            <span className="text-[10px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <FieldDt label="Building Approval Plan">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-building-com-rent" onChange={(e) => handleDocumentUpload("buildingApprovalPlan", e)} />
          <label htmlFor="dt-building-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
            <span className="text-[10px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <FieldDt label="Completion Certificate">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-completion-com-rent" onChange={(e) => handleDocumentUpload("completionCertificate", e)} />
          <label htmlFor="dt-completion-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
            <span className="text-[10px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <FieldDt label="Occupancy Certificate">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-occupancy-com-rent" onChange={(e) => handleDocumentUpload("occupancyCertificate", e)} />
          <label htmlFor="dt-occupancy-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
            <span className="text-[10px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <FieldDt label="Rental Agreement">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-rentalAgreement-com-rent" onChange={(e) => handleDocumentUpload("rentalAgreement", e)} />
          <label htmlFor="dt-rentalAgreement-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
            <span className="text-[10px] text-[#00695C] font-semibold mt-0.5">Upload</span>
          </label>
        </div>
      ...
      <FieldDt label="Other Supporting Documents">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" multiple className="hidden" id="dt-otherDocs-com-rent" onChange={(e) => {
            const files = Array.from(e.target.files);
            const validFiles = files.filter(f => f.type === 'application/pdf');
            if (validFiles.length !== files.length) {
              alert('Only PDF files are allowed');
          ...
          <label htmlFor="dt-otherDocs-com-rent" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
