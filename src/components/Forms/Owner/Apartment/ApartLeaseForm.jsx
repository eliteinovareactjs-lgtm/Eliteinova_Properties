...
export default function ApartLeaseForm({ isOpen, onClose }) {
  ...
  const [formData, setFormData] = useState({
    ...
    propertyType: "Apartment", listingType: "", purpose: "Lease",
...
function MobContentApartLease({ step, inp, formData, updateForm, imagePreviews, handleImageUpload, removeImage, handleVideoUpload, videoPreview, removeVideo, handleDocumentUpload, toggleAmenity, toggleApartmentAmenity, availableAmenities, customAmenitiesList, addCustomAmenity, removeCustomAmenity, yesNoOptions, furnishingOptions, facingOptions, tenantTypeOptions, leaseDurationOptions, contactTimeOptions, apartmentLeaseAmenities }) {
      ...
      <Field label="Listing Type" required>
        {["Owner", "Agent", "Builder"].map(t => (
          <label key={t} className="flex items-center gap-2 text-[11px] mb-1 cursor-pointer">
            <input type="radio" name="mob-listing-lease" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.listingType === t} onChange={() => updateForm("listingType", t)} />
            {t}
          </label>
        ))}
      </Field>
      ...
      <Field label="Contact via">
        <div className="flex gap-4">
          {["Owner", "Agent", "Builder"].map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[11px] cursor-pointer">
              <input type="radio" name="mob-contact-lease" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.contactVia === opt} onChange={() => updateForm("contactVia", opt)} />
              {opt}
            </label>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-1">
          {contactTimeOptions.map(t => (
            <label key={t} className="flex items-center gap-1 text-[9px] cursor-pointer">
              <input type="radio" name="mob-time-lease" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.preferredContactTime === t} onChange={() => updateForm("preferredContactTime", t)} />
              {t}
            </label>
          ))}
        </div>
    ...
    <>
      ...
      <Field label="Property Images" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept="image/*" multiple className="hidden" id="m-imgs-apart-lease" onChange={handleImageUpload} />
          <label htmlFor="m-imgs-apart-lease" className="cursor-pointer flex flex-col items-center">
      ...
      <Field label="Property Video">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept="video/mp4,video/mov" className="hidden" id="m-vid-apart-lease" onChange={handleVideoUpload} />
          <label htmlFor="m-vid-apart-lease" className="cursor-pointer flex flex-col items-center">
  ...
  if (step === 4) return (
    <>
      ...
      {[
        { id: "m-own-apart-lease", e: "📄", t: "Upload Ownership Document", s: "Sale deed, registry (PDF/JPG/PNG)", l: "Ownership Proof", r: true, key: "ownershipDoc" },
        { id: "m-id-apart-lease", e: "🪪", t: "Upload ID Proof", s: "Aadhaar, PAN or Passport", l: "Owner ID Proof", r: true, key: "idProofDoc" },
        { id: "m-add-apart-lease", e: "📎", t: "+ Add More Documents", s: "Tax receipts, NOC, floor plans", l: "Additional Documents", r: false, key: "additionalDocs" }
      ].map(d => (
...
function DtContentApartLease({ step, inp, formData, updateForm, imagePreviews, handleImageUpload, removeImage, handleVideoUpload, videoPreview, removeVideo, handleDocumentUpload, toggleAmenity, toggleApartmentAmenity, availableAmenities, customAmenitiesList, addCustomAmenity, removeCustomAmenity, yesNoOptions, furnishingOptions, facingOptions, tenantTypeOptions, leaseDurationOptions, contactTimeOptions, apartmentLeaseAmenities }) {
      ...
      <FieldDt label="Listing Type" required>
        {["Owner", "Agent", "Builder"].map(t => (
          <label key={t} className="flex items-center gap-2 text-[13px] mb-2 cursor-pointer">
            <input type="radio" name="dt-listing-lease" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.listingType === t} onChange={() => updateForm("listingType", t)} />
            {t}
          </label>
        ))}
      </FieldDt>
    ...
    <>
      ...
      <FieldDt label="Contact via">
        <div className="flex gap-5">
          {["Owner", "Agent", "Builder"].map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-contact-lease" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.contactVia === opt} onChange={() => updateForm("contactVia", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Preferred Contact Time">
        <div className="grid grid-cols-2 gap-2">
          {contactTimeOptions.map(t => (
            <label key={t} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-time-lease" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.preferredContactTime === t} onChange={() => updateForm("preferredContactTime", t)} />
              {t}
            </label>
          ))}
        </div>
    ...
    <>
      ...
      <FieldDt label="Property Images" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept="image/*" multiple className="hidden" id="property-images-apart-lease" onChange={handleImageUpload} />
          <label htmlFor="property-images-apart-lease" className="cursor-pointer flex flex-col items-center">
      ...
      <FieldDt label="Property Video">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept="video/mp4,video/mov" className="hidden" id="property-video-apart-lease" onChange={handleVideoUpload} />
          <label htmlFor="property-video-apart-lease" className="cursor-pointer flex flex-col items-center">
  ...
  if (step === 4) return (
    <>
      ...
      {[
        { id: "ownership-proof-apart-lease", e: "📄", t: "Upload Ownership Document", s: "Sale deed, registry (PDF/JPG/PNG)", l: "Property Ownership Proof", r: true, key: "ownershipDoc" },
        { id: "id-proof-apart-lease", e: "🪪", t: "Upload ID Proof", s: "Aadhaar, PAN, or Passport", l: "Owner ID Proof", r: true, key: "idProofDoc" },
        { id: "additional-docs-apart-lease", e: "📎", t: "+ Add More Documents", s: "Tax receipts, NOC, floor plans", l: "Additional Documents", r: false, key: "additionalDocs" }
      ].map(d => (
