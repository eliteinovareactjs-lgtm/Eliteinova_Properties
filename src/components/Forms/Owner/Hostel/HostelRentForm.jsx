...
// Rent-specific options
const stayDurationOptions = ["1 Month", "3 Months", "6 Months", "9 Months", "11 Months", "12 Months"];
const rentFrequencyOptions = ["Monthly", "Quarterly", "Half-Yearly", "Yearly"];
const paymentModeOptions = ["Cash", "Bank Transfer", "UPI", "Cheque"];
...
export default function HostelRentForm({ isOpen, onClose }) {
  ...
  const [formData, setFormData] = useState({
    ...
    immediateOccupancy: "", minStayDuration: "",
    utilitiesIncluded: "", electricityIncluded: "", waterIncluded: "",
    // Media (Step 4)
  ...
  const validateStep = (s) => {
    ...
    if (s === 2) {
      ...
      if (!formData.propertyType) e.propertyType = "Please select a hostel type";
      if (!formData.hostelCategory) e.hostelCategory = "Please select a hostel category";
      ...
      if (!formData.minStayDuration) e.minStayDuration = "Please select minimum stay duration";
      if (!formData.rentFrequency) e.rentFrequency = "Please select rent frequency";
    }
    if (s === 3) {
      if (!formData.expectedPrice) e.expectedPrice = "Expected rent amount is required";
      else if (!isOnlyNumbers(formData.expectedPrice)) e.expectedPrice = "Expected rent amount must be a number";
      if (!formData.priceType) e.priceType = "Please select price type";
      if (formData.securityDeposit && !isOnlyNumbers(formData.securityDeposit)) e.securityDeposit = "Security deposit must be a number";
    ...
    if (s === 5) {
      ...
      if (!formData.hostelLicense) e.hostelLicense = "Hostel license is required";
      if (!formData.fireSafetyCertificate) e.fireSafetyCertificate = "Fire safety certificate is required";
    }
    if (s === 6) {
  ...
  const handleSubmit = () => {
    try {
      console.log("Hostel Rent Form submitted:", formData);
      onClose();
    } catch (err) {
      ...
      <div className="fixed inset-0 z-50 flex flex-col sm:hidden">
        ...
        <div className="flex-1 bg-white rounded-3xl flex flex-col overflow-hidden shadow-2xl mx-5 mb-5">
          <div className="relative flex flex-col items-center justify-center px-4 pt-3 pb-3 overflow-hidden shrink-0 rounded-t-3xl"
            ...
            <h1 className="text-[13px] font-extrabold text-white tracking-wide relative z-10 text-center">Rent Hostel</h1>
            <p className="text-[10px] text-white/80 relative z-10 mt-0.5 text-center">List your hostel for rent</p>
          </div>
          ...
          <div className="px-3 py-2.5 overflow-y-auto flex-1">
            <MobContentRent
              ...
              rentFrequencyOptions={rentFrequencyOptions}
      ...
      <div className="fixed inset-0 bg-black/60 z-50 hidden sm:flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
          <div className="relative flex flex-col items-center justify-center min-h-[65px] px-4 pt-2.5 pb-2.5 overflow-hidden shrink-0 rounded-3xl"
            ...
            <button onClick={onClose} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/25 hover:bg-white/40 flex items-center justify-center z-10 text-white font-bold text-[11px]">✕</button>
            <div className="text-xl mb-0.5 relative z-10">🏨</div>
            <h1 className="text-[14px] font-extrabold text-white tracking-wide relative z-10">Rent Hostel</h1>
            <p className="text-[10px] text-white/80 relative z-10 mt-0.5">List your hostel for rent</p>
          </div>
...
function MobContentRent({ 
  ...
  stayDurationOptions, rentFrequencyOptions, paymentModeOptions, bankOptions,
  ...
  if (step === 2) return (
    <>
      ...
      <Field label="Hostel Title / Name" required error={errors.propertyTitle}>
      ...
      <Field label="Hostel Type" required error={errors.propertyType}>
      ...
      <Field label="Bathroom Type" required error={errors.bathroomType}>
        <div className="flex gap-3">
      ...
      <Field label="Minimum Stay Duration" required error={errors.minStayDuration}>
        {stayDurationOptions.map(d => (
          <label key={d} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
            <input type="radio" name="mob-stay-duration" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.minStayDuration === d} onChange={() => updateForm("minStayDuration", d)} />
            {d}
          </label>
        ))}
      </Field>

      <Field label="Rent Frequency" required error={errors.rentFrequency}>
        {rentFrequencyOptions.map(rf => (
          <label key={rf} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
            <input type="radio" name="mob-rent-frequency" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.rentFrequency === rf} onChange={() => updateForm("rentFrequency", rf)} />
            {rf}
          </label>
        ))}
      </Field>
      ...
      <Field label="Payment Mode">
        {paymentModeOptions.map(pm => (
          <label key={pm} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
            <input type="radio" name="mob-payment-mode" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.paymentMode === pm} onChange={() => updateForm("paymentMode", pm)} />
            {pm}
          </label>
        ))}
      </Field>
      ...
      <Field label="Immediate Occupancy">
        <div className="flex gap-3">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-immediate" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.immediateOccupancy === option} onChange={() => updateForm("immediateOccupancy", option)} />
              {option}
            </label>
          ))}
        </div>
      ...
      <Field label="Utilities Included">
        <div className="flex gap-3">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-utilities" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.utilitiesIncluded === option} onChange={() => updateForm("utilitiesIncluded", option)} />
              {option}
            </label>
          ))}
        </div>
  ...
  if (step === 3) return (
    <>
      ...
      <Field label="Security Deposit (₹)" hint="Enter the refundable deposit amount">
        <input className={inp} type="text" inputMode="numeric" placeholder="e.g. 30,000" value={formData.securityDeposit} onChange={(e) => handleNumericFieldChange("securityDeposit", e.target.value)} />
      ...
      <Field label="Maintenance Included">
        <div className="flex gap-4">
          {yesNoOptions.map(option => (
  ...
  if (step === 5) return (
    <>
      ...
      <Field label="Rental Agreement">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-rentalAgreement" onChange={(e) => handleDocumentUpload("rentalAgreement", e)} />
          <label htmlFor="m-rentalAgreement" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
            <span className="text-[9px] text-[#00695C] font-semibold mt-0.5">Upload Rental Agreement</span>
            <span className="text-[9px] text-gray-400">PDF (Max 5MB)</span>
          </label>
        </div>
  ...
  if (step === 6) return (
    <>
      ...
      <p className="text-[9px] text-gray-400 mb-2">Enter your bank details for rent payments</p>
      ...
      <div className="space-y-2">
        <div>
          <label className="flex items-start gap-1.5 text-[10px] cursor-pointer">
            ...
            <span>I confirm that I am the legal owner or an authorized representative of this hostel property.</span>
          </label>
...
function DtContentRent({ 
  ...
  if (step === 2) return (
    <>
      ...
      <FieldDt label="Hostel Title / Name" required error={errors.propertyTitle}>
        <input className={inp} placeholder="e.g. Green Valley Hostel" value={formData.propertyTitle} onChange={(e) => updateForm("propertyTitle", e.target.value)} />
      </FieldDt>

      <FieldDt label="Hostel Type" required error={errors.propertyType}>
      ...
      <FieldDt label="Bathroom Type" required error={errors.bathroomType}>
        <div className="flex gap-4">
          {bathroomOptions.map(bt => (
      ...
      <FieldDt label="Minimum Stay Duration" required error={errors.minStayDuration}>
        {stayDurationOptions.map(d => (
          <label key={d} className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-stay-duration" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.minStayDuration === d} onChange={() => updateForm("minStayDuration", d)} />
            {d}
          </label>
        ))}
      </FieldDt>

      <FieldDt label="Rent Frequency" required error={errors.rentFrequency}>
        {rentFrequencyOptions.map(rf => (
          <label key={rf} className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-rent-frequency" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.rentFrequency === rf} onChange={() => updateForm("rentFrequency", rf)} />
            {rf}
          </label>
        ))}
      </FieldDt>

      <FieldDt label="Payment Mode">
        {paymentModeOptions.map(pm => (
          <label key={pm} className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-payment-mode" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.paymentMode === pm} onChange={() => updateForm("paymentMode", pm)} />
            {pm}
          </label>
        ))}
      </FieldDt>
      ...
      <FieldDt label="Immediate Occupancy">
        <div className="flex gap-4">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-immediate" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.immediateOccupancy === option} onChange={() => updateForm("immediateOccupancy", option)} />
              {option}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Utilities Included">
        <div className="flex gap-4">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-utilities" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.utilitiesIncluded === option} onChange={() => updateForm("utilitiesIncluded", option)} />
              {option}
            </label>
          ))}
        </div>
  ...
  if (step === 3) return (
    <>
      ...
      <FieldDt label="Maintenance Included">
        <div className="flex gap-5">
          {yesNoOptions.map(option => (
            <label key={option} className="flex items-center gap-2 text-[13px] cursor-pointer">
      ...
      <FieldDt label="Other Amenities">
        <div className="flex gap-2">
          <input className={inp} placeholder="e.g. Play Area, Study Room..." value={formData.otherAmenities} onChange={(e) => updateForm("otherAmenities", e.target.value)} onKeyPress={(e) => e.key === 'Enter' && addCustomAmenity()} />
  ...
  if (step === 5) return (
    <>
      ...
      <FieldDt label="Rental Agreement">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-rentalAgreement" onChange={(e) => handleDocumentUpload("rentalAgreement", e)} />
          <label htmlFor="dt-rentalAgreement" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
            <span className="text-[10px] text-[#00695C] font-semibold mt-0.5">Upload Rental Agreement</span>
            <span className="text-[10px] text-gray-400">PDF (Max 5MB)</span>
          </label>
        </div>
  ...
  if (step === 6) return (
    <>
      ...
      <p className="text-[11px] text-gray-400 mb-3">Enter your bank details for rent payments</p>
