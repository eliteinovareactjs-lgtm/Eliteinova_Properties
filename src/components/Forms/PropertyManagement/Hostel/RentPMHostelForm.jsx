...
export default function RentPMHostelForm({ isOpen, onClose }) {
  ...
  const validateStep = (stepNumber) => {
    ...
    switch(stepNumber) {
      ...
      case 5: // Pricing & Amenities
        ...
        const durationValidation = validateField(formData.rentDuration, { required: true });
        if (!durationValidation.valid) { newErrors.rentDuration = durationValidation.message; isValid = false; }
        break;
  ...
  const handleSubmit = () => {
    if (validateStep(step)) {
      ...
      console.log("Rent PM Hostel Form submitted:", formData);
      onClose();
    }
  };
      ...
      <div className="fixed inset-0 z-50 flex flex-col sm:hidden">
        ...
        <div className="flex-1 bg-white rounded-3xl flex flex-col overflow-hidden shadow-2xl mx-5 mb-5">
          <div className="relative flex flex-col items-center justify-center px-4 pt-3 pb-3 overflow-hidden shrink-0 rounded-t-3xl"
            ...
            <h1 className="text-[13px] font-extrabold text-white tracking-wide relative z-10 text-center">Rent Hostel - Property Management</h1>
            <p className="text-[10px] text-white/80 relative z-10 mt-0.5 text-center">List hostel for rent as property management</p>
          </div>
      ...
      <div className="fixed inset-0 bg-black/60 z-50 hidden sm:flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
          <div className="relative flex flex-col items-center justify-center min-h-[65px] px-4 pt-2.5 pb-2.5 overflow-hidden shrink-0 rounded-3xl"
            ...
            <button onClick={onClose} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/25 hover:bg-white/40 flex items-center justify-center z-10 text-white font-bold text-[11px]">✕</button>
            <div className="text-xl mb-0.5 relative z-10">🏢</div>
            <h1 className="text-[14px] font-extrabold text-white tracking-wide relative z-10">Rent Hostel - Property Management</h1>
            <p className="text-[10px] text-white/80 relative z-10 mt-0.5">List hostel for rent as property management</p>
          </div>
...
// ==================== MOBILE CONTENT - Rent PM Hostel ====================
function MobContentRentPMHostel({ 
  ...
  serviceAreaOptions, bankOptions, ownershipTypeOptions,
  ...
  if (step === 4) return (
    <>
      ...
      <Field label="Property Age">
        <input className={inp} type="text" placeholder="Enter property age in years" value={formData.propertyAge} onChange={(e) => updateForm("propertyAge", handleNumericFieldChange(e.target.value))} />
      </Field>
  ...
  if (step === 5) return (
    <>
      ...
      <Field label="Rent Amount (₹/month)" required error={errors.rentAmount}>
        <input className={`${inp} ${getErrorClass('rentAmount')}`} type="text" placeholder="Enter monthly rent amount" value={formData.rentAmount} onChange={(e) => updateForm("rentAmount", handleNumericFieldChange(e.target.value))} />
      </Field>
        ...
        <div className="grid grid-cols-2 gap-1">
          {rentDurationOptions.map(d => (
            <label key={d} className="flex items-center gap-1 text-[10px] cursor-pointer">
              <input type="radio" name="mob-duration-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.rentDuration === d} onChange={() => updateForm("rentDuration", d)} />
              {d}
            </label>
          ))}
        </div>
      ...
      <Field label="Rent Renewal Option">
        <div className="flex gap-4">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[11px] cursor-pointer">
              <input type="radio" name="mob-renewal-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.rentRenewalOption === opt} onChange={() => updateForm("rentRenewalOption", opt)} />
              {opt}
            </label>
          ))}
        </div>
...
// ==================== DESKTOP CONTENT - Rent PM Hostel ====================
function DtContentRentPMHostel({ 
  step, inp, errorBorder, formData, errors, updateForm, 
  imagePreviews, handleImageUpload, removeImage,
  handleVideoUpload, videoPreview, removeVideo,
  handleDocumentUpload, toggleAmenity,
    ...
    <>
        ...
        <div className="flex flex-wrap gap-3">
          {rentDurationOptions.map(d => (
            <label key={d} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-duration-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.rentDuration === d} onChange={() => updateForm("rentDuration", d)} />
              {d}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Rent Renewal Option">
        <div className="flex gap-5">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-renewal-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.rentRenewalOption === opt} onChange={() => updateForm("rentRenewalOption", opt)} />
              {opt}
            </label>
          ))}
        </div>
