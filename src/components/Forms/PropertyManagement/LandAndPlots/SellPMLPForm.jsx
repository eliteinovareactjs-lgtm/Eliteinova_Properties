...
export default function SellPMLPForm({ isOpen, onClose }) {
  ...
  const [formData, setFormData] = useState({
    ...
    expectedPrice: "", budgetRange: { min: "", max: "" }, priceType: "",
    ...
    switch(stepNumber) {
      ...
      case 4: // Land Details
        ...
        if (!formData.landCity.trim()) {
          ...
          newErrors.landCity = "Only letters and spaces allowed";
          isValid = false;
        }
      ...
      case 9: // Declaration
        if (!formData.declarationAuthorized) {
          newErrors.declarationAuthorized = "You must confirm that you are the authorized representative";
          isValid = false;
        }
        if (!formData.declarationAccurate) {
          newErrors.declarationAccurate = "You must certify that all information is accurate";
          isValid = false;
        }
        ...
        if (!formData.declarationTerms) {
          newErrors.declarationTerms = "You must agree to the Terms & Conditions";
          isValid = false;
        }
  ...
  const handleSubmit = () => {
    if (validateStep(step)) {
      ...
      console.log("Sell Land PM Form submitted:", formData);
      onClose();
    }
  };
      ...
      <div className="fixed inset-0 z-50 flex flex-col sm:hidden">
        ...
        <div className="flex-1 bg-white rounded-3xl flex flex-col overflow-hidden shadow-2xl mx-5 mb-5">
          <div className="relative flex flex-col items-center justify-center px-4 pt-3 pb-3 overflow-hidden shrink-0 rounded-t-3xl"
            ...
            <h1 className="text-[13px] font-extrabold text-white tracking-wide relative z-10 text-center">Land Sell - Property Management</h1>
      ...
      <div className="fixed inset-0 bg-black/60 z-50 hidden sm:flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
          <div className="relative flex flex-col items-center justify-center min-h-[65px] px-4 pt-2.5 pb-2.5 overflow-hidden shrink-0 rounded-3xl"
            ...
            <button onClick={onClose} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/25 hover:bg-white/40 flex items-center justify-center z-10 text-white font-bold text-[11px]">✕</button>
            <div className="text-xl mb-0.5 relative z-10">🏞️</div>
            <h1 className="text-[14px] font-extrabold text-white tracking-wide relative z-10">Land Sell - Property Management</h1>
...
// ==================== MOBILE CONTENT - SELL PM LAND/PLOT ====================
function MobContentSellPMLP({ 
  ...
  if (step === 0) return (
    <>
      <Field label="Property Management Company Name" required error={errors.pmCompanyName}>
        <input className={`${inp} ${errors.pmCompanyName ? errorBorder : ''}`} placeholder="Enter company name" value={formData.pmCompanyName} onChange={(e) => handleAlphaNumericChange("pmCompanyName", e.target.value)} />
      </Field>
      <Field label="Business Registration Number" required error={errors.pmBusinessRegNumber}>
        <input className={`${inp} ${errors.pmBusinessRegNumber ? errorBorder : ''}`} placeholder="Enter registration number" value={formData.pmBusinessRegNumber} onChange={(e) => handleAlphaNumericChange("pmBusinessRegNumber", e.target.value)} />
      </Field>
      ...
      <Field label="Company Logo" required hint="JPG, PNG max 2MB" error={errors.pmCompanyLogo}>
        <div className={`border-2 border-dashed ${errors.pmCompanyLogo ? 'border-red-500' : 'border-teal-300'} rounded-xl p-2.5 text-center hover:bg-green-50`}>
          <input type="file" accept="image/*" className="hidden" id="m-pm-logo-slp" onChange={handleCompanyLogoUpload} />
          <label htmlFor="m-pm-logo-slp" className="cursor-pointer flex flex-col items-center">
  ...
  if (step === 1) return (
    <>
      ...
      <Field label="Full Name" required error={errors.authFullName}>
        <input className={`${inp} ${errors.authFullName ? errorBorder : ''}`} placeholder="Enter authorized representative's full name" value={formData.authFullName} onChange={(e) => handleAlphaChange("authFullName", e.target.value)} />
      </Field>
      <Field label="Designation" required error={errors.authDesignation}>
        <input className={`${inp} ${errors.authDesignation ? errorBorder : ''}`} placeholder="e.g. Director, Manager" value={formData.authDesignation} onChange={(e) => handleAlphaChange("authDesignation", e.target.value)} />
      </Field>
      <Field label="Mobile Number" required error={errors.authMobile}>
        <input className={`${inp} ${errors.authMobile ? errorBorder : ''}`} type="tel" placeholder="Enter 10-digit mobile number" value={formData.authMobile} onChange={(e) => handleNumericChange("authMobile", e.target.value)} maxLength="10" />
      </Field>
  ...
  if (step === 2) return (
    <>
      ...
      <Field label="City" required error={errors.officeCity}>
        <input className={`${inp} ${errors.officeCity ? errorBorder : ''}`} placeholder="Enter city" value={formData.officeCity} onChange={(e) => handleAlphaChange("officeCity", e.target.value)} />
      </Field>
      <Field label="District" required error={errors.officeDistrict}>
        <input className={`${inp} ${errors.officeDistrict ? errorBorder : ''}`} placeholder="Enter district" value={formData.officeDistrict} onChange={(e) => handleAlphaChange("officeDistrict", e.target.value)} />
      </Field>
      <Field label="State" required error={errors.officeState}>
        <input className={`${inp} ${errors.officeState ? errorBorder : ''}`} placeholder="Enter state" value={formData.officeState} onChange={(e) => handleAlphaChange("officeState", e.target.value)} />
      </Field>
      <Field label="PIN Code" required error={errors.officePinCode}>
        <input className={`${inp} ${errors.officePinCode ? errorBorder : ''}`} type="tel" placeholder="Enter 6-digit PIN code" value={formData.officePinCode} onChange={(e) => handleNumericChange("officePinCode", e.target.value)} maxLength="6" />
      </Field>
  ...
  if (step === 3) return (
    <>
      ...
      <Field label="Aadhaar Number" required error={errors.aadhaarNumber}>
        <input className={`${inp} ${errors.aadhaarNumber ? errorBorder : ''}`} type="tel" placeholder="Enter 12-digit Aadhaar number" value={formData.aadhaarNumber} onChange={(e) => handleNumericChange("aadhaarNumber", e.target.value)} maxLength="12" />
      </Field>
      <Field label="PAN Number" required error={errors.panNumber}>
        <input className={`${inp} ${errors.panNumber ? errorBorder : ''}`} placeholder="Enter 10-character PAN number" value={formData.panNumber} onChange={(e) => updateForm("panNumber", e.target.value.toUpperCase())} maxLength="10" />
      </Field>
      ...
      <Field label="Upload Aadhaar Card" required error={errors.aadhaarCard}>
        <div className={`border-2 border-dashed ${errors.aadhaarCard ? 'border-red-500' : 'border-teal-300'} rounded-xl p-2.5 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="m-aadhaar-slp" onChange={(e) => handleDocumentUpload("aadhaarCard", e)} />
          <label htmlFor="m-aadhaar-slp" className="cursor-pointer flex flex-col items-center">
      ...
      <Field label="Upload PAN Card" required error={errors.panCard}>
        <div className={`border-2 border-dashed ${errors.panCard ? 'border-red-500' : 'border-teal-300'} rounded-xl p-2.5 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="m-pan-slp" onChange={(e) => handleDocumentUpload("panCard", e)} />
          <label htmlFor="m-pan-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
      ...
      <Field label="Upload Business Registration Certificate" required error={errors.pmBusinessRegCert}>
        <div className={`border-2 border-dashed ${errors.pmBusinessRegCert ? 'border-red-500' : 'border-teal-300'} rounded-xl p-2.5 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="m-pm-reg-slp" onChange={(e) => handleDocumentUpload("pmBusinessRegCert", e)} />
          <label htmlFor="m-pm-reg-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
      ...
      <Field label="Upload GST Certificate (Optional)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-pm-gst-slp" onChange={(e) => handleDocumentUpload("pmGstCert", e)} />
          <label htmlFor="m-pm-gst-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
      ...
      <Field label="Upload RERA Certificate (Optional)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-pm-rera-slp" onChange={(e) => handleDocumentUpload("pmReraCert", e)} />
          <label htmlFor="m-pm-rera-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-6 h-6 text-[#00695C]" />
  ...
  if (step === 4) return (
    <>
      ...
      <Field label="Land City" required error={errors.landCity}>
        <input className={`${inp} ${errors.landCity ? errorBorder : ''}`} placeholder="Enter land city name" value={formData.landCity} onChange={(e) => handleAlphaChange("landCity", e.target.value)} />
      </Field>
        ...
        <div className="flex gap-4">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[11px] cursor-pointer">
              <input type="radio" name="mob-electricity-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.electricityAvailable === opt} onChange={() => updateForm("electricityAvailable", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <Field label="Property Condition">
        {conditionOptions.map(c => (
          <label key={c} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
            <input type="radio" name="mob-condition-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.propertyCondition === c} onChange={() => updateForm("propertyCondition", c)} />
            {c}
          </label>
        ))}
      </Field>
      <Field label="Ownership Type">
        {ownershipOptions.map(o => (
          <label key={o} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
            <input type="radio" name="mob-ownership-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.ownershipType === o} onChange={() => updateForm("ownershipType", o)} />
            {o}
          </label>
        ))}
      </Field>
        ...
        <div className="flex gap-2">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-loan-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.loanOutstanding === opt} onChange={() => updateForm("loanOutstanding", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <Field label="Garden Space">
        <div className="flex gap-2">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-garden-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.gardenSpace === opt} onChange={() => updateForm("gardenSpace", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <Field label="Terrace / Balcony">
        <div className="flex gap-2">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[10px] cursor-pointer">
              <input type="radio" name="mob-terrace-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.terrace === opt} onChange={() => updateForm("terrace", opt)} />
              {opt}
            </label>
          ))}
        </div>
  ...
  if (step === 5) return (
    <>
      ...
      <Field label="Expected Price (₹)" required error={errors.expectedPrice}>
        <input className={`${inp} ${errors.expectedPrice ? errorBorder : ''}`} type="number" min="0" placeholder="e.g. 45,00,000" value={formData.expectedPrice} onChange={(e) => updateForm("expectedPrice", e.target.value)} />
      </Field>
      ...
      <Field label="Price Type" required error={errors.priceType}>
        <div className="flex gap-4">
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-priceType-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "fixed"} onChange={() => updateForm("priceType", "fixed")} />
            Fixed
          </label>
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-priceType-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "negotiable"} onChange={() => updateForm("priceType", "negotiable")} />
            Negotiable
          </label>
        </div>
      </Field>
  ...
  if (step === 6) return (
    <>
      ...
      <Field label="Account Holder Name" required error={errors.accountHolderName}>
        <input className={`${inp} ${errors.accountHolderName ? errorBorder : ''}`} placeholder="Enter account holder name" value={formData.accountHolderName} onChange={(e) => handleAlphaChange("accountHolderName", e.target.value)} />
      </Field>
      ...
      <Field label="Account Number" required error={errors.accountNumber}>
        <input className={`${inp} ${errors.accountNumber ? errorBorder : ''}`} type="tel" placeholder="Enter account number (9-18 digits)" value={formData.accountNumber} onChange={(e) => handleNumericChange("accountNumber", e.target.value)} maxLength="18" />
      </Field>
      <Field label="IFSC Code" required error={errors.ifscCode}>
        <input className={`${inp} ${errors.ifscCode ? errorBorder : ''}`} placeholder="Enter IFSC code (e.g. SBIN0123456)" value={formData.ifscCode} onChange={(e) => updateForm("ifscCode", e.target.value.toUpperCase())} maxLength="11" />
      </Field>
      <Field label="UPI ID">
    ...
    <>
      ...
      <Field label="Company Logo" required hint="JPG, PNG max 2MB" error={errors.pmCompanyLogoDoc}>
        <div className={`border-2 border-dashed ${errors.pmCompanyLogoDoc ? 'border-red-500' : 'border-teal-300'} rounded-xl p-2.5 text-center hover:bg-green-50`}>
          <input type="file" accept="image/*" className="hidden" id="m-pm-logo-doc-slp" onChange={(e) => handleDocumentUpload("pmCompanyLogoDoc", e, 2)} />
          <label htmlFor="m-pm-logo-doc-slp" className="cursor-pointer flex flex-col items-center">
            <ImagePlus className="w-5 h-5 text-[#00695C]" />
            <span className="text-[10px] font-semibold text-[#00695C]">Upload Logo</span>
            <span className="text-[9px] text-gray-400">JPG/PNG (Max 2MB)</span>
          </label>
      ...
      <Field label="Business Registration Certificate" required error={errors.pmBusinessRegCertDoc}>
        <div className={`border-2 border-dashed ${errors.pmBusinessRegCertDoc ? 'border-red-500' : 'border-teal-300'} rounded-xl p-2.5 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="m-pm-reg-doc-slp" onChange={(e) => handleDocumentUpload("pmBusinessRegCertDoc", e)} />
          <label htmlFor="m-pm-reg-doc-slp" className="cursor-pointer flex flex-col items-center">
      ...
      <Field label="Aadhaar Card" required error={errors.aadhaarCardDoc}>
        <div className={`border-2 border-dashed ${errors.aadhaarCardDoc ? 'border-red-500' : 'border-teal-300'} rounded-xl p-2.5 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="m-aadhaar-doc-slp" onChange={(e) => handleDocumentUpload("aadhaarCardDoc", e)} />
          <label htmlFor="m-aadhaar-doc-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
            <span className="text-[10px] font-semibold text-[#00695C]">Upload Aadhaar</span>
            <span className="text-[9px] text-gray-400">PDF (Max 2MB)</span>
          </label>
      ...
      <Field label="GST Certificate (Optional)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-pm-gst-doc-slp" onChange={(e) => handleDocumentUpload("pmGstCertDoc", e)} />
          <label htmlFor="m-pm-gst-doc-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
            <span className="text-[10px] font-semibold text-[#00695C]">Upload GST</span>
            <span className="text-[9px] text-gray-400">PDF (Max 5MB)</span>
          </label>
      ...
      <Field label="RERA Certificate (Optional)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-pm-rera-doc-slp" onChange={(e) => handleDocumentUpload("pmReraCertDoc", e)} />
          <label htmlFor="m-pm-rera-doc-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
            <span className="text-[10px] font-semibold text-[#00695C]">Upload RERA</span>
            <span className="text-[9px] text-gray-400">PDF (Max 5MB)</span>
          </label>
      ...
      <Field label="Office Address Proof" required error={errors.officeAddressProofDoc}>
        <div className={`border-2 border-dashed ${errors.officeAddressProofDoc ? 'border-red-500' : 'border-teal-300'} rounded-xl p-2.5 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="m-office-proof-doc-slp" onChange={(e) => handleDocumentUpload("officeAddressProofDoc", e)} />
          <label htmlFor="m-office-proof-doc-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
            <span className="text-[10px] font-semibold text-[#00695C]">Upload Address Proof</span>
            <span className="text-[9px] text-gray-400">PDF (Max 5MB)</span>
          </label>
      ...
      <Field label="Company Profile/Brochure (Optional)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="m-pm-brochure-slp" onChange={(e) => handleDocumentUpload("pmCompanyBrochure", e)} />
          <label htmlFor="m-pm-brochure-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-5 h-5 text-[#00695C]" />
      ...
      <Field label="Upload Cover Image" required hint="Max 2MB" error={errors.coverImage}>
        <div className={`border-2 border-dashed ${errors.coverImage ? 'border-red-500' : 'border-teal-300'} rounded-xl p-2.5 text-center hover:bg-green-50`}>
          <input type="file" accept="image/*" className="hidden" id="m-cover-slp-pm" onChange={handleCoverImageUpload} />
          <label htmlFor="m-cover-slp-pm" className="cursor-pointer flex flex-col items-center">
            <ImagePlus className="w-5 h-5 text-[#00695C]" />
      ...
      <Field label="Upload Land Photos (Max 3)" required hint={`${formData.propertyImages.length}/3 images uploaded`} error={errors.propertyImages}>
        <div className={`border-2 border-dashed ${errors.propertyImages ? 'border-red-500' : 'border-teal-300'} rounded-xl p-2.5 text-center hover:bg-green-50`}>
          <input type="file" accept="image/*" multiple className="hidden" id="m-imgs-slp-pm" onChange={handleImageUpload} disabled={formData.propertyImages.length >= 3} />
          <label htmlFor="m-imgs-slp-pm" className={`cursor-pointer flex flex-col items-center ${formData.propertyImages.length >= 3 ? 'opacity-50 cursor-not-allowed' : ''}`}>
            <ImagePlus className="w-5 h-5 text-[#00695C]" />
      ...
      <Field label="Upload Land Video (Optional)" hint="Max 10MB">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-2.5 text-center hover:bg-green-50">
          <input type="file" accept="video/mp4,video/mov" className="hidden" id="m-vid-slp-pm" onChange={handleVideoUpload} />
          <label htmlFor="m-vid-slp-pm" className="cursor-pointer flex flex-col items-center">
      ...
      <Field label="Upload Floor Plan" required hint="PDF only (Max 5MB)" error={errors.floorPlan}>
        <div className={`border-2 border-dashed ${errors.floorPlan ? 'border-red-500' : 'border-teal-300'} rounded-xl p-2.5 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="m-floorplan-slp-pm" onChange={handleFloorPlanUpload} />
          <label htmlFor="m-floorplan-slp-pm" className="cursor-pointer flex flex-col items-center">
...
// ==================== DESKTOP CONTENT - SELL PM LAND/PLOT ====================
function DtContentSellPMLP({ 
  step, inp, errorBorder, formData, errors, updateForm, 
  imagePreviews, handleImageUpload, removeImage, 
  handleVideoUpload, videoPreview, removeVideo, 
  handleDocumentUpload, toggleFeature,
    ...
    <>
      ...
      <FieldDt label="Company Logo" required hint="JPG, PNG max 2MB" error={errors.pmCompanyLogo}>
        <div className={`border-2 border-dashed ${errors.pmCompanyLogo ? 'border-red-500' : 'border-teal-300'} rounded-xl p-3 text-center hover:bg-green-50`}>
          <input type="file" accept="image/*" className="hidden" id="dt-pm-logo-slp" onChange={handleCompanyLogoUpload} />
          <label htmlFor="dt-pm-logo-slp" className="cursor-pointer flex flex-col items-center">
    ...
    <>
      ...
      <FieldDt label="Upload Aadhaar Card" required error={errors.aadhaarCard}>
        <div className={`border-2 border-dashed ${errors.aadhaarCard ? 'border-red-500' : 'border-teal-300'} rounded-xl p-3 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="dt-aadhaar-slp" onChange={(e) => handleDocumentUpload("aadhaarCard", e)} />
          <label htmlFor="dt-aadhaar-slp" className="cursor-pointer flex flex-col items-center">
      ...
      <FieldDt label="Upload PAN Card" required error={errors.panCard}>
        <div className={`border-2 border-dashed ${errors.panCard ? 'border-red-500' : 'border-teal-300'} rounded-xl p-3 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="dt-pan-slp" onChange={(e) => handleDocumentUpload("panCard", e)} />
          <label htmlFor="dt-pan-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-7 h-7 text-[#00695C]" />
      ...
      <FieldDt label="Upload Business Registration Certificate" required error={errors.pmBusinessRegCert}>
        <div className={`border-2 border-dashed ${errors.pmBusinessRegCert ? 'border-red-500' : 'border-teal-300'} rounded-xl p-3 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="dt-pm-reg-slp" onChange={(e) => handleDocumentUpload("pmBusinessRegCert", e)} />
          <label htmlFor="dt-pm-reg-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-7 h-7 text-[#00695C]" />
      ...
      <FieldDt label="Upload GST Certificate (Optional)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-pm-gst-slp" onChange={(e) => handleDocumentUpload("pmGstCert", e)} />
          <label htmlFor="dt-pm-gst-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-7 h-7 text-[#00695C]" />
      ...
      <FieldDt label="Upload RERA Certificate (Optional)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-pm-rera-slp" onChange={(e) => handleDocumentUpload("pmReraCert", e)} />
          <label htmlFor="dt-pm-rera-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-7 h-7 text-[#00695C]" />
    ...
    <>
        ...
        <div className="flex gap-5">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-electricity-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.electricityAvailable === opt} onChange={() => updateForm("electricityAvailable", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Property Condition">
        {conditionOptions.map(c => (
          <label key={c} className="flex items-center gap-2 text-[13px] mb-1.5 cursor-pointer">
            <input type="radio" name="dt-condition-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.propertyCondition === c} onChange={() => updateForm("propertyCondition", c)} />
            {c}
          </label>
        ))}
      </FieldDt>
      <FieldDt label="Ownership Type">
        {ownershipOptions.map(o => (
          <label key={o} className="flex items-center gap-2 text-[13px] mb-1.5 cursor-pointer">
            <input type="radio" name="dt-ownership-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.ownershipType === o} onChange={() => updateForm("ownershipType", o)} />
            {o}
          </label>
        ))}
      </FieldDt>
      <FieldDt label="Loan Outstanding">
        <div className="flex gap-5">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-loan-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.loanOutstanding === opt} onChange={() => updateForm("loanOutstanding", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Garden Space">
        <div className="flex gap-5">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-garden-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.gardenSpace === opt} onChange={() => updateForm("gardenSpace", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Terrace / Balcony">
        <div className="flex gap-5">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-terrace-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.terrace === opt} onChange={() => updateForm("terrace", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Price Type" required error={errors.priceType}>
        <div className="flex gap-5">
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-priceType-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "fixed"} onChange={() => updateForm("priceType", "fixed")} />
            Fixed
          </label>
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-priceType-slp-pm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "negotiable"} onChange={() => updateForm("priceType", "negotiable")} />
            Negotiable
          </label>
        </div>
      </FieldDt>
    ...
    <>
      ...
      <FieldDt label="Company Logo" required hint="JPG, PNG max 2MB" error={errors.pmCompanyLogoDoc}>
        <div className={`border-2 border-dashed ${errors.pmCompanyLogoDoc ? 'border-red-500' : 'border-teal-300'} rounded-xl p-3 text-center hover:bg-green-50`}>
          <input type="file" accept="image/*" className="hidden" id="dt-pm-logo-doc-slp" onChange={(e) => handleDocumentUpload("pmCompanyLogoDoc", e, 2)} />
          <label htmlFor="dt-pm-logo-doc-slp" className="cursor-pointer flex flex-col items-center">
            <ImagePlus className="w-7 h-7 text-[#00695C]" />
      ...
      <FieldDt label="Business Registration Certificate" required error={errors.pmBusinessRegCertDoc}>
        <div className={`border-2 border-dashed ${errors.pmBusinessRegCertDoc ? 'border-red-500' : 'border-teal-300'} rounded-xl p-3 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="dt-pm-reg-doc-slp" onChange={(e) => handleDocumentUpload("pmBusinessRegCertDoc", e)} />
          <label htmlFor="dt-pm-reg-doc-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-7 h-7 text-[#00695C]" />
            <span className="text-[12px] font-semibold text-[#00695C] mt-1">Upload Registration Certificate</span>
            <span className="text-[11px] text-gray-400">PDF (Max 5MB)</span>
          </label>
      ...
      <FieldDt label="Aadhaar Card" required error={errors.aadhaarCardDoc}>
        <div className={`border-2 border-dashed ${errors.aadhaarCardDoc ? 'border-red-500' : 'border-teal-300'} rounded-xl p-3 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="dt-aadhaar-doc-slp" onChange={(e) => handleDocumentUpload("aadhaarCardDoc", e)} />
          <label htmlFor="dt-aadhaar-doc-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-7 h-7 text-[#00695C]" />
            <span className="text-[12px] font-semibold text-[#00695C] mt-1">Upload Aadhaar Card</span>
            <span className="text-[11px] text-gray-400">PDF (Max 2MB)</span>
          </label>
      ...
      <FieldDt label="GST Certificate (Optional)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-pm-gst-doc-slp" onChange={(e) => handleDocumentUpload("pmGstCertDoc", e)} />
          <label htmlFor="dt-pm-gst-doc-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-7 h-7 text-[#00695C]" />
            <span className="text-[12px] font-semibold text-[#00695C] mt-1">Upload GST Certificate</span>
            <span className="text-[11px] text-gray-400">PDF (Max 5MB)</span>
          </label>
      ...
      <FieldDt label="RERA Certificate (Optional)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-pm-rera-doc-slp" onChange={(e) => handleDocumentUpload("pmReraCertDoc", e)} />
          <label htmlFor="dt-pm-rera-doc-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-7 h-7 text-[#00695C]" />
            <span className="text-[12px] font-semibold text-[#00695C] mt-1">Upload RERA Certificate</span>
            <span className="text-[11px] text-gray-400">PDF (Max 5MB)</span>
          </label>
      ...
      <FieldDt label="Office Address Proof" required error={errors.officeAddressProofDoc}>
        <div className={`border-2 border-dashed ${errors.officeAddressProofDoc ? 'border-red-500' : 'border-teal-300'} rounded-xl p-3 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="dt-office-proof-doc-slp" onChange={(e) => handleDocumentUpload("officeAddressProofDoc", e)} />
          <label htmlFor="dt-office-proof-doc-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-7 h-7 text-[#00695C]" />
            <span className="text-[12px] font-semibold text-[#00695C] mt-1">Upload Address Proof</span>
            <span className="text-[11px] text-gray-400">PDF (Max 5MB)</span>
          </label>
      ...
      <FieldDt label="Company Profile/Brochure (Optional)">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf" className="hidden" id="dt-pm-brochure-slp" onChange={(e) => handleDocumentUpload("pmCompanyBrochure", e)} />
          <label htmlFor="dt-pm-brochure-slp" className="cursor-pointer flex flex-col items-center">
            <FileText className="w-7 h-7 text-[#00695C]" />
      ...
      <FieldDt label="Upload Cover Image" required hint="Max 2MB" error={errors.coverImage}>
        <div className={`border-2 border-dashed ${errors.coverImage ? 'border-red-500' : 'border-teal-300'} rounded-xl p-3 text-center hover:bg-green-50`}>
          <input type="file" accept="image/*" className="hidden" id="dt-cover-slp-pm" onChange={handleCoverImageUpload} />
          <label htmlFor="dt-cover-slp-pm" className="cursor-pointer flex flex-col items-center">
            <ImagePlus className="w-7 h-7 text-[#00695C]" />
      ...
      <FieldDt label="Upload Land Photos (Max 3)" required hint={`${formData.propertyImages.length}/3 images uploaded`} error={errors.propertyImages}>
        <div className={`border-2 border-dashed ${errors.propertyImages ? 'border-red-500' : 'border-teal-300'} rounded-xl p-3 text-center hover:bg-green-50`}>
          <input type="file" accept="image/*" multiple className="hidden" id="dt-imgs-slp-pm" onChange={handleImageUpload} disabled={formData.propertyImages.length >= 3} />
          <label htmlFor="dt-imgs-slp-pm" className={`cursor-pointer flex flex-col items-center ${formData.propertyImages.length >= 3 ? 'opacity-50 cursor-not-allowed' : ''}`}>
            <ImagePlus className="w-7 h-7 text-[#00695C]" />
      ...
      <FieldDt label="Upload Land Video (Optional)" hint="Max 10MB">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept="video/mp4,video/mov" className="hidden" id="dt-vid-slp-pm" onChange={handleVideoUpload} />
          <label htmlFor="dt-vid-slp-pm" className="cursor-pointer flex flex-col items-center">
      ...
      <FieldDt label="Upload Floor Plan" required hint="PDF only (Max 5MB)" error={errors.floorPlan}>
        <div className={`border-2 border-dashed ${errors.floorPlan ? 'border-red-500' : 'border-teal-300'} rounded-xl p-3 text-center hover:bg-green-50`}>
          <input type="file" accept=".pdf" className="hidden" id="dt-floorplan-slp-pm" onChange={handleFloorPlanUpload} />
          <label htmlFor="dt-floorplan-slp-pm" className="cursor-pointer flex flex-col items-center">
    ...
    <>
      ...
      <div className="space-y-2.5">
        <div>
          ...
          {errors.declarationAuthorized && <p className="text-[10px] text-red-500 ml-6">{errors.declarationAuthorized}</p>}
        </div>
        <div>
          <label className="flex items-start gap-2.5 text-[13px] cursor-pointer">
          ...
          {errors.declarationAccurate && <p className="text-[10px] text-red-500 ml-6">{errors.declarationAccurate}</p>}
        </div>
        <div>
          <label className="flex items-start gap-2.5 text-[13px] cursor-pointer">
          ...
          {errors.declarationAuthorization && <p className="text-[10px] text-red-500 ml-6">{errors.declarationAuthorization}</p>}
        </div>
        <div>
          <label className="flex items-start gap-2.5 text-[13px] cursor-pointer">
          ...
          {errors.declarationTerms && <p className="text-[10px] text-red-500 ml-6">{errors.declarationTerms}</p>}
        </div>
      </div>
    </>
  );
