...
export default function RentPMApartForm({ isOpen, onClose }) {
  ...
  const [formData, setFormData] = useState({
    ...
    rent: "", budgetRange: { min: "", max: "" }, securityDeposit: "",
    maintenanceIncluded: "", priceType: "",
  ...
  const handleSubmit = () => {
    if (validateStep(step)) {
      ...
      console.log("Apartment Rent PM Form submitted:", formData);
      onClose();
    }
  };
      ...
      <div className="fixed inset-0 z-50 flex flex-col sm:hidden">
        ...
        <div className="flex-1 bg-white rounded-3xl flex flex-col overflow-hidden shadow-2xl mx-5 mb-5">
          <div className="relative flex flex-col items-center justify-center px-4 pt-3 pb-3 overflow-hidden shrink-0 rounded-t-3xl"
            ...
            <h1 className="text-[13px] font-extrabold text-white tracking-wide relative z-10 text-center">Apartment Rent - Property Management</h1>
      ...
      <div className="fixed inset-0 bg-black/60 z-50 hidden sm:flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
          <div className="relative flex flex-col items-center justify-center min-h-[65px] px-4 pt-2.5 pb-2.5 overflow-hidden shrink-0 rounded-3xl"
            ...
            <button onClick={onClose} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/25 hover:bg-white/40 flex items-center justify-center z-10 text-white font-bold text-[11px]">✕</button>
            <div className="text-xl mb-0.5 relative z-10">🏢</div>
            <h1 className="text-[14px] font-extrabold text-white tracking-wide relative z-10">Apartment Rent - Property Management</h1>
...
// MOBILE CONTENT - RENT PM APARTMENT
function MobContentRentPMApart({ 
  ...
  furnishingOptions, facingOptions, tenantTypeOptions, 
  rentalDurationOptions, contactTimeOptions, apartmentRentAmenities, 
  toggleArrayItem, handleCoverImageUpload, handleFloorPlanUpload, 
  ...
  handlePANChange, handleIFSCChange, validateAccountNumber
}) {
  ...
  if (step === 0) return (
    <>
      ...
      <Field label="Years of Experience" required error={errors.pmYearsOfExperience}>
        <input className={`${inp} ${errors.pmYearsOfExperience ? errorBorder : ''}`} type="number" min="0" placeholder="Enter years of experience" value={formData.pmYearsOfExperience} onChange={(e) => updateForm("pmYearsOfExperience", handleNumericFieldChange(e.target.value))} />
      </Field>
  ...
  if (step === 5) return (
    <>
      ...
      <Field label="Monthly Rent (₹)" required error={errors.rent}>
        <input className={`${inp} ${errors.rent ? errorBorder : ''}`} type="text" placeholder="Enter monthly rent" value={formData.rent} onChange={(e) => updateForm("rent", handleNumericFieldChange(e.target.value))} />
      </Field>
      ...
      <Field label="Price Type" required error={errors.priceType}>
        <div className="flex gap-4">
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-priceType-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "fixed"} onChange={() => updateForm("priceType", "fixed")} />
            Fixed
          </label>
          <label className="flex items-center gap-1.5 text-[11px] cursor-pointer">
            <input type="radio" name="mob-priceType-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "negotiable"} onChange={() => updateForm("priceType", "negotiable")} />
            Negotiable
          </label>
        </div>
      </Field>
...
// DESKTOP CONTENT - RENT PM APARTMENT
function DtContentRentPMApart({ 
  step, inp, errorBorder, formData, errors, updateForm, 
  imagePreviews, handleImageUpload, removeImage, 
  handleVideoUpload, videoPreview, removeVideo, 
  handleDocumentUpload, toggleAmenity, customAmenitiesList, 
  ...
  if (step === 5) return (
    <>
      ...
      <FieldDt label="Monthly Rent (₹)" required error={errors.rent}>
        <input className={`${inp} ${errors.rent ? errorBorder : ''}`} type="text" placeholder="Enter monthly rent" value={formData.rent} onChange={(e) => updateForm("rent", handleNumericFieldChange(e.target.value))} />
      </FieldDt>
      ...
      <FieldDt label="Price Type" required error={errors.priceType}>
        <div className="flex gap-5">
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-priceType-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "fixed"} onChange={() => updateForm("priceType", "fixed")} />
            Fixed
          </label>
          <label className="flex items-center gap-2 text-[13px] cursor-pointer">
            <input type="radio" name="dt-priceType-rent" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.priceType === "negotiable"} onChange={() => updateForm("priceType", "negotiable")} />
            Negotiable
          </label>
        </div>
      </FieldDt>
