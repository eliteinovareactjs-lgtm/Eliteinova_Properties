...
export default function RentPMApartForm({ isOpen, onClose }) {
  ...
  const [formData, setFormData] = useState({
    ...
    // Property Details
    propertyType: "Apartment", listingType: "Property Management", purpose: "Rent",
  ...
  const handleSubmit = () => {
    console.log("Rent Property Management Apartment Form submitted:", formData);
    onClose();
  };
      ...
      <div className="fixed inset-0 z-50 flex flex-col sm:hidden">
        ...
        <div className="flex-1 bg-white rounded-3xl flex flex-col overflow-hidden shadow-2xl mx-5 mb-5">
          <div className="relative flex flex-col items-center justify-center px-4 pt-3 pb-3 overflow-hidden shrink-0 rounded-t-3xl"
            ...
            <h1 className="text-[13px] font-extrabold text-white tracking-wide relative z-10 text-center">Rent Apartment - Property Management</h1>
            <p className="text-[10px] text-white/80 relative z-10 mt-0.5 text-center">List apartment for rent by property management</p>
          </div>
      ...
      <div className="fixed inset-0 bg-black/60 z-50 hidden sm:flex items-center justify-center p-4">
        <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
          <div className="relative flex flex-col items-center justify-center min-h-[65px] px-4 pt-2.5 pb-2.5 overflow-hidden shrink-0 rounded-3xl"
            ...
            <button onClick={onClose} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-white/25 hover:bg-white/40 flex items-center justify-center z-10 text-white font-bold text-[11px]">✕</button>
            <div className="text-xl mb-0.5 relative z-10">🏢</div>
            <h1 className="text-[14px] font-extrabold text-white tracking-wide relative z-10">Rent Apartment - Property Management</h1>
            <p className="text-[10px] text-white/80 relative z-10 mt-0.5">List apartment for rent by property management</p>
          </div>
    ...
    <>
      ...
      <Field label="Number of Bedrooms">
        {["Studio", "1 BHK", "2 BHK", "3 BHK", "4 BHK+"].map(bhk => (
          <label key={bhk} className="flex items-center gap-2 text-[11px] mb-1 cursor-pointer">
            <input type="radio" name="mob-bhk-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.bedrooms === bhk} onChange={() => updateForm("bedrooms", bhk)} />
            {bhk}
          </label>
        ))}
      </Field>
      <Field label="Number of Bathrooms">
        {["1", "2", "3", "4+"].map(b => (
          <label key={b} className="flex items-center gap-2 text-[11px] mb-1 cursor-pointer">
            <input type="radio" name="mob-bath-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.bathrooms === b} onChange={() => updateForm("bathrooms", b)} />
            {b}
          </label>
        ))}
      </Field>
        ...
        <div className="grid grid-cols-2 gap-1">
          {facingOptions.map(f => (
            <label key={f} className="flex items-center gap-1 text-[10px] cursor-pointer">
              <input type="radio" name="mob-facing-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.facingDirection === f} onChange={() => updateForm("facingDirection", f)} />
              {f}
            </label>
          ))}
        </div>
        ...
        <div className="flex gap-4">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[11px] cursor-pointer">
              <input type="radio" name="mob-balcony-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.balcony === opt} onChange={() => updateForm("balcony", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <Field label="Corner Unit">
        <div className="flex gap-4">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[11px] cursor-pointer">
              <input type="radio" name="mob-corner-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.cornerUnit === opt} onChange={() => updateForm("cornerUnit", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <Field label="Current Status" required>
        {["Vacant", "Occupied", "Under Maintenance"].map(t => (
          <label key={t} className="flex items-center gap-2 text-[11px] mb-1 cursor-pointer">
            <input type="radio" name="mob-status-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.currentStatus === t} onChange={() => updateForm("currentStatus", t)} />
            {t}
          </label>
        ))}
      </Field>
      ...
      <Field label="Furnishing Status">
        <div className="grid grid-cols-2 gap-1">
          {furnishingOptions.map(f => (
            <label key={f} className="flex items-center gap-1 text-[10px] cursor-pointer">
              <input type="radio" name="mob-furnish-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.furnishing === f} onChange={() => updateForm("furnishing", f)} />
              {f}
            </label>
          ))}
        </div>
    ...
    <>
      ...
      <Field label="Maintenance Charges Included">
        <div className="flex gap-4">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[11px] cursor-pointer">
              <input type="radio" name="mob-maint-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.maintenanceIncluded === opt} onChange={() => updateForm("maintenanceIncluded", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <Field label="Rent Negotiable">
        <div className="flex gap-4">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[11px] cursor-pointer">
              <input type="radio" name="mob-negotiable-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.rentNegotiable === opt} onChange={() => updateForm("rentNegotiable", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <Field label="Pet Friendly">
        <div className="flex gap-4">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[11px] cursor-pointer">
              <input type="radio" name="mob-pet-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.petFriendly === opt} onChange={() => updateForm("petFriendly", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <Field label="Dietary Preference">
        <div className="flex gap-4">
          {["Veg Only", "No Restriction"].map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[11px] cursor-pointer">
              <input type="radio" name="mob-diet-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.dietaryPreference === opt} onChange={() => updateForm("dietaryPreference", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <Field label="Smoking Allowed">
        <div className="flex gap-4">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[11px] cursor-pointer">
              <input type="radio" name="mob-smoking-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.smokingAllowed === opt} onChange={() => updateForm("smokingAllowed", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <Field label="Immediate Move-in">
        <div className="flex gap-4">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[11px] cursor-pointer">
              <input type="radio" name="mob-immediate-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.immediateMoveIn === opt} onChange={() => updateForm("immediateMoveIn", opt)} />
              {opt}
            </label>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-1">
          {rentalDurationOptions.map(d => (
            <label key={d} className="flex items-center gap-1 text-[10px] cursor-pointer">
              <input type="radio" name="mob-duration-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.minimumRentalDuration === d} onChange={() => updateForm("minimumRentalDuration", d)} />
              {d}
            </label>
          ))}
        </div>
      ...
      <Field label="Contact via">
        <div className="flex gap-4">
          {["Owner", "Agent", "Builder", "Property Management"].map(opt => (
            <label key={opt} className="flex items-center gap-1.5 text-[11px] cursor-pointer">
              <input type="radio" name="mob-contact-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.contactVia === opt} onChange={() => updateForm("contactVia", opt)} />
              {opt}
            </label>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-1">
          {contactTimeOptions.map(t => (
            <label key={t} className="flex items-center gap-1 text-[9px] cursor-pointer">
              <input type="radio" name="mob-time-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.preferredContactTime === t} onChange={() => updateForm("preferredContactTime", t)} />
              {t}
            </label>
          ))}
        </div>
    ...
    <>
      ...
      <Field label="Property Images" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept="image/*" multiple className="hidden" id="m-imgs-rentpmapart" onChange={handleImageUpload} />
          <label htmlFor="m-imgs-rentpmapart" className="cursor-pointer flex flex-col items-center">
      ...
      <Field label="Property Video">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept="video/mp4,video/mov" className="hidden" id="m-vid-rentpmapart" onChange={handleVideoUpload} />
          <label htmlFor="m-vid-rentpmapart" className="cursor-pointer flex flex-col items-center">
  ...
  if (step === 4) return (
    <>
      ...
      <Field label="Property Ownership Document" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="m-ownership-rentpmapart" onChange={(e) => handleDocumentUpload("ownershipDoc", e)} />
          <label htmlFor="m-ownership-rentpmapart" className="cursor-pointer flex flex-col items-center">
      ...
      <Field label="ID Proof of Owner" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="m-ownerid-rentpmapart" onChange={(e) => handleDocumentUpload("ownerIdProof", e)} />
          <label htmlFor="m-ownerid-rentpmapart" className="cursor-pointer flex flex-col items-center">
            <div className="w-8 h-8 rounded-full bg-[#00695C] flex items-center justify-center mb-1">
              <Upload className="w-4 h-4 text-white" />
            </div>
      ...
      <Field label="Management Agreement/Authorization Letter" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-3 text-center hover:bg-green-50">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="m-agreement-rentpmapart" onChange={(e) => handleDocumentUpload("managementAgreement", e)} />
          <label htmlFor="m-agreement-rentpmapart" className="cursor-pointer flex flex-col items-center">
            <div className="w-8 h-8 rounded-full bg-[#00695C] flex items-center justify-center mb-1">
              <Upload className="w-4 h-4 text-white" />
            </div>
...
// DESKTOP CONTENT - Rent Property Management Apartment (follows same pattern as mobile with FieldDt)
function DtContentRentPMApart({ step, inp, formData, updateForm, imagePreviews, handleImageUpload, removeImage, handleVideoUpload, videoPreview, removeVideo, handleDocumentUpload, removeDocument, ownershipDocPreview, ownerIdProofPreview, managementAgreementPreview, toggleAmenity, toggleApartmentAmenity, toggleService, availableAmenities, availableServices, customAmenitiesList, customServicesList, addCustomAmenity, removeCustomAmenity, addCustomService, removeCustomService, yesNoOptions, furnishingOptions, facingOptions, tenantTypeOptions, rentalDurationOptions, contactTimeOptions, apartmentRentAmenities }) {
  const ta = `${inp} resize-y`;
    ...
    <>
        ...
        <div className="flex flex-wrap gap-3">
          {["Studio", "1 BHK", "2 BHK", "3 BHK", "4 BHK+"].map(bhk => (
            <label key={bhk} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-bhk-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.bedrooms === bhk} onChange={() => updateForm("bedrooms", bhk)} />
              {bhk}
            </label>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          {["1", "2", "3", "4+"].map(b => (
            <label key={b} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-bath-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.bathrooms === b} onChange={() => updateForm("bathrooms", b)} />
              {b}
            </label>
          ))}
        </div>
        ...
        <div className="grid grid-cols-4 gap-2">
          {facingOptions.map(f => (
            <label key={f} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-facing-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.facingDirection === f} onChange={() => updateForm("facingDirection", f)} />
              {f}
            </label>
          ))}
        </div>
        ...
        <div className="flex gap-5">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-balcony-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.balcony === opt} onChange={() => updateForm("balcony", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Corner Unit">
        <div className="flex gap-5">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-corner-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.cornerUnit === opt} onChange={() => updateForm("cornerUnit", opt)} />
              {opt}
            </label>
          ))}
        </div>
        ...
        <div className="flex gap-5">
          {["Vacant", "Occupied", "Under Maintenance"].map(t => (
            <label key={t} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-status-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.currentStatus === t} onChange={() => updateForm("currentStatus", t)} />
              {t}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Furnishing Status">
        <div className="flex flex-wrap gap-3">
          {furnishingOptions.map(f => (
            <label key={f} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-furnish-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.furnishing === f} onChange={() => updateForm("furnishing", f)} />
              {f}
            </label>
          ))}
        </div>
    ...
    <>
      ...
      <FieldDt label="Maintenance Charges Included">
        <div className="flex gap-5">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-maint-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.maintenanceIncluded === opt} onChange={() => updateForm("maintenanceIncluded", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Rent Negotiable">
        <div className="flex gap-5">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-negotiable-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.rentNegotiable === opt} onChange={() => updateForm("rentNegotiable", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Pet Friendly">
        <div className="flex gap-5">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-pet-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.petFriendly === opt} onChange={() => updateForm("petFriendly", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Dietary Preference">
        <div className="flex gap-5">
          {["Veg Only", "No Restriction"].map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-diet-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.dietaryPreference === opt} onChange={() => updateForm("dietaryPreference", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Smoking Allowed">
        <div className="flex gap-5">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-smoking-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.smokingAllowed === opt} onChange={() => updateForm("smokingAllowed", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Immediate Move-in">
        <div className="flex gap-5">
          {yesNoOptions.map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-immediate-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.immediateMoveIn === opt} onChange={() => updateForm("immediateMoveIn", opt)} />
              {opt}
            </label>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          {rentalDurationOptions.map(d => (
            <label key={d} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-duration-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.minimumRentalDuration === d} onChange={() => updateForm("minimumRentalDuration", d)} />
              {d}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Contact via">
        <div className="flex gap-5">
          {["Owner", "Agent", "Builder", "Property Management"].map(opt => (
            <label key={opt} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-contact-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.contactVia === opt} onChange={() => updateForm("contactVia", opt)} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <FieldDt label="Preferred Contact Time">
        <div className="grid grid-cols-2 gap-2">
          {contactTimeOptions.map(t => (
            <label key={t} className="flex items-center gap-2 text-[13px] cursor-pointer">
              <input type="radio" name="dt-time-rentpm" className="accent-[#00695C] w-3.5 h-3.5 cursor-pointer" checked={formData.preferredContactTime === t} onChange={() => updateForm("preferredContactTime", t)} />
              {t}
            </label>
          ))}
        </div>
    ...
    <>
      ...
      <FieldDt label="Property Images" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept="image/*" multiple className="hidden" id="property-images-rentpmapart" onChange={handleImageUpload} />
          <label htmlFor="property-images-rentpmapart" className="cursor-pointer flex flex-col items-center">
      ...
      <FieldDt label="Property Video">
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept="video/mp4,video/mov" className="hidden" id="property-video-rentpmapart" onChange={handleVideoUpload} />
          <label htmlFor="property-video-rentpmapart" className="cursor-pointer flex flex-col items-center">
  ...
  if (step === 4) return (
    <>
      ...
      <FieldDt label="Property Ownership Document" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="dt-ownership-rentpmapart" onChange={(e) => handleDocumentUpload("ownershipDoc", e)} />
          <label htmlFor="dt-ownership-rentpmapart" className="cursor-pointer flex flex-col items-center">
      ...
      <FieldDt label="ID Proof of Owner" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="dt-ownerid-rentpmapart" onChange={(e) => handleDocumentUpload("ownerIdProof", e)} />
          <label htmlFor="dt-ownerid-rentpmapart" className="cursor-pointer flex flex-col items-center">
            <div className="w-10 h-10 rounded-full bg-[#00695C] flex items-center justify-center mb-2">
              <Upload className="w-5 h-5 text-white" />
            </div>
      ...
      <FieldDt label="Management Agreement/Authorization Letter" required>
        <div className="border-2 border-dashed border-teal-300 rounded-xl p-4 text-center cursor-pointer hover:bg-green-50">
          <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" id="dt-agreement-rentpmapart" onChange={(e) => handleDocumentUpload("managementAgreement", e)} />
          <label htmlFor="dt-agreement-rentpmapart" className="cursor-pointer flex flex-col items-center">
            <div className="w-10 h-10 rounded-full bg-[#00695C] flex items-center justify-center mb-2">
              <Upload className="w-5 h-5 text-white" />
            </div>
